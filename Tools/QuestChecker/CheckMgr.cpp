...
namespace TextHelper
{
	...
	{
		BM::CAutoMutex kLock(m_kTextMutex);
		CONT_TEXT_TABLE::_Pairib kRet = kContText.insert( std::make_pair(iTextID, rkText) );
		return kRet.second;
	}
...
namespace ResultHelper
{
	...
	void CheckExistIDToFailList(CONT_NOT_EXIST_ID& List)
	{
		...
		{
			CONT_NOT_EXIST_ID::_Pairib Rst = List.insert(Fail_iter->first);
			if( !Rst.second )
			{
				//error
			}
	...
	void CheckExistIDToSuccessList(CONT_NOT_EXIST_ID& List)
	{
		...
		{
			CONT_NOT_EXIST_ID::_Pairib Rst = List.insert(Suc_iter->first);
			if( !Rst.second )
			{
				//error
			}
...
namespace WorldHelper
{
	...
	TiXmlElement const* AddNpc(TiXmlElement const* pkNode, int const iMapNo)
	{
		...
		PgStringUtil::ReplaceStr(kNpcName, std::wstring(L">"), std::wstring(L"&gt;"), kNpcName);
		{
			...
			CONT_NPC_GUID_INFO::_Pairib kRet = kNpcInfo.insert( std::make_pair(kNpcGuid, CONT_NPC_GUID_INFO::mapped_type()) );
