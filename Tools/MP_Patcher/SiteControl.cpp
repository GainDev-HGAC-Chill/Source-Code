		...
		if( strcmp(pcTagName, "SITECONTROL") == 0 )
		{
			...
			while( pkSubEle )
			{
				...
				if( strcmp(pcTagSubName, "BTN") == 0 || strcmp(pcTagSubName, "PREV_BTN") == 0 || strcmp(pcTagSubName, "NEXT_BTN") == 0 )
				{
					...
					std::pair<CONT_BUTTON::iterator, bool> Result = pkContainer->insert(std::make_pair(BtnInfo.ID, pkElement));
					if( Result.second )
					{	//이미지 정보를 불러온다
						CONT_BUTTON::mapped_type &kElement = Result.first->second;
						std::vector< char >	kData;
						BM::PgDataPackManager::LoadFromPack(DAT, BtnInfo.BgName.c_str(), kData);
				...
				else if( strcmp(pcTagSubName, "SITE") == 0 )
				{
					...
					std::pair<CONT_SITE_INFO::iterator, bool> Result = m_kSiteContainer.insert(std::make_pair(iIDX, SiteInfo));
					if( Result.second )
					{
