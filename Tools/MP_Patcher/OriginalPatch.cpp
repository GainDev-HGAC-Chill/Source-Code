...
namespace PgMergeUtil
{
	...
	{// lhs에
		...
		{// rkKey 폴더가 없으면, 추가하고
			std::pair<BM::FolderHash::iterator, bool> kRet = lhs.insert( std::make_pair(rkKey, rkVal) );
			if( !kRet.second )
			{
				::MessageBox(NULL, L"Memmory Error", L"Error", MB_OK);
		...
		{// rkKey 폴더가 이미 존재하면
			...
			{// rkKey 폴더에 존재하는 파일이
				...
				{// lhs에 존재 하지 않으면 추가하고
					std::pair<BM::FileHash::iterator, bool> kRet = rkLeft->kFileHash.insert( std::make_pair(rkFileKey, rkFileVal) );
					if( !kRet.second )
					{
						::MessageBox(NULL, L"Memmory Error2", L"Error", MB_OK);
						INFO_LOG( BM::LOG_LV5, _T("[%s]-[%d] throw failed\n\r"), _T(__FUNCTION__), __LINE__);
						DEBUG_BREAK;
						throw eTHROW_FAILED;
					}
