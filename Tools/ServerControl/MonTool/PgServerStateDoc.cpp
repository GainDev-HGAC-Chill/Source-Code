...
bool PgServerStateDoc::RefreshServerState( BM::CPacket& rkPacket )
{
	...
	while ( iSize-- )
	{
		...
		{
			CONT_CONTROL_INFO::_Pairib kPair = m_kControlInfo.insert( std::make_pair( iSiteNo, CONT_CONTROL_INFO::mapped_type()) );
			itr = kPair.first;
		}
		...
		for ( ; itr != m_kControlInfo.end() ; ++itr )
		{
			...
			for ( ; state_itr != kList.end() ; ++state_itr )
			{
				...
				case CEL::ST_CONTENTS:
					{
						std::map< short, SRealmUserRecord >::_Pairib kPair = kContRecord.insert( std::make_pair( state_itr->second.kTbl.nRealm, kTempRecord ) );
