...
public:
	...
	int				Append(const CMyArray<T>& e) {
		if(e.m_nCount == 0) return m_nCount;
	...
	void			FreeExtra()
	{
		if(!m_pBuf || m_nSize == m_nCount) return ;
...
//			SetSize(nIndex + nCount);
			...
			if((m_nCount + nCount) > m_nSize) {
				...
				memcpy(&p[nIndex + nCount], &m_pBuf[nIndex], sizeof(T) * (m_nSize - nIndex));
				delete[] (unsigned char*)m_pBuf;
				m_pBuf = p;
				...
				memmove(&m_pBuf[nIndex + nCount], &m_pBuf[nIndex], (m_nCount - nIndex) * sizeof(T));
	...
	void			RemoveAt(int nIndex, int nCount = 1)
	{
		...
		if(nMoveCount) {
			memmove( &((T*)m_pBuf)[nIndex], &((T*)m_pBuf)[nIndex + nCount], nMoveCount * sizeof(T));
		}
		m_nCount -= nCount;
	}
