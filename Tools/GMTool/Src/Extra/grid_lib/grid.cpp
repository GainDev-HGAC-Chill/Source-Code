...
//	m_hHorzSize = LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_HORZ));
//	m_hVertSize = LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_VERT));
//	m_hRight = LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_RIGHT));
//	m_hDown = LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_DOWN));
//	m_hCross = LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_CROSSALL));
...
void CGrid::Draw(HDC hdc, DWORD dwTick)
{
	RECT rc;
	GetClipBox(hdc, &rc);
	...
	DrawTopLeftHeaderCells(hdc, ptOrigin, &rcTopLeftHeader, &rcTopLeftHeaderPos, dwTick);

	m_nDrawedBodyRows = rcBody.bottom - rcBody.top + rcTopHeader.bottom - rcTopHeader.top;
	m_nDrawedBodyCols = rcBody.right - rcBody.left + rcLeftHeader.right - rcLeftHeader.left;
...
void CGrid::DrawTopHeaderCells(HDC hDC, POINT ptOrigin, RECT* pCells, RECT* pPos, DWORD dwTick)
{
	if(pCells->right < pCells->left || pCells->bottom < pCells->top) return ;
		...
		for(i = pCells->left; i < pCells->right; ++ i) {
			...
			if(pMerge) {
				if(pMerge->m_dwDrawTick < dwTick) {
					...
					DrawCell(hDC, rcMerge, pMerge->m_range.GetID().x, pMerge->m_range.GetID().y, dwTick);
...
void CGrid::DrawLeftHeaderCells(HDC hDC, POINT ptOrigin, RECT* pCells, RECT* pPos, DWORD dwTick)
{
	if(pCells->right < pCells->left || pCells->bottom < pCells->top) return ;
...
void CGrid::DrawTopLeftHeaderCells(HDC hDC, POINT ptOrigin, RECT* pCells, RECT* pPos, DWORD dwTick)
{
	if(pCells->right < pCells->left || pCells->bottom < pCells->top) return ;
...
void CGrid::DrawBodyCells(HDC hDC, POINT ptOrigin, RECT* pCells, RECT* pPos, DWORD dwTick)
{
	if(pCells->right < pCells->left || pCells->bottom < pCells->top) return ;
		...
		for(i = pCells->left; i <= pCells->right; ++ i) {
			...
			if(pMerge) {
				if(pMerge->m_dwDrawTick < dwTick) {
					...
					if(IsSelected(pMerge->m_range.GetID().x, pMerge->m_range.GetID().y)) {
...
void CGrid::GetDrawDetail(RECT* prcDraw, 
						  ...
						  RECT* pTopHeaderPos, RECT* pLeftHeaderPos, RECT* pTopLeftHeaderPos, RECT* pBodyPos)
{
	...
	SetRect(pTopLeftHeaderPos, ptHeaderPos.x, ptHeaderPos.y, ptBodyPos.x, ptBodyPos.y);
	...
	if(nTop >= m_nFixRowCount)
		...
		SetRect(pTopHeader, nLeft < m_nFixColCount + m_cellIndex.x ? m_nFixColCount + m_cellIndex.x : nLeft, nTop, nRight, m_nFixRowCount);

	if(nLeft >= m_nFixColCount)
		SetRect(pLeftHeader, 0, 0, 0, 0);
	else
		SetRect(pLeftHeader, nLeft, nTop < m_nFixRowCount + m_cellIndex.y ? m_nFixRowCount + m_cellIndex.y : nTop, m_nFixColCount, nBottom);

	
	SetRect(pTopLeftHeader, 
		nLeft, nTop, 
		nRight < m_nFixColCount ? nRight : m_nFixColCount, 
		nBottom < m_nFixRowCount ? nBottom : m_nFixRowCount);

	SetRect(pBody, 
		nLeft < m_nFixColCount + m_cellIndex.x ? m_nFixColCount + m_cellIndex.x : nLeft, 
		nTop < m_nFixRowCount + m_cellIndex.y ? m_nFixRowCount + m_cellIndex.y : nTop, 
		nRight, nBottom);
}

void CGrid::DrawSelectedBorder(HDC hdc, RECT &rc, int x, int y, CMergeCell* pMerge)
{
	return ;
	...
	if(m_selectRgn.GetSubRectCount() == 1) {
		...
		if(pMerge) {
			if(x + pMerge->m_range.GetCX() == m_selectRgn.GetSubRect(0)->Right()) {
...
{
	if(y >= m_nFixRowCount && y <= m_nLastRow + m_nFixRowCount && m_cellIndex.y != y - m_nFixRowCount) {
		y -= m_nFixRowCount;
		m_cellIndex.y = y;
...
{
	if(x >= m_nFixColCount && x <= m_nLastCol + m_nFixColCount && m_cellIndex.x != x - m_nFixColCount) {
		x -= m_nFixColCount;
		m_cellIndex.x = x;
...
void CGrid::OnUpKey(int nCtrl, int nShift)
{
	...
	if(nCtrl && !nShift) {
		...
		if(m_idClickDown.x < m_nFixColCount && m_idClickDown.y >= m_nFixRowCount) {				// click in left header
			if(m_idLastSelect.y > m_nFixRowCount) {
				...
				m_selectRgn.Init(m_nFixColCount, t, m_nCols - m_nFixColCount, h);
		...
		else if(m_idClickDown.x >= m_nFixColCount && m_idClickDown.y < m_nFixRowCount) {		// click in top header
		}
		else if(m_idClickDown.x < m_nFixColCount && m_idClickDown.y < m_nFixRowCount) {			// click in top left header
		}
		else {																					// click in body
			if(m_idLastSelect.y > m_nFixRowCount) {
				-- m_idLastSelect.y;
...
/*				CMergeCell* pMerge = m_merge.GetMergeCell(id.x, id.y);
				...
				if(m_idLastSelect.y < m_cellIndex.y + m_nFixRowCount && m_cellIndex.y > 0) {
					::SetScrollPos(m_hWnd, SB_VERT, -- m_cellIndex.y, TRUE);
					m_nYExcursion -= GetRowHeight(m_cellIndex.y + m_nFixRowCount);
				}
		...
		id = m_idFocus;
		-- id.y;
		if(id.y < m_nFixRowCount) id.y = m_nFixRowCount;
		...
		if(id.y < m_nFixRowCount + m_cellIndex.y - 1 || 
			...
			if(id.y > m_nLastRow + m_nFixRowCount) id.y = m_nLastRow + m_nFixRowCount;
			SetTopIndex(id.y);
		}
		else {
	...
	if(nCtrl && !nShift) {
		...
		if(m_idClickDown.x < m_nFixColCount && m_idClickDown.y >= m_nFixRowCount) {				// click in left header
			...
			if(m_idLastSelect.x > m_nFixColCount) {
				...
				m_selectRgn.Init(l, m_nFixRowCount, w, m_nRows - m_nFixRowCount);
...
/*				CMergeCell* pMerge = m_merge.GetMergeCell(id.x, id.y);
				...
				if(m_idLastSelect.x < m_cellIndex.x + m_nFixColCount && m_cellIndex.x > 0) {
					::SetScrollPos(m_hWnd, SB_HORZ, -- m_cellIndex.x, TRUE);
					m_nXExcursion -= GetColWidth(m_cellIndex.x + m_nFixColCount);
				}
		...
		id = m_idFocus;
		-- id.x;
		if(id.x < m_nFixColCount) id.x = m_nFixColCount;
		...
		if(id.x < m_nFixColCount + m_cellIndex.x - 1 || 
			...
			if(id.x > m_nLastCol + m_nFixColCount) id.x = m_nLastCol + m_nFixColCount;
			SetLeftIndex(id.x);
		}
		else {
...
void CGrid::OnRightKey(int nCtrl, int nShift)
{
	...
	CCellRange range;
	if(nCtrl && !nShift) {
		m_nXExcursion += GetBodyCellLeft(m_nLastCol + m_nFixColCount, m_nFixColCount + m_cellIndex.x);
		...
		if(m_idClickDown.x < m_nFixColCount && m_idClickDown.y >= m_nFixRowCount) {				// click in left header
			...
			if(m_idLastSelect.x < m_nCols - 1) {
				...
				if(m_idLastSelect.x > m_nFixColCount + m_cellIndex.x + m_nDrawedBodyCols - 1) {
					m_nXExcursion += GetColWidth(m_cellIndex.x + m_nFixColCount);
					::SetScrollPos(m_hWnd, SB_HORZ, ++ m_cellIndex.x, TRUE);
				}
				InvalidateRect(m_hWnd, NULL, TRUE);
			...
			if(m_idLastSelect.x < m_nCols - 1) {
				CMergeCell* pMerge = m_merge.GetMergeCell(m_idLastSelect.x, m_idLastSelect.y);
				if(pMerge) {
...
/*				pMerge = m_merge.GetMergeCell(id.x, id.y);
		...
		if(id.x >= m_nCols) id.x = m_nCols - 1;
		m_idLastSelect = m_idFocus = m_idClickDown = id;
		m_selectRgn.Init(id.x, id.y, 1, 1);
...
void CGrid::OnDownKey(int nCtrl, int nShift)
{
	...
	CCellRange range;
	if(nCtrl && !nShift) {
		m_nYExcursion += GetBodyCellTop(m_nLastRow + m_nFixRowCount, m_nFixRowCount + m_cellIndex.y);
		...
		if(m_idClickDown.x < m_nFixColCount && m_idClickDown.y >= m_nFixRowCount) {				// click in left header
			if(m_idLastSelect.y < m_nRows - 1) {
				...
				if(m_idLastSelect.y > m_nFixRowCount + m_cellIndex.y + m_nDrawedBodyRows - 1) {
					m_nYExcursion += GetRowHeight(m_cellIndex.y + m_nFixRowCount);
					::SetScrollPos(m_hWnd, SB_VERT, ++ m_cellIndex.y, TRUE);
				}
				InvalidateRect(m_hWnd, NULL, TRUE);
...
/*				pMerge = m_merge.GetMergeCell(id.x, id.y);
		...
		if(id.y >= m_nRows) id.y = m_nRows - 1;
		m_idLastSelect = m_idFocus = m_idClickDown = id;
		m_selectRgn.Init(id.x, id.y, 1, 1);
...
void CGrid::OnDelKey(int nCtrl, int nShift)
{
	...
	int nDelBottom = m_matrix.GetRows();
	CMyRect rcTmp;
	for(int i = 0; i < n; ++ i) {
...
{
	if(m_idLastSelect.y == m_nFixRowCount) return ;
...
{
	if(m_idLastSelect.y == m_nRows - 1) return ;

	CCellRange range;
	int nOldY = m_cellIndex.y;
	int n = m_nDrawedBodyRows - m_nFixRowCount;
	m_cellIndex.y += n;
	if(m_cellIndex.y > m_nLastRow) m_cellIndex.y = m_nLastRow;
	::SetScrollPos(m_hWnd, SB_VERT, m_cellIndex.y, TRUE);
...
{
	return m_matrix.GetElement(x, y);
}

void CGrid::InsertRow(int y, int cy)
{
	m_matrix.InsertRow(y, cy);
...
#ifdef PG_USE_CONST
void CGrid::SetCellConst(CellID id, const bool e)
{
	CMyVariant *pCell = m_matrix.GetElement(id.x, id.y);
	if(!pCell) {
		CGridCell v;
		pCell = m_matrix.SetElement(id.x, id.y, v);
	}

	pCell->SetConst(e);
}

bool CGrid::GetCellConst(CellID id)
{
	CMyVariant *pCell = m_matrix.GetElement(id.x, id.y);
	if(pCell)
		return pCell->GetConst();
	else
		return false;
}
#endif
...
/*	TCHAR sz[100];
	...
	::SendMessage(m_hWnd, WM_GRID_SELECTCHANGED, MAKEWPARAM(range.GetID().x, range.GetID().y), MAKELPARAM(range.GetCX(), range.GetCY()));
}

CGridColumn* CGrid::GetColumnInfo()
{
	return &m_columnInfo;
}
...
void CGrid::UnEditCell(bool bSave)
{
	...
	HWND hWnd = this->m_columnInfo.GetEditHandle(m_cellEditting.x - m_nFixColCount);
	if(!hWnd) return ;
...
HWND CGrid::EditCell(CellID id, UINT nChar)
{
	...
	VT_TYPE type = GetColumnType(id.x);
	if(type == MYVT_NONE) {
...
void DrawGrid(CGrid *pGrid, HDC hdc)
{
	...
	pGrid->Draw(dc.m_hdc, pGrid->GetNowTick());
}
...
void CGrid::OnSize(UINT nType, int cx, int cy)
{
	...
	POINT pt = GetOriginPos();
	CalcVInfo(rc.bottom - pt.y);
	if(m_nLastRow == 0) m_cellIndex.y = 0;
	::SetScrollRange(m_hWnd, SB_VERT, 0, m_nLastRow, TRUE);

	CalcHInfo(rc.right - pt.x);
	if(m_nLastCol == 0) m_cellIndex.x = 0;
...
void CGrid::OnVScroll(UINT nSBCode, UINT nPos, HWND hScrollBar)
{
	...
	case SB_PAGEUP:
		if(m_cellIndex.y > 0) {
			...
			m_cellIndex.y -= 10;
			if(m_cellIndex.y < 0) m_cellIndex.y = 0;
...
void CGrid::OnHScroll(UINT nSBCode, UINT nPos, HWND hScrollBar)
{
	...
	case SB_PAGELEFT:
		if(m_cellIndex.x > 0) {
			...
			m_cellIndex.x -= 10;
			if(m_cellIndex.x < 0) m_cellIndex.x = 0;
	...
	case SB_PAGERIGHT:
		if(m_cellIndex.x < m_nLastCol) {
			j = m_cellIndex.x;
			m_cellIndex.x += 10;
			if(m_cellIndex.x > m_nLastCol) m_cellIndex.x = m_nLastCol;
...
void CGrid::OnLButtonDown(UINT nFlags, POINT point)
{
	...
	if(m_resizeType == vert) {
		...
		DrawResizeLine(hdc, ptOrigin.x, m_nResizePos, rcClient.right, m_nResizePos);
		::ReleaseDC(m_hWnd, hdc);
		...
		RECT rcClient;
		::GetClientRect(m_hWnd, &rcClient);
		DrawResizeLine(hdc, m_nResizePos, ptOrigin.y, m_nResizePos, rcClient.bottom);
		::ReleaseDC(m_hWnd, hdc);
	...
	if(id.x < m_nFixColCount && id.y >= m_nFixRowCount) {				// click in left header
		...
		if(!nCtrl && nShift) {
			...
			m_selectRgn.Init(rcSelect.Left(), rcSelect.Top(), rcSelect.Width(), rcSelect.Height());

			range.Set(rcSelect.Left(), rcSelect.Top(), rcSelect.Width(), rcSelect.Height());
			OnSelectChanged(range);
		}
		else if(nCtrl && !nShift) {
	...
	else if(id.x >= m_nFixColCount && id.y < m_nFixRowCount) {			// click in top header
	...
	else if(id.x < m_nFixColCount && id.y < m_nFixRowCount) {			// click in top left header
		rcSelect.Set(m_nFixColCount, m_nFixRowCount, m_nCols - m_nFixColCount, m_nRows - m_nFixRowCount);
		m_selectRgn.Init(rcSelect.Left(), rcSelect.Top(), rcSelect.Width(), rcSelect.Height());
...
void CGrid::OnLButtonUp(UINT nFlags, POINT point)
{
	...
	VT_TYPE type = this->GetColumnType(id.x);
...
{
	POINT pt = GetOriginPos();
	if(point.x <= pt.x || point.y <= pt.y) return ;
...
TRY_Y:
			...
			if(m_resizeType == vert) {
				...
				RECT rcClient;
				::GetClientRect(m_hWnd, &rcClient);
				DrawResizeLine(hdc, pt.x, m_nResizePos, rcClient.right, m_nResizePos);
				...
				RECT rcClient;
				::GetClientRect(m_hWnd, &rcClient);
				DrawResizeLine(hdc, m_nResizePos, pt.y, m_nResizePos, rcClient.bottom);
	...
	if(m_bLButtonDown) {
		if(m_idLastSelect.x < m_nFixColCount && m_idLastSelect.y < m_nFixRowCount)
			return ;
		...
		if(PtInRect(&rc, point)) {
			...
			if(id.x != m_idLastSelect.x || id.y != m_idLastSelect.y) {
				...
				if(m_idClickDown.x < m_nFixColCount) {					// move in left head
					l = m_nFixColCount;
					t = min(m_idFocus.y, m_idLastSelect.y);
					w = m_nCols - l;
					h = max(m_idFocus.y, m_idLastSelect.y) - t + 1;
				}
				else if(m_idClickDown.y < m_nFixRowCount) {			// move in top head
			...
			if(!m_bTimerOn) {
				m_bTimerOn = true;
				::SetTimer(m_hWnd, SELECT_TIMER_ID, SELECT_TIMER_INTERVAL, NULL);
			}
		}
	}
}
...
void CGrid::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if(nChar != VK_ESCAPE && nChar != VK_RETURN) {
		if(nChar == VK_SPACE) {
			VT_TYPE type = GetColumnType(m_idFocus.x);
...
LRESULT CALLBACK GridWindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	if(pGrid) {
		...
		case WM_SIZE:
			...
			break;
		case WM_VSCROLL:
			pGrid->OnVScroll((int)LOWORD(wParam), (short int) HIWORD(wParam), (HWND) lParam);
			break;
		case WM_HSCROLL:
			pGrid->OnHScroll((int)LOWORD(wParam), (short int) HIWORD(wParam), (HWND) lParam);
			break;
		case WM_LBUTTONDOWN:
			SetFocus(pGrid->m_hWnd);
		...
		case WM_RBUTTONDOWN:
			...
			break;
		case WM_KEYDOWN:
			pGrid->OnKeyDown((UINT)wParam, (UINT)(LOWORD(lParam)), (UINT)(HIWORD(lParam)));
			break;
		case WM_CHAR:
			pGrid->OnChar((UINT)wParam, (UINT)(LOWORD(lParam)), (UINT)(HIWORD(lParam)));
			break;
		case WM_MOUSEMOVE:
			pt.x = (int)(short)LOWORD(lParam);
			pt.y = (int)(short)HIWORD(lParam);
