...
#ifdef PG_USE_CONST
void CMyVariant::SetConst(bool bConst)
{
	m_bConst = bConst;
}

bool CMyVariant::GetConst()
{
	return m_bConst;
}
#endif
...
VT_TYPE	CMyVariant::GetType(TCHAR* psz)
{
	...
	if( (psz[0] < num_first || psz[0] > num_last) && psz[0] != dot && psz[0] != plus && psz[0] != sub)
		return MYVT_PTCHAR;
	else {
		bool bDot = false;
		int nPos = 0;
		...
		for(int i = nPos; i < n; ++ i) {
			if( ((psz[i] < num_first || psz[i] > num_last) && psz[i] != dot) || (bDot && psz[i] == dot))
				return MYVT_PTCHAR;
			if(psz[i] == dot)
				bDot = true;
		}
