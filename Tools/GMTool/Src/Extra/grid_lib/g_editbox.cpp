...
CGridEditBox::CGridEditBox()
{
	if(!lpfnWndProc) {
		WNDCLASS wc;
		if(!GetClassInfo(NULL, _TEXT("EDIT"), &wc)) throw 1;
...
HWND CGridEditBox::CreateEdit(DWORD dwStyle, int x, int y, int cx, int cy, HWND hWndParent, HINSTANCE hInstance)
{
	m_hWnd = CreateWindow(GRID_EDITBOX_CLASSNAME, NULL, dwStyle | ES_AUTOVSCROLL | ES_AUTOHSCROLL | ES_MULTILINE, x, y, cx, cy, hWndParent, NULL, hInstance, NULL);
	if(m_hWnd) {
...
LRESULT	CALLBACK CGridEditBox::EditWindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	POINT pt;
	CGridEditBox *pEditBox = (CGridEditBox*)GetWindowLong(hWnd, GWL_USERDATA);
	switch(msg) {
	case WM_KEYDOWN:
		if(!pEditBox->OnKeyDown((UINT)wParam, (UINT)(LOWORD(lParam)), (UINT)(HIWORD(lParam))))
			return 0;
		break;
	case WM_KEYUP:
		if(!pEditBox->OnKeyUp((UINT)wParam, (UINT)(LOWORD(lParam)), (UINT)(HIWORD(lParam))))
			return 0;
		break;
	case WM_CHAR:
		if(!pEditBox->OnChar((UINT)wParam, (UINT)(LOWORD(lParam)), (UINT)(HIWORD(lParam))))
			return 0;
		break;
	case WM_SYSCOMMAND:
...
bool CGridEditBox::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	else if(nChar == VK_LEFT || nChar == VK_UP || nChar == VK_DOWN || nChar == VK_RIGHT) {
		::SendMessage(::GetParent(m_hWnd), WM_EDITED, 0, 0);
		::SetFocus(::GetParent(m_hWnd));
		::PostMessage(::GetParent(m_hWnd), WM_KEYDOWN, (WPARAM)nChar, MAKELPARAM(nRepCnt, nFlags));
		return false;
	}
...
void CGridEditBox::OnSysCommand(UINT uCmdType, POINT pt)
{
	if(uCmdType == SC_KEYMENU) {
		::SendMessage(m_hWnd, EM_REPLACESEL, (WPARAM)TRUE, (LPARAM)_TEXT("\r\n"));
	}
}
/*
