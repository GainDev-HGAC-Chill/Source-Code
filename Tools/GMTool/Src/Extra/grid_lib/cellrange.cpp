...
{
	if(id.x >= m_cellID.x && id.y >= m_cellID.y && 
		id.x < (m_cellID.x + cx) && id.y < (m_cellID.y + cy)) return true;
	return false;
}

bool CCellRange::IsCellIn(int x, int y)
{
	if(x >= m_cellID.x && y >= m_cellID.y && 
		x < (m_cellID.x + cx) && y < (m_cellID.y + cy)) return true;
	return false;
}
