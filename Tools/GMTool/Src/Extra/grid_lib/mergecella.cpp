		...
		if(pMergeCell) {
			...
			for(j = 0; j < rcRange.GetCY(); ++ j) {
				for(i = 0; i < rcRange.GetCX(); ++ i) {
					pvt = m_mergeCells.GetElement(rcRange.GetID().x + i, rcRange.GetID().y + j);
					if(pvt) {
						pvt->Clear();
					}
				}
...
CMergeCell* CMergeCellA::MergeCells(int x, int y, int cx, int cy, RECT* pRectPos)
{
...
void CMergeCellA::InsertRow(int y, int cy)
{
	...
	for(int i = 0; i < n; ++ i) {
		...
		if(nTopIndex >= y) nTopIndex += cy;
		if(nBottomIndex > y) nBottomIndex += cy;
		

		if(nBottomIndex > y) {
			if(nTopIndex < y) {
				pMerge->m_range.Set(pMerge->m_range.GetID().x, nTopIndex, 
					pMerge->m_range.GetCX(), pMerge->m_range.GetCY() + cy);
				...
				for(int i = 0; i < cy; ++ i) {
					...
					for(int j = pMerge->m_range.GetID().x; j < pMerge->m_range.GetRight(); ++ j) {
						pLine->SetAt(j, vt);
					}
				}
			}
				...
				pMerge->m_range.Set(pMerge->m_range.GetID().x, nTopIndex, 
					pMerge->m_range.GetCX(), pMerge->m_range.GetCY());
...
void CMergeCellA::RemoveRow(int y, int cy)
{
	...
	for(int i = 0; i < n; ++ i) {
		...
		if(nBottomIndex > y) {
			if(nTopIndex < y) {
				pMerge->m_range.Set(pMerge->m_range.GetID().x, pMerge->m_range.GetID().y, 
					pMerge->m_range.GetCX(), 
					(pMerge->m_range.GetBottom() >= y + cy) ? 
					(pMerge->m_range.GetCY() - cy) : 
					(y - pMerge->m_range.GetID().y));
			}
			else if(nTopIndex >= y && nTopIndex < y + cy && nBottomIndex > y + cy) {
				...
				pMerge->m_range.Set(pMerge->m_range.GetID().x, y, 
					pMerge->m_range.GetCX(), pMerge->m_range.GetBottom() - y - cy);

				m_matrixCellID[i].y = y;
				
			}
			else if(nTopIndex >= y + cy) {
				pMerge->m_range.Set(pMerge->m_range.GetID().x, pMerge->m_range.GetID().y - cy, 
					pMerge->m_range.GetCX(), pMerge->m_range.GetCY());
				m_matrixCellID[i].y -= cy;
			}
			else {
...
void CMergeCellA::InsertCol(int x, int cx)
{
	...
	for(int i = 0; i < n; ++ i) {
		...
		if(nLeftIndex >= x) nLeftIndex += cx;
		if(nRightIndex > x) nRightIndex += cx;
		

		if(nRightIndex > x) {
			if(nLeftIndex < x) {
				pMerge->m_range.Set(nLeftIndex, pMerge->m_range.GetID().y, 
					pMerge->m_range.GetCX() + cx, pMerge->m_range.GetCY());

				CMyVariant vt;
				vt = pMerge;

				for(int i = pMerge->m_range.GetID().y; i < pMerge->m_range.GetBottom(); ++ i) {
...
void CMergeCellA::RemoveCol(int x, int cx)
{
	...
	for(int i = 0; i < n; ++ i) {
		...
		if(nRightIndex > x) {
			...
			else if(nLeftIndex >= x && nLeftIndex < x + cx && nRightIndex > x + cx) {
				m_mergeCells.GetElement(pMerge->m_range.GetID().x, pMerge->m_range.GetID().y)->bRelease = false;
				m_mergeCells.GetElement(x + cx, pMerge->m_range.GetID().y)->bRelease = true;

				pMerge->m_range.Set(x, pMerge->m_range.GetID().y, 
					pMerge->m_range.GetRight() - x - cx, pMerge->m_range.GetCY());

				m_matrixCellID[i].x = x;
				
			}
			else if(nLeftIndex >= x + cx) {
				pMerge->m_range.Set(pMerge->m_range.GetID().x - cx, pMerge->m_range.GetID().y, 
					pMerge->m_range.GetCX(), pMerge->m_range.GetCY());
				m_matrixCellID[i].x -= cx;
			}
			else {
