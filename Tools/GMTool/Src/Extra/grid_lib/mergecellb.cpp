...
CMergeCell* CMergeCellB::MergeCells(int x, int y, int cx, int cy, RECT* pRectPos)
{
...
void CMergeCellB::InsertRow(int y, int cy)
{
	...
	for(int i = 0; i < n; ++ i) {
		...
		if(nBottom > y) {
			...
			pRange->Set(pRange->GetID().x, nTop, pRange->GetCX(), nBottom - nTop);
		}
	}
}
...
void CMergeCellB::InsertCol(int x, int cx)
{
	...
	for(int i = 0; i < n; ++ i) {
		...
		if(nRight > x) {
			...
			pRange->Set(nLeft, pRange->GetID().y, nRight - nLeft, pRange->GetCY());
		}
	}
}
