...
//
LPTOP_LEVEL_EXCEPTION_FILTER MSJExceptionHandler::m_pPreviousFilter;
_se_translator_function MSJExceptionHandler::mPrevTranslatorFunc;
...
//	assert(g_szLogPath);
	...
	nNumberOfBytes += _stprintf_s(szBuffer+nNumberOfBytes, EXCEPTIONHANDLER_BUFFSIZE-nNumberOfBytes,
			...
			_T("1.0"), GetCurrentProcessId(), GetCurrentThreadId(),
...
#define EXCEPTION(x) case EXCEPTION_##x: return _T("EXCEPTION_")_T(#x);
