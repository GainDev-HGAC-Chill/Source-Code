...
void PgVariables< T_KEY >::Add( PgVariables< T_KEY > const &rhs )
{
	...
	{
		std::pair<CONT_VARIABLES::iterator, bool> kPair = m_kContVariable.insert( std::make_pair( itr->first, itr->second ) );
		if ( !kPair.second )
		{
...
template< typename T_KEY >
void PgVariables< T_KEY >::WriteToPacket( Stream &kPacket )const
{
	_Traits_MarshallingPacket< CONT_VARIABLE >::WriteToPacket( kPacket, m_kContVariable );
}

template< typename T_KEY >
bool PgVariables< T_KEY >::ReadFromPacket( Stream &kPacket )
{
	m_kContVariable.clear();
	return _Traits_MarshallingPacket< CONT_VARIABLE >::ReadFromPacket( kPacket, m_kContVariable );
}
...
HRESULT PgVariableMgr< T_GROUPKEY, T_KEY >::Set( T_GROUPKEY const &kGroupKey, T_KEY const &kKey, T const &kElement )
{
	...
	{
		std::pair<CONT_VARIABLES::iterator, bool> kPair = m_kContVariables.insert( std::make_pair( kGroupKey, PgVariables<T_KEY>() ) );
...
void PgVariableMgr< T_GROUPKEY, T_KEY >::Add( PgVariableMgr< T_GROUPKEY, T_KEY > const &rhs )
{
	...
	{
		std::pair<CONT_VARIABLES::iterator, bool> kPair = m_kContVariables.insert( std::make_pair( itr->first, itr->second ) );
		if ( !kPair.second )
		{
...
template< typename T_GROUPKEY, typename T_KEY >
void PgVariableMgr< T_GROUPKEY, T_KEY >::WriteToPacket( Stream &kPacket )const
{
	_Traits_MarshallingPacket< CONT_VARIABLES >::WriteToPacket( kPacket, m_kContVariables );
}

template< typename T_GROUPKEY, typename T_KEY >
bool PgVariableMgr< T_GROUPKEY, T_KEY >::ReadFromPacket( Stream &kPacket )
{
	m_kContVariables.clear();
	return _Traits_MarshallingPacket< CONT_VARIABLES >::ReadFromPacket( kPacket, m_kContVariables );
}
