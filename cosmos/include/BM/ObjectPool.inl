...
/*
	...
	if(wait_itor != m_kContWait.end())
	{
		...
		std::pair<POOL_OBJ_HASH::iterator, bool> out_ret = m_kContOut.insert( std::make_pair(pObj,0));
		if(!out_ret.second)
		{
...
/*
	...
	if(out_itor != m_kContOut.end())//나간거 검색.
	{
		...
		std::pair<POOL_OBJ_HASH::iterator, bool> wait_ret = m_kContWait.insert( std::make_pair(pObj,0));
		if(!wait_ret.second)
		{
