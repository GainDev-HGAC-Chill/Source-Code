		...
		if( arr_size )
		{
			if( RemainSize() >= arr_size*(sizeof(T_KEY)+sizeof(T_VALUE)) )
			{
				while( arr_size )
				{
					...
					std::pair<std::map< T_KEY, T_VALUE >::iterator, bool> ret = out.insert( std::make_pair( first, second ) );
		...
		if( arr_size )
		{
			if( RemainSize() >= arr_size*(sizeof(T_KEY)+sizeof(T_VALUE)) )
			{
				while( arr_size )
				{
					...
					std::pair<std::unordered_map< T_KEY, T_VALUE, T_HASHER >::iterator, bool> ret = out.insert( std::make_pair( first, second ) );
