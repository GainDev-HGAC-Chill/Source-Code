...
namespace BM
{
	...
	public:
		...
		{
			std::pair<CONT_VARIABLE::iterator, bool> kPair = m_kContVariable.insert( std::make_pair( kKey, kElement ) );
			if ( !kPair.second )
			{
		...
		void WriteToPacket( Stream &kPacket )const;
		bool ReadFromPacket( Stream &kPacket );
