...
bool PgGlobalPartyMgrImpl::AddCharToParty(BM::GUID const &rkCharGuid, BM::GUID const &rkPartyGuid)
{
	...
	std::pair<ContCharToParty::iterator, bool> kRet = m_kCharToParty.insert( std::make_pair(rkCharGuid, rkPartyGuid) );
	return kRet.second;
}
	...
	case PT_T_T_REQ_JOIN_OTHER_CHANNEL_PARTY:
		{
			...
			{// 파티 마스터가 파티에 속해 있는지 검사
				std::pair<ContOtherChannelJoinInfo::iterator, bool> Ret = m_ContOtherChannelJoinInfo.insert( std::make_pair( UserInfo.kCharGuid, JoinInfo) );
				if( !Ret.second )
				{	// 이미 있으면.
...
void PgGlobalPartyMgrImpl::SendToLocalPartyMgr( PgGlobalParty const *pkParty, BM::Stream const& rkPacket )
{
	...
	while(kVec.end() != iter)
	{
		...
		{
			std::pair<CheckOverlap::iterator, bool> bRet = kCheck.insert( kPartyMember.kChnGndKey );
			if( bRet.second )
			{
...
void PgGlobalPartyMgrImpl::SendToLocalPartyMgr(PgGlobalExpedition const * pExpedition, const BM::Stream & Packet)
{
	...
	while( iter != Vec_Guid.end() )
	{
		...
		{
			std::pair<CheckOverlap::iterator, bool> bRet = Check.insert( PartyMember.kChnGndKey );
			if( bRet.second )
			{
		...
		if( pkParty )
		{
			...
			if( S_OK == ::GetPlayerByGuid(kMasterGuid, false, kUser) )
			{
				...
				if( (find_iter->second.iAttr & GKIND_MISSION)
					...
					|| (find_iter->second.iAttr & GKIND_SUPER)
					|| (find_iter->second.iAttr & GATTR_FLAG_STATIC_DUNGEON) )
				{
					return EPS_RUNNING;
				}
				else if((find_iter->second.iAttr & GKIND_HARDCORE) )
				{
					return EPS_REFUSE;
				}
				else
...
HRESULT PgGlobalPartyMgrImpl::CreateFindParty(SContentsUser const& rkChar, std::wstring& rkContents)
{
	...
	std::pair<FindPartyUserInfo::iterator, bool> kRet = m_kContFindPartyUserInfo.insert(std::make_pair(rkChar.kCharGuid, pkNewFindPartyUser));
	if( !kRet.second )
	{
...
HRESULT PgGlobalPartyMgrImpl::ReqJoinExpedition(SContentsUser const & rkMaster, SContentsUser const &rkUser, BM::GUID &rkExpeditionGuid, BM::Stream &rkPacket)
{
	...
	{
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return ") << hCheck );
		return hCheck;//실패
	}
...
bool PgGlobalPartyMgrImpl::AddCharToExpedition(BM::GUID const & rkCharGuid, BM::GUID const & rkExpeditionGuid)
{
	...
	std::pair<ContCharToExpedition::iterator, bool> kRet = m_kCharToExpedition.insert(std::make_pair(rkCharGuid, rkExpeditionGuid));
	return kRet.second;	
}
...
bool PgGlobalPartyMgrImpl::AddPartyToExpedition(BM::GUID const & rkPartyGuid, BM::GUID const & rkExpeditionGuid)
{
	...
	std::pair<ContPartyToExpedition::iterator, bool> kRet = m_kPartyToExpedition.insert(std::make_pair(rkPartyGuid, rkExpeditionGuid));
	return kRet.second;
}
