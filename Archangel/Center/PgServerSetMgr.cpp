	...
	default:
		{
			...
			// 맵 진입 실패 시, 원정대 로비맵이나 원정대 던전에 있었는지 확인해야 된다.
			// 원정대 속성을 가진 맵이었을 경우에는 일반채널에는 맵이 연결되어 있지 않아서 접속할 수가 없다.
			...
			if( pContMap )
			{
				...
				if( map_iter != pContMap->end() )
				{
					...
					if( (DefMap.iAttr & GKIND_EXPEDITION)
						|| (DefMap.iAttr & GKIND_EXPEDITION_LOBBY) )	// 원정대 속성을 가진 맵인 경우.
					{	
						UNIT_PTR_ARRAY::const_iterator unit_itor = rkUnitArray.begin();
						if( unit_itor != rkUnitArray.end() )
						{
							...
							if( pPlayer )
							{
								SRecentPlayerNormalMapData MapData;		// 이동하려는 유저캐릭터가 가지고 있는 일반 맵정보를 가져온다.
								pPlayer->GetRecentNormalMap(MapData);
								SGroundKey CasterKey(MapData.GroundNo);

								if( S_OK == GetServerIdentity(CasterKey, rkRMM.kTargetSI ) )	// 다시 맵 진입 시도.
								{
