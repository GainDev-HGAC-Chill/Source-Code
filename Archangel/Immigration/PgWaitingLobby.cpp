		...
		if( S_OK == g_kProcessCfg.Locked_GetServerInfo(CEL::ST_CONTENTS, kContServer) )//센터의 IDENTITY 를 받아와야함.
		{
			...
			{		
				...
				if(kSI.nRealm == rkAuthInfo.RealmNo())
				{
					...
					std::pair<CONT_SWITCH_WAIT::iterator, bool> ret = m_kContSwitchWait.insert( std::make_pair(kRSRM.guidMember, kRSRM) );//! 스위치 웨이팅에 등록
					if( ret.second )
...
HRESULT PgWaitingLobby::AddLoginedUser(SSwitchPlayerData const &kSPD, bool bRealLogin)
{
	...
	if(pData)
	{
		const std::pair<CONT_CENTER_PLAYER_BY_ID::iterator, bool> ret = m_kContLoginedUserID.insert( std::make_pair( kSPD.ID(), pData) );
		if( ret.second )//넣기 성공
		{
			const std::pair<CONT_CENTER_PLAYER_BY_KEY::iterator, bool> ret2 = m_kContLoginedUserKey.insert( std::make_pair( kSPD.guidMember, pData) );
...
{
	std::pair<CONT_WAIT_AUTH::iterator, bool> kPair = m_kContAuthWait.insert( std::make_pair( kKey, kElement ) );
