	...
	if ( TRUE == ::PathFileExists(lpFileName) )
	{
		...
		if( ::GetPrivateProfileString( _T("USER_INFO"), _T("MAX_USER"), NULL, chValue, MAX_PATH, lpFileName ) > 0 )
		{
			...
			for(int i = 0; i < iUserCount; ++i )
			{
				...
				std::pair<CONT_MON_TOOL_SESSION::iterator, bool> kPair = m_kUserAuthInfo.insert( std::make_pair(kAuthInfo.kID, SMonToolInfo(kAuthInfo)) );
			}
...
bool PgMMCManager::Locked_LoadDB( int const iDBIndex )
{
	...
	{
		BM::CAutoMutex kLock(m_kMutex);
		std::pair<CONT_SITE::iterator, bool> kPair = m_kContSite.insert( std::make_pair( iDBIndex, pkControl ) );
		if ( !kPair.second )
		{
...
bool PgMMCManager::ReadIni_Path( CEL::E_SESSION_TYPE const &eST, std::wstring const &strCategory, std::wstring const &strFileName )
{
	...
	{
		std::pair<CON_SYNC_PATH::iterator, bool> ret = m_kSyncPath.insert( std::make_pair(eST, SGameServerPath(szSrcFolder, szExe)) );
		return ret.second;
	}
		...
		)
	{
		...
		if ( pkControl )
		{
			...
			while(rkResult.vecArray.end() != itr)
			{
				...
				std::pair<CONT_REALM_CANDIDATE::iterator, bool> kPair = kRealmCandi.insert( std::make_pair( nRealmNo,  SRealmElement(nRealmNo, strRealmName, strRealmTblName, strNotice_TblName) ) );
				if ( !kPair.second )
				{
...
void PgMMCManager::LoadPatchFileList( CON_SYNC_PATH const &kSyncPath, CONT_PATCH_FILELIST &rkOutPatchFileList )
{
	...
	{
		std::pair<CONT_PATCH_FILELIST::iterator, bool> kPair = rkOutPatchFileList.insert( std::make_pair( path_Itr->second.kSrcPath, CONT_PATCH_FILELIST::mapped_type() ) );
