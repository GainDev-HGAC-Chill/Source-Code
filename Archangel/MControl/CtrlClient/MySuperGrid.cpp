...
void CMySuperGrid::InitializeGrid()
{
	...
	*/
	...
	LPTSTR lpszCols[] = {_T("SERVERS"),_T("Name"),_T("ID"),_T("IP"),_T("Port"),_T("Status"), _T("Users"), 0};
	int iColumnWidth[] = { 200, 150, 50, 100, 80, 120, 50 };
	LV_COLUMN   lvColumn;
    ...
    {
		lvColumn.cx = iColumnWidth[x];
		lvColumn.pszText = lpszCols[x];
		InsertColumn(x,&lvColumn);
    }

	ObjectMapIterLoc<int, PgChannel*> kChannel;
	g_kChannelMgr.GetFirstChannel(kChannel);
	while (g_kChannelMgr.GetNextChannel(kChannel))
	{
		CItemInfo* lp = new CItemInfo();
		lp->SetImage(4);
		TCHAR chText[100];
		_stprintf_s(chText, 100, _T("[Channel %04d]"), kChannel.m_kItor->second->GetChannelNo());
		lp->SetItemText(chText);
		lp->AddSubItemText(_T(""));
		_stprintf_s(chText, 100, _T("%d"), kChannel.m_kItor->second->GetChannelNo());
		lp->AddSubItemText(chText);

		CTreeItem * pRoot = InsertRootItem(lp);

		ObjectMapIterLoc<int, ERIC_SERVERINFO*> kServer;
		kChannel.m_kItor->second->GetFirstServer(kServer);
		while (kChannel.m_kItor->second->GetNextServer(kServer))
		{
			AddServer(pRoot, kServer.m_kItor->second);
		}

		int iScroll = 0;
		ExpandAll(pRoot, iScroll);
	}
...
bool CMySuperGrid::AddServer(CTreeItem* pkRoot, ERIC_SERVERINFO* pkInfo)
{
	CString strNode;
	switch (pkInfo->kInfo.nServerType)
	{
	case CEL::ST_CENTER:
		strNode = _T("CENTER");
		break;
	...
	CTreeItem* pkFind = SearchEx(pkRoot, strNode);
	if (pkFind == NULL)
	{
		CItemInfo* pkNew = new CItemInfo();
		pkNew->SetItemText((LPCTSTR)strNode);
		pkFind = InsertItem(pkRoot, pkNew);
	}

	TCHAR chText[100];
	CItemInfo* pkServerNode = new CItemInfo();
	pkServerNode->SetItemText(pkInfo->kInfo.szName);
	pkServerNode->AddSubItemText(pkInfo->kInfo.szName);
	_stprintf_s(chText, 100, _T("%d"), pkInfo->kInfo.iServerNo);
	pkServerNode->AddSubItemText(chText);
	pkServerNode->AddSubItemText(pkInfo->kInfo.addrServerBind.StrIP().c_str());
	_stprintf_s(chText, 100, _T("%hd"), pkInfo->kInfo.addrServerBind.wPort);
	pkServerNode->AddSubItemText(chText);
	GetServerStatusText(pkInfo->eStatus, chText, 100);
	//pkServerNode->AddSubItemText(_T("unknown"));
	pkServerNode->SetSubItemText(4, chText, GetServerStatusImageIndex(pkInfo->eStatus));
	if (pkInfo->kInfo.nServerType == CEL::ST_MAP)
	{
		pkServerNode->AddSubItemText(_T("??"));
	}
	
	CTreeItem* pkAdded = InsertItem(pkFind, pkServerNode);
	return (pkAdded == NULL) ? false : true;
}

void CMySuperGrid::GetServerStatusText(EServerStatus eStatus , LPTSTR lpszText, int iTextSize)
{
	LPTSTR lpszTemp = NULL;
	switch (eStatus)
	{
	case EServerStatus::SSTATUS_STOP:
		lpszTemp = _T("STOPPED");
		break;
	case EServerStatus::SSTATUS_STARTING:
		lpszTemp = _T("STARTING");
		break;
	case EServerStatus::SSTATUS_RUNNING:
		lpszTemp = _T("RUNNING");
		break;
	case EServerStatus::SSTATUS_STOPPING:
		lpszTemp = _T("STOPPING");
		break;
	default:
		lpszTemp = _T("UNKNOWN");
		break;
	...
	if (lpszTemp != NULL)
	{
		_tcscpy_s(lpszText, iTextSize, lpszTemp);
	}
}

int CMySuperGrid::GetServerStatusImageIndex(EServerStatus eStatus)
{
	int iIndex = 0;
	switch (eStatus)
	{
