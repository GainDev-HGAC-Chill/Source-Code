...
void PgClientMng::MakeStatistic1m()
{
	...
	while (itor != m_kCollect.end())
	{
		...
		if (itor2 == kTempStat.end())
		{
			...
			MAP_STAT_DATA::_Pairib bRet = kTempStat.insert(std::make_pair(itor->eState, kTemp));
			itor2 = bRet.first;
		}
...
//	kLogin.SetPW(pkTestInfo->chPassword);
	...
	BM::CPacket kLPacket(PT_C_L_TRY_LOGIN, kLogin);
...
//	kAccess.SetPW(g_kNetwork.GetTestInfo()->chPassword);
	...
	BM::CPacket kAPacket(PT_C_S_TRY_ACCESS_SWITCH, kAccess);
...
//		INFO_LOG(BM::LOG_LV4, _T("[%s] TraAccessSwitch Failure UserID[%s], ReturnCode[%d]"), __FUNCTIONW__, kClient.m_pkObject->chID, eRet);
	...
	BM::CPacket kCPacket(PT_C_S_REQ_CHARACTER_LIST);
...
void PgClientMng::DoSelectCharacter(CLIENT_INFO* pkClient)
{
	...
	BM::CPacket kSPacket(PT_C_S_REQ_SELECT_CHARACTER, pkClient->kCharacterGuid);
...
/*
	BM::CPacket kMPacket(PT_C_S_REQ_ENTER_MAP_SERVER, kRSMS);
...
void PgClientMng::ReqMapLoaded(CLIENT_INFO* pkClient)
{
	...
	BM::CPacket kOPacket(PT_C_M_NFY_MAPLOADED);
