...
void CDummyClient::DoMapLoadComplete(BYTE const cCause, SGroundKey const &rkGroundKey)
{
	...
	BM::CPacket kPacket( PT_C_M_NFY_MAPLOADED, (BYTE)cCause );
...
bool CDummyClient::SendToLogin(const BM::CPacket &rkPacket)const
{
	BM::CAutoMutex kLock(m_kMutex);
	const BM::CPacket::PACKET_DATA &rkData = rkPacket.Data();
	
	if(rkData.size())
	{
		const BM::CPacket::DEF_PACKET_TYPE kType = *(const BM::CPacket::DEF_PACKET_TYPE*)&rkData.at(0);
...
bool CDummyClient::SendToSwitch(const BM::CPacket &rkPacket)const
{
	BM::CAutoMutex kLock(m_kMutex);
	const BM::CPacket::PACKET_DATA &rkData = rkPacket.Data();
...
void CDummyClient::CheckStateAntiHack(BM::CPacket& rkPacket, BM::GUID const& rkSwitchGuid)
{
...
bool CDummyClient::OnRecvFromLogin(CEL::CSession_Base *pkSession, BM::CPacket * const pkPacket)
{
	BM::CAutoMutex kLock(m_kMutex);
	BM::CPacket::DEF_PACKET_TYPE kType;
	pkPacket->Pop(kType);
	...
	case PT_L_C_NFY_REALM_LIST:
		{
			...
			{
				BM::CPacket kPacket(PT_C_L_REQ_CHANNEL_LIST);
				...
				if (S_OK == ::RandomElement(kContRealm, realm_itor))
				{
					...
					if(S_OK == ::RandomElement(kContChannel, channel_itor))
					{
						if ((*channel_itor).second.IsAlive())
						{
							...
							BM::CPacket kPacket(PT_C_L_TRY_LOGIN);
...
bool CDummyClient::OnRecvFromSwitch(CEL::CSession_Base *pkSession, BM::CPacket * const pkPacket)
{
	BM::CAutoMutex kLock(m_kMutex);
	BM::CPacket::DEF_PACKET_TYPE kType;
	pkPacket->Pop(kType);
...
//	INFO_LOG(BM::LOG_LV4, _T("S-RecvPacket Type[%d]=[0x%X] Size=[%d]"), kType, kType, pkPacket->Size());
	...
	//	{
	//		SPT_M_C_UM_ANS_MARKET_QUERY kData;
	//		kData.ReadFromPacket(*pkPacket);
	...
	//		case MQPT_START:
	//			{
	//				m_kContMarketList.clear();
	//			}break;
	//		case MQPT_PROCESS:
	//			{
	//				m_kContMarketList.insert(m_kContMarketList.begin(),kData.ContMarket().begin(),kData.ContMarket().end());
	//			}break;
	//		case MQPT_END:
	//			{
	//			}break;
	...
	case PT_M_C_UM_ANS_MY_MARKET_QUERY:
		{
			SPT_M_C_UM_ANS_MY_MARKET_QUERY kData;
			kData.ReadFromPacket(*pkPacket);
			ContMyArticle(kData.ContArticle());
			ContMyDealing(kData.ContDealing());

			switch(this->AiActionTypeSecond())
			{
			case E_AI_STEP_OM_DEREGITEM_REFLASH_WAIT:
				{
					this->AiActionTypeSecond(E_AI_STEP_OM_DEREGITEM_REQ);
				}break;
			case E_AI_STEP_OM_READDEALING_REFLASH_WAIT:
				{
					this->AiActionTypeSecond(E_AI_STEP_OM_READDEALING_REQ);
				}break;
			}
		}break;
	...
	case PT_A_ENCRYPT_KEY:
		{
			...
			if(S_OK == pkSession->VSetEncodeKey(kKey))
			{
				...
				BM::CPacket Packet(PT_C_S_TRY_ACCESS_SWITCH, kTAS);
	...
	case PT_S_C_REQ_PING://핑에 캐릭 정보가 가는 이유는??
		{
...

			{
				...
				BM::CPacket kResPacket(PT_C_S_ANS_ONLY_PING, kTempGuid);
				SendToSwitch(kResPacket);
			}

			if( NS_IN_GAME == AiActionTypeSecond() )
			{
				BM::CPacket kResPacket(PT_C_S_NFY_UNIT_POS, kGUID);
...
EDoAction_Return CDummyClient::DoTrade(EAiActionType_Second const eState,BM::CPacket * const pkPacket, BM::GUID const &rkOtherGuid)
{
	...
	case E_AI_STEP_TRADE_REQ_WAIT:	// 상대방의 요청을 기다리는 상태
		{
			...
			if (TR_REQUEST == eTrade)
			{
				...
				BM::CPacket kSendPacket(PT_C_M_NFY_EXCHANGE_ITEM_ANS, kTradeGuid);
	...
	case E_AI_STEP_TRADE_REQ_ANS:	// Trade 요청에 응답한 상태
		{
			...
			BM::CPacket kSendPacket(PT_C_M_NFY_EXCHANGE_ITEM_ITEM);
	...
	case E_AI_STEP_TRADE_XCHANGE:
		{
			...
			{
				BM::CPacket kSendPacket(PT_C_M_NFY_EXCHANGE_ITEM_READY, kInfoPacket.kTradeGuid);
				SendToSwitch(kSendPacket);
			}
		}break;
	...
	default:
		{
			// pkOther 과 거래를 시작하자.
			BM::CPacket kDPacket(PT_C_M_REQ_EXCHANGE_ITEM_REQ, rkOtherGuid);
...
void CDummyClient::Recv_PT_M_C_NFY_ITEM_CHANGE(BM::CPacket *pkPacket)
{
	...
	while(kArray.end() != item_itor )
	{
		...
		BM::CPacket kAddonData = rkCurModify.kAddonData;
