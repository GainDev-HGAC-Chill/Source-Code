...
inline void PgIndun::InitWaitUser( VEC_GUID& rkCharGuidList )
{
	...
	{
		std::pair<ConWaitUser::iterator, bool> kPair = m_kWaitUserList.insert(SWaitUser(*guid_itr,E_OPENING_NONE));
		if ( !kPair.second )
		{
...
inline HRESULT PgIndun::ReleaseWaitUser( PgPlayer *pkUser )
{
	...
	if ( user_itr != m_kWaitUserList.end() )
	{
		...
		{
			auto userItr = *user_itr;
			userItr.kStep = m_eOpening;
			m_kWaitUserList.erase(user_itr);
			m_kWaitUserList.insert(userItr);
