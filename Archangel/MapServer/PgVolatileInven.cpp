...
namespace VolatileInvenUtil
{
	void AddBuff(CONT_BS_BUFF_COUNT& rkCont, int const iBuff, CONT_BS_BUFF& rkAddOutBuff)
	{
		if( 0 != iBuff )
		{
			std::pair<CONT_BS_BUFF_COUNT::iterator, bool> kRet = rkCont.insert( std::make_pair(iBuff, 1) );
			if( !kRet.second )
			{
				++(*kRet.first).second;
			}
	...
	void AddIcon(CONT_BS_MINIMAP_ICON& rkCont, int const iIcon)
	{
		if( 0 != iIcon )
		{
			std::pair<CONT_BS_MINIMAP_ICON::iterator, bool> kRet = rkCont.insert( std::make_pair(iIcon, 1) );
			if( !kRet.second )
			{
				++(*kRet.first).second;
			}
...
size_t PgVolatileInven::Push(CONT_BS_ITEM const& rkBSItem, PgBase_Item const& rkNewItem, CONT_BS_BUFF& kOutAddBuff)
{
	...
	std::pair<CONT_VOLATILE_INVEN_ITEM::iterator, bool> kRet = m_kContVolatileItem.insert( std::make_pair(iNewInvenPos, rkNewItem) );
	if( !kRet.second )
	{
...
bool PgVolatileInvenMng::AddInven(BM::GUID const& rkOwnerGuid)
{
	...
	std::pair<CONT_VOLATILE_INEVEN::iterator, bool> kRet = m_kContVolatileIneven.insert( std::make_pair(rkOwnerGuid, pkNewInven) );
	if( !kRet.second )
	{
