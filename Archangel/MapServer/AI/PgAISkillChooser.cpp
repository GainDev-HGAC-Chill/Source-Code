...
// 	}
	...
	std::pair<ContAISkillWeightInfo::iterator, bool> kInsert = m_kInfo.insert(std::make_pair(iClass, pkNew));
	if (!kInsert.second)
	{
																	   ...
																	   const CAbilObject* pkAbilObj)
{
	...
	SkillWeightInfo* pkNew = m_kSkillWeightInfoPool.New();
	std::pair<ContAISkillWeightInfo::iterator, bool> kInsert = rkContainer.insert(std::make_pair(kKey, pkNew));
	if (!kInsert.second)
	{
		m_kSkillWeightInfoPool.Delete(pkNew);
