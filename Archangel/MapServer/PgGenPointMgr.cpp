...
{
	std::pair<OwnChildCont::iterator, bool> ret = m_kOwnChild.insert(kUnitGuid);
	if(!ret.second)
	{
							...
							CONT_MAP_BUILD_DATA const &rkMapBuildData)
{
	...
	while(regen_itor != rkMonsterRegen.end())
	{
				...
				for(int iIDX = 0; MAX_MONSTERBAG_ELEMENT > iIDX ; iIDX++ )
				{
					...
					if(iMonBagNo)
					{
						...
						else
						{
							...
							{
								std::pair<CONT_REGEN_GROUP_MONSTER::iterator, bool> ret = m_kGen_Monster.insert( std::make_pair( SGenGroupKey(iMapNo,iIDX), PgGenGroup<PgGenPoint>() ) );
		...
		case ERegenBag_Type_InstanceItem:
			{
				...
				else
				{
					...
					{
						std::pair<CONT_REGEN_GROUP_MONSTER::iterator, bool> ret = m_kGen_Monster.insert(std::make_pair( SGenGroupKey(iMapNo, 0), PgGenGroup<PgGenPoint>() ));
		...
		case ERegenBag_Type_OjbUnit:
			{
				std::pair<CONT_REGEN_GROUP_OBJECT::iterator, bool> ret = m_kGen_Object.insert( std::make_pair(iMapNo, PgGenGroup<PgGenPoint_Object>()) );
		...
		case ERegenBag_Type_SummonNPC:
			{
				std::pair<CONT_REGEN_GROUP_OBJECT::iterator, bool> ret = m_kGen_SummonNPC.insert( std::make_pair(iMapNo, PgGenGroup<PgGenPoint_Object>()) );
