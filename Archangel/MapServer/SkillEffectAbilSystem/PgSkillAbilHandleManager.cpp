...
{	
	...
	while(kContDef.end() != def_itor)
	{
		...
		std::pair<CONT_SKILL_FUNC::iterator, bool> ret = m_kConSkill.insert(std::make_pair(iSkillNo, kTempFunc));
...
{
	PgISkillFunction* kTempFunc = NULL;
//	std::pair<CONT_SKILL_FUNC::iterator, bool> ret = m_kConSkillPool.insert(std::make_pair(iFuncType, kTempFunc));
...
bool PgFunctionTypeXmlLoader::ParseXml(std::wstring const &strXmlPath)
{
	...
	while(pkNode)
	{
		...
		if(0 == strcmp(szName, "FUNCTION"))
		{
			...
			if ( iNo )
			{
				std::pair<CONT_FUNC_TYPE_TABLE::iterator, bool> ret = m_kConFuncType.insert(std::make_pair(iNo, iFunctionType));
				if(!ret.second)
				{
