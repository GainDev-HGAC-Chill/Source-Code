...
//기록 몬스터 출현.
							{
								std::pair<CONT_DROPS::iterator, bool> ret = kContDrops_Mon.insert(std::make_pair(iMonNo, 1));
								if(!ret.second)
								{//이미 있음.
...
//기록 드랍아이템
							...
							while(kOut.end() != ret_itor)
							{
								if((*ret_itor).ItemNo())
								{
									std::pair<CONT_DROPS::iterator, bool> ret = kContDrops_Item.insert(std::make_pair((*ret_itor).ItemNo(), 1));
									if(!ret.second)
									{//이미 있음.
										++(*ret.first).second;//수량 증가.
									}
								...
								else
								{
									...
									std::pair<CONT_DROPS::iterator, bool> ret = kContDrops_Money.insert(std::make_pair(iMoney, 1));
									if(!ret.second)
									{//이미 있음.
										++(*ret.first).second;//수량 증가.
									}
