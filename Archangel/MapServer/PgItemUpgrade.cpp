...
TBL_DEF_ITEMPLUSUPGRADE const * PgAction_ItemPlusUpgrade::GetPlusInfo( PgBase_Item const &kTargetItem, int const iLv, EPlusItemUpgradeResult &rkOutRet )
{
	...
	if( pDef )
	{
		...
		int iMaxLv = 0;
		if ( true == PgItemRarityUpgradeFormula::GetMaxGradeLevelRestrict( eItemGrade, pDef->IsPetItem(), iMaxLv, kTargetItem) )
		{
			if ( iLv > iMaxLv )
			{
...
int const MAX_DOWNGRADE_RATE = 30;
		...
		else
		{
			...
			else
			{
				...
				case LOCAL_MGR::NC_USA:
					{
						// 유럽, 북미는 보험 사용시 등급 하락 없다.
					}break;
				default:
					{
						if(IPULL_UNIQUE_LIMIT < m_iNowLevel)	// 16 등급 부터 인첸트 보험을 사용해도 실패 하면 1 단계 등급이 하락한다.
						{
