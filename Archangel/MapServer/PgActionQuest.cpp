		...
		if( g_kQuestMan.GetQuest(rkState.iQuestID, pkQuestInfo) )
		{
			...
			while( rkDependGlobal.end() != global_iter )
			{
				...
				case QET_GLOBAL_Combo_IncParam:
					{
						if( m_iCombo &&	rkGlobalEvent.iVal1 <= m_iCombo )
						{
...
//		PgAction_MonsterQuestCheck
//////////////////////////////////////////////////////////////////////////////
PgAction_MonsterQuestCheck::PgAction_MonsterQuestCheck(int const iCurGroundNo, SGroundKey const &rkGndKey, unsigned int iCombo, int const iMissionKey, int const iMissionLevel, int const iMssionStageCount)
	:m_iGroundNo(iCurGroundNo), m_kGndKey(rkGndKey), m_iCombo(iCombo), m_iMissionKey(iMissionKey), m_iMissionLevel(iMissionLevel), m_iMissionStageCount(iMssionStageCount)
{
}
...
//////////////////////////////////////////////////////////////////////////////
PgAction_GroundQuestCheck::PgAction_GroundQuestCheck(int const iCurGroundNo, SGroundKey const &rkGndKey, unsigned int iCombo)
	: m_iGroundNo(iCurGroundNo), m_kGndKey(rkGndKey), m_iCombo(iCombo)
{	
}
...
void PgAction_GiveQuestReward::CreateReward(PgQuestInfo const *pkQuestInfo, PgPlayer *pkPC, CONT_PLAYER_MODIFY_ORDER &rkOrder)
{
	...
	if( pDef_LevelReward )
	{
		...
		if( pDef_LevelReward->end() != kitor_Reward )
		{
			...
			iMoney += (*kitor_Reward).second.iMoney * fRate;
		}
	}
	//돈.
			...
			if( g_kQuestMan.GetQuest(iQuestID, pkQuestInfo) )
			{
				...
				while( rkDependMission.end() != mission_iter )
				{
					...
					if( rkMissionKey.iKey == rkElement.iMissionKey )
					{
						...
						case QET_MISSION_DEFENCE_CLEAR:
							{
								if( eMissionMode::MO_DEFENCE == MissionMode )
						...
						case QET_MISSION_STRATEGY_DEFENCE_CLEAR:
							{
								if( eMissionMode::MO_DEFENCE7 == MissionMode )
