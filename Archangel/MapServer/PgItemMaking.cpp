...
HRESULT PgAction_ItemMaking::ProcessCooking(CUnit* pkCaster, SReqItemMaking const& rkReqItemMaking, __int64& iTotalNeedMoney, CONT_NEED_ITEMS& kContNeedItems, CONT_RESULT_ITEM_DATA& kContResultItems, int& iMakingSuccessCount, int& iWarnMessage)
{
	...
	while(iMakingCount < rkReqItemMaking.iMakingCount)
	{
		...
		if(rkReqItemMaking.bUseOptionItem)
		{
			...
			{
				std::pair<CONT_NEED_ITEMS::iterator, bool> ret = kContHaveOptionItem.insert(std::make_pair(iOptItemNo, pInv->GetTotalCount(iOptItemNo)));
...
bool PgAction_SoulStoneTrade::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	...
	CONT_RESULT_ITEM_DATA kContResultItems;
	CONT_DEF_SOULTRADE const * pkDefItemSoulTrade = NULL;
	g_kTblDataMgr.GetContDef(pkDefItemSoulTrade);

	CONT_DEF_SOULTRADE::const_iterator it_DefSoulTrade = pkDefItemSoulTrade->find(kReqSoulStoneTrade.gidSerial);

	if (it_DefSoulTrade == pkDefItemSoulTrade->end())
	{
		iWarnMessage = 1502;
		goto __SEND_RESULT_FAILED;
	}

	CONT_DEF_SOULTRADE::mapped_type const& kItem = (*it_DefSoulTrade).second;
	int iTotalMaterialNeedsCount = kItem.iMaterialNeedsCount * kReqSoulStoneTrade.iTradeCount; // 한 개 제작시 필요한 재료 수 * 총 제작할 아이템 수
	{
		CItemDef const *pRetkItemDef = m_kItemDefMgr.GetDef(kItem.iItemNo);
		if(!pRetkItemDef)
		{
			iWarnMessage = 1502;
			goto __SEND_RESULT_FAILED;
		...
		PgBase_Item kRetItem;
		if(S_OK != ::CreateSItem(kItem.iItemNo, kReqSoulStoneTrade.iTradeCount, GIOT_MAKING, kRetItem))
		{
			goto __SEND_RESULT_FAILED;
		}

		size_t const now_count = pInv->GetTotalCount(kItem.iMaterialNo);
		...
		SPMOD_Add_Any kDelData(kItem.iMaterialNo, -iTotalMaterialNeedsCount);
		SPMO kIMO(IMET_ADD_ANY, pkCaster->GetID(), kDelData);
		kOrder.push_back(kIMO);
