...
,	m_kContTrigger()
,	m_eUnlockBidirection(UT_PET)	//펫은 기본적으로 방향 무시
...
HRESULT PgGroundResource::LoadGsa( char const *pcGsaPath )
{
	...
	for (int i=0 ; i<iTotal ; i++)
	{
			...
			if (pkNifRoot)
			{
				...
				NiNode *pkNewNode = 0;
				{
						...
						++iChildCount)
					{
						...
						// 본이 있는 Character라면 NewNode가 아닌 그 밑의 더미 노드에 TRS를 준다.
		...
		{
			pkEntity->GetPropertyData("KFM File Path", kPath);	//Nif정보가 없고 KFM정보만 있는 트랩류일 경우
			if(kPath.Exists())
			{
				// Transform
				NiPoint3 kTranslation;
...
void PgGroundResource::FindMinMax(NiNode *pkRootNode, NxVec3 &kMin, NxVec3 &kMax)
{
	...
	// 아래 코드가 제대로 작동될 때 까지 아래 코드로 돌린다.
	/*
...
PgNpc* PgGroundResource::AddNPC( char const *pcName, char const *pcActor, char const *pcScript, char const *pcLocation, BM::GUID const &rkGuid, int iID, int GiveEffectNo, ENpcType const kNpcType )
{
	...
	if( pkNpc)
	{
		...
		std::pair<CONT_OBJECT_MGR_UNIT::iterator, bool> kPair = m_kNpcCont.insert(std::make_pair(rkGuid, pkNpc));
...
void PgGroundResource::CopyContGTrigger( CONT_GTRIGGER const& rkFrom )
{
	...
	while( rkFrom.end() != iter )
	{
		...
		if( pkElement )
		{
			...
			if( pkNewTrigger )
			{
				std::pair<CONT_GTRIGGER::iterator, bool> kRet = m_kContTrigger.insert( std::make_pair((*iter).first, pkNewTrigger) );
				if( !kRet.second )
				{
		...
		case GTRIGGER_TYPE_PORTAL_EMPORIA:
			{
				...
				{
					std::pair<CONT_EMPORIA_PORTAL::iterator, bool> kPair = m_kContEmporiaPortal.insert( std::make_pair( pkGTrigger_PortalEmporia->GetEmporiaKey(),PgPortal_Emporia( pkGTrigger_PortalEmporia ) ) );
					if ( true == kPair.second )
					{
	...
	if ( pkGTrigger )
	{
		...
		while ( pkElement )
		{
			if ( !bInsertComplete && !::strcmp( pkElement->Value(), "ACTION") )
			{
				...
				{
					if(pkGTrigger->GetType() == GTRIGGER_TYPE_CHECK_POINT) //달리기 이벤트용 트리거라면
					{
						m_kContCheckPointOrder.push_back(kID); //트리거 순서대로 키 값을 별도 컨테이너에 담음
					}

					std::pair<CONT_GTRIGGER::iterator, bool> kPair = m_kContTrigger.insert( std::make_pair( kID, pkGTrigger ) );
					if ( kPair.second )
					{
				...
				{
					if(!GetSceneRoot()->GetObjectByName( kID.c_str() ))	//트랩같은 경우 ProjectG Property가 Trap이라 트리거목록에서는 찾을 수 가 없다.
					{
