...
bool PgGroundUnitMgr::Add(CUnit* pkUnit, int iGroundNo, T_GNDATTR const kGndAttr)
{
	...
	if( iGroundNo && pkUnit && pkUnit->UnitType() == UT_PLAYER )
	{
		...
		std::pair<CONT_UNIT_MGR::iterator, bool> bRet = ContUnitAll.insert(std::make_pair(pkUnit->GetID(), CONT_UNIT_MGR::mapped_type() ));
		std::pair<CONT_UNIT_MGR::mapped_type::iterator, bool> bSunRet = (*bRet.first).second.insert(std::make_pair(iGroundNo, pkUnit));
