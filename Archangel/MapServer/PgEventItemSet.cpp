...
void PgPartyEquiptItemInfoMgr::Regist(BM::GUID const& rkPartyGuid, BM::GUID const& rkGuid)
{
	...
	{
		std::pair<CONT_PARTY::iterator, bool> kRet = m_kContPartyInfo.insert( std::make_pair(rkPartyGuid, CONT_PARTY::mapped_type()) );
		if( kRet.second )
		{
...
bool PgEventItemSetMgr::Build()
{
	...
	while( pkDef->end() != def_iter )
	{
		...
		{
			std::pair<CONT_DEF_BUILDED_EVENT_ITEM_SET::iterator, bool> kRet = m_kContEvent.insert( std::make_pair(rkDef.iEventItemSetID, CONT_DEF_BUILDED_EVENT_ITEM_SET::mapped_type(rkDef)) );
			if( kRet.second )
			{
				find_iter = kRet.first;
			}
