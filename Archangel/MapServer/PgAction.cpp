...
static int const g_aiDefenceCandidate[] = {//방어시 까이는 대상.
		EQUIP_POS_HELMET,							EQUIP_POS_EARRING,
		EQUIP_POS_SHOULDER,							//훈장은 안깜.	EQUIP_POS_MEDAL
...
static int const g_aiDeathPenaltyCandidate[] = {//사망시 까이는 대상.
		EQUIP_POS_HELMET,							EQUIP_POS_EARRING,
		EQUIP_POS_SHOULDER,							//훈장은 안깜.	EQUIP_POS_MEDAL
...
bool PgAction_ReqAddMoney::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	...
	{// 죽었을때 소비는 되어야 하기때문에....
		...
		{
			LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
			return false; // 죽어있으면 못 먹는다
		}
	}
	
	CONT_PLAYER_MODIFY_ORDER kOrder;
	SPMOD_Add_Money kDelMoneyData(m_iAddMoney);//필요머니 빼기.
	SPMO kIMO(IMET_ADD_MONEY, pkCaster->GetID(), kDelMoneyData);
	kOrder.push_back(kIMO);

	PgAction_ReqModifyItem kItemModifyAction(m_kCause, m_kGndKey, kOrder, m_kPacket, true);//위에서 사망관련 검사 하므로 데스 상태를 무시 해라.
...
bool PgAction_ReqStoreItemBuy::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	...
	{//사지면. 돈을 까.
		...
		if(S_OK == kOutStore.GetGoods(m_iItemNo, kGoods))
		{
			...
			if(pItemDef)
			{
				if( !pItemDef->IsAmountItem() )//내구도 아이템은 1개로 지급
				{
					iCount = 1;
				}
				...
				if( kGoods.iPrice )
				{
					if(iCount > pItemDef->MaxAmount())
					{
						LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false : iCount > MaxAmount"));
						return false;
					}

					__int64 const i64TotalPrice = kGoods.iPrice * iCount;
					if( i64TotalPrice <= pkCaster->GetAbil64(AT_MONEY) && i64TotalPrice > 0 )
					{
						SPMOD_Add_Money kDelMoneyData(-i64TotalPrice);//필요머니 빼기.
						SPMO kIMO(IMET_ADD_MONEY, pkCaster->GetID(), kDelMoneyData);
						kOrder.push_back(kIMO);
					}
					else
					{
						LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false : i64TotalPrice <= AT_Money || i64TotalPrice < 0"));
						return false;
					}
				}
				...
				&&	kGoods.iCP )
				{
					...
					{
						SPMOD_Add_CP kDelCPData(-iTotalCP);//필요 CP 빼기.
...
bool PgAction_ReqStoreItemSell::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	...
	if(pkInv)
	{
		...
		&&	kItem.Count() )
		{
			...
			if(!iSellPrice 
			&& !iStockSellPrice)//둘다 가격 없으면 판매가 안됨
			{
				goto __FAILED;
			}
			...
			if(iStockSellPrice)
			{
				iSellPrice = iStockSellPrice;//주식 값으로 보정.
			}
			...
			if(S_OK == CalcSellMoney(kItem, iEarnMoney, iSellPrice, m_iCount, pkPlayer->GetPremium()))
			{
				CONT_PLAYER_MODIFY_ORDER kOrder;
				{
					SPMOD_Add_Money kDelMoneyData(iEarnMoney);//돈 지급.
					SPMO kIMO(IMET_ADD_MONEY, pkCaster->GetID(), kDelMoneyData);
					kOrder.push_back(kIMO);
				}
		...
		case UT_PLAYER:
			{
				...
				else
				{
					...
					if ( pkPlayer )
					{
						...
						{//Create 와 Modify 가 같은 이벤트인데 구분을 어찌하나?
...
///		kPacket.Push((BYTE)m_iEventType);//어떤 건지
	...
	{//파티에게 날림.
		...
		kTempPacket.Push(pkCaster->GetID());//누가
		PU::TWriteArray_M(kTempPacket, kPartyBBSArray);//뭘
...
bool PgAction_ModifyItem_AddOnPacketProcess::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	if(!m_kAddonPacket.IsEmpty())
	{
		...
		case PT_M_C_ANS_OPEN_GAMBLE:
			{
				if(S_OK == Ret())
				{
					// 수령 할 아이템이다. 인벤이 가득찰 경우를 대비해서 오더를 따로 만들어서 컨텐츠 응답이 오면 아이템 생성을 다시 서버로 날린다.
					// 박스류의 아이템 문제도 이렇게 해야만 해결가능(받고 싶은 아이템을 골라 받는 동작 방지용...)
					CONT_PLAYER_MODIFY_ORDER kOrder;
		...
		case PT_M_C_ANS_CREATE_CHARACTER_CARD:
			{
				...
				kResult.Push(Ret());
				pkCaster->Send(kResult);
				if(S_OK == Ret())	// 주변 사람들에게 카드 생성을 통보한다.
				{
					PgPlayer * pkPlayer = dynamic_cast<PgPlayer *>(pkCaster);
					if(pkPlayer)
					{
		...
		case PT_M_I_REQ_AFTER_QUEST_ACCEPT: // GM커맨드 용
			{
				int iQuestID = 0;
		...
		case PT_M_I_REQ_QUEST_EVENT:
			{
				...
				std::wstring kNpcName; // Log용 NPC이름
		...
		case PT_A_M_ADDON_WARPPED_PACKET:
			{
				...
				m_kAddonPacket = kPacket; // 원래 패킷으로 교체

				DoAction(pkCaster, pkTarget); // 재귀 호출
			}break;
...
{//!!!!!!!!!!!! 아이템 시스템 룰을 모르면 손 대지 마시오.

	typedef std::set< int > ContQuestIDSet;

	// pkCaster는 PgPlayer가 아닐수도 있다(Pet일수 있다)
	if( pkCaster )
	{
		...
		CONT_ITEM_VEC kItemVec;//파티원에게 습득 아이템 알리기
		ContItemNoSet kItemSet;//변동된 아이템 번호를 겹치지 않게 Set 으로 구성(퀘스트에서 씀)
		...
		while(change_itor != m_kChangeArray.end())
		{
								...
								if( !kVec.empty() )
								{
									...
									while( kVec.end() != iter )
									{
										...
										if( g_kQuestMan.GetQuest(iQuestID, pkQuestInfo) )
										{
											...
											{
												kFailedOrder.push_back( SPMO(IMET_ADD_INGQUEST, pkPlayer->GetID(), SPMOD_AddIngQuest(iQuestID, QS_Failed)) ); // 실패 처리
											}
										}
										++iter;
									}
			...
			case DISCT_MODIFY_POPULARPOINT:
				{
					...
					{// 누적 인기도 업적
						int const iMaxValue = std::min<__int64>(i64TotalPP,std::numeric_limits<int>::max());// 20억 이상은 넘을수 없도록 하자 
						PgSyncClinetAchievementHandler<> kSA(AT_ACHIEVEMENT_POPULARPOINT,iMaxValue,m_pkGround->GroundKey());
					}
					{// 오늘의 인기도 업적
						int const iMaxValue = std::min<__int64>(i64TodayPP,std::numeric_limits<int>::max());// 20억 이상은 넘을수 없도록 하자 
			...
			case DISCT_COMPLETE_ACHIEVEMENT:
				{
					...
					if ( pkPlayer )
					{
...

						{
							...
							kAbilEvent.DoAction(pkPlayer, NULL); // 부분 점수
						}
						{
							PgAction_QuestAbil kAbilEvent(GndKey(), AT_ACHIEVEMENT_TOTAL_POINT);
							kAbilEvent.DoAction(pkPlayer, NULL); // 전체 점수
						}
			...
			case DISCT_ADD_ABIL:
				{
					...
					case IMET_SET_ABIL:
						{
							...
							case AT_LEVEL:
								{
									{//레벨업 했으니 빌드 할 수 있는 퀘스트가 생겼을 수 있다.
										PgPlayer *pkPlayer = dynamic_cast<PgPlayer *>(pkCaster);
										if( pkPlayer )
										{
											CONT_PLAYER_MODIFY_ORDER kOrder;
									...
									// Class 와 Level 은 SetAbil에서 알아서 보낸다.
									bNeedGroundQuestRefresh = true;
								}break;
							case AT_STR_ADD:
							case AT_INT_ADD:
							...
							if (bSend)
							{
								pkCaster->SendAbil((EAbilType)iType, dwSendType);//맵이동 중이라도 보내야.
							}
					...
					case IMET_ADD_ABIL://Add 라도 결과는 Set.
						{
							int iValue = 0;
							kAddonData.Pop(iValue);
					...
					case IMET_ADD_ABIL64://Add 라도 결과는 Set.
						{
							__int64 i64Value = 0;
							kAddonData.Pop(i64Value);
							pkCaster->SetAbil64(iType, i64Value);
							pkCaster->SendAbil64((EAbilType)iType, E_SENDTYPE_SELF | E_SENDTYPE_MUSTSEND);//맵이동 중이라도 보내야.
						}break;
			...
			case DISCT_MODIFY_SKILL:
				{
					...
					if( pkPlayer )
					{
						...
						if(iType)//리셋.
						{
							...
							while ((iSkillNo = pkPlayer->GetMySkill()->GetSkillNo(EST_PASSIVE, szIndex++)) > 0)
							{
								...
								if (NULL != pkSkillDef)
								{
									...
									if(NULL != pkEffect)
									{
										//실제로 걸려있는 EffectNo에 해당되는 스킬 Def를 얻어 와야 한다.
										//OverLevel에 의해서 저장된 정보와 걸려있는 정보가 다를 수 있다.
							...
							//pkPlayer->GetMySkill()->Init(UT_PLAYER);//Init 으로 지우는데.
							PgMySkill* pkMySkill = pkPlayer->GetMySkill();
							if( pkMySkill )
							{
								pkMySkill->EraseSkill(SDT_Normal);	// Skill 지우고
						...
						else
						{
							...
							if( bPassiveSkill )
							{
								// 이전에 배운 스킬이 있으면 그 스킬의 효과를 없애기 위한 작업이 필요하다.
								int const iLearn = pkPlayer->GetMySkill()->GetLearnedSkill(iSkillNo);
								if ( iLearn > 0 )
								{
									//Passive Skill은 SkillNo 과 EffectNo이 같다.
							...
							RefreshGroundQuestInfo(pkCaster);	//스킬 배우면 퀘스트 다시 확인하자. 스킬 소유 여부로 퀘스트 체크하는것 추가되었음(2011.11.14) By 조현건
						}
					}
					else
					{
			...
			case DISCT_MODIFY_MONEY:
				{
					...
					pkCaster->SetAbil64(AT_MONEY, i64NewMoney);//돈 보내.
					pkCaster->SendAbil64(AT_MONEY, E_SENDTYPE_SELF | E_SENDTYPE_MUSTSEND);

					if( iDiffMoney )//클라이언트 시스템 메시지
					{
			...
			case DISCT_MODIFY_CP:
				{
					...
					pkCaster->SetAbil(AT_CP, iNewCP);//돈 보내.
					pkCaster->SendAbil(AT_CP, E_SENDTYPE_SELF);

					if( iDiffCP )//클라이언트 시스템 메시지
					{
									...
									case QS_Finished:
										{
											...
											if( g_kQuestMan.GetQuest(iQuestID, pkOut) )
											{
												...
												case QT_Wanted:
												case QT_Day:
												case QT_Couple:		// 커플 퀘스트
												case QT_SweetHeart: // 연인 퀘스트												
			...
			case DISCT_EMPORIAFUNC_UPDATE:
				{//따로 처리 할꺼 없음. 맵이동 패킷에 묻어서 가게 되어있음.
				}break;
...
// 									}
// 								}
								std::pair<ContItemNoSet::iterator, bool> kRet = kItemSet.insert(rkItemChange.ItemWrapper().ItemNo());
							}
						}
					}
			...
			default:
				{
					...
					{
						if(	SUCCEEDED(pInv->GetItem(rkItemChange.ItemWrapper().Pos(), kPrevItem))
						&&	kPrevItem.ItemNo() )//이전 아이템이 위치에 있었으면
						{
					...
					std::pair<ContItemNoSet::iterator, bool> kRet = kItemSet.insert(iChangedItemNo);//변동이 있었다면.		
					...
					if(IMC_DEC_DUR_BY_USE & rkItemChange.Cause())
					{
						...
						if(NULL != pItemDef)
						{
							...
							{// 아이템 사용에 따른 업적이 있다면 이곳에 추가
								int const iCustomType = pItemDef->GetAbil(AT_USE_ITEM_CUSTOM_TYPE);
								switch(iCustomType)
								{
								case UICT_MOVETOPARTYMEMBER: // 파티워프 아이템 사용 업적
									{
					...
					// 내구도가 바닥나서 비활성화 되거나 수리해서 다시 활성화 될때의 처리
					bool bFitItem = (IT_FIT == rkItemChange.ItemWrapper().Pos().x) || (IT_FIT_CASH == rkItemChange.ItemWrapper().Pos().x);
					...
					// ItemChange 값이 SItemPos() 기준으로 넘어 온다.
					// 즉, 아이템위치 교환일 경우에
					//	ItemPos()값은 변경되지 않고
					//	ItemWrapper() 에는 새로운 아이템번호 / PrevItemWrapper() 에는 이전아이템번호
					//	가 입력되어 있다.
					if ( 0 != (IMET_MODIFY_POS & rkItemChange.Cause()))
					{
						...
						{
							/*
							// Item 이동 패킷을 맵서버에서 받았을 때, AddCoolTime 하도록 변경되었음.
							// 왜냐하면 : Item 서버로 부터 응답이 오기 전에, Client가 같은 Action 요청을 할 수 있기 때문이다.
							// CoolTime : 아이템장착
							...
							*/
							...
							case EQUIP_POS_PET:
								{
									bNeedGroundQuestRefresh = true; // 캐릭터의 펫관련 퀘스트를 다시 체크 한다
								}break;
							default:
								{
								}break;
						...
						//else if (IT_FIT == rkItemChange.PrevItemWrapper().Pos().x)	//장착 해제 하였음.
						//{
						//	pkCaster->GetInven()->AddCoolTime(rkItemChange.PrevItemWrapper().ItemNo(), PgInventory::EICool_Unequip);
						//}
					}
		...
		ResultProcess(pkCaster, pkTarget);//아이템 변경 이벤트에 대해서 모든 처리

		PgQuestActionUtil::CheckItemQuestOrder(GndKey(), kItemSet, pkCaster);//퀘스트 체크
		NotifyPartyMember(kItemVec, pkCaster, pkTarget);//파티원에게 알림

		PgAction_SendItemChange kSendAction(m_kCause, GndKey(), m_kChangeArray, m_kLocalPartyMgr);
		kSendAction.DoAction(pkCaster, pkTarget);//알림.

		if( bAbilRefresh )// Equip이 바뀌었을 때만 해 주자.
		{
			...
			{// 세트 아이템 착용 업적
				...
				if( pkInv )
				{
					...
					CONT_HAVE_ITEM_DATA kFitCont;			// 장착한 모든 아이템
					CONT_HAVE_ITEM_DATA kFitSetCont;		// 장착한 세트 아이템
					PgInventory::CONT_SETITEM_NO kSetCont;	// 장착한 아이템들의 세트번호
							...
							if( iEquipPiece )
							{
								...
								if( pEquipSetDef )
								{
									...
									{// 풀셋이다 
										...
										int iMaxLevelLimit = 0; // 세트아이템 각각의 요구레벨이 틀리기 때문에 제일 큰놈을 기준으로 업적을 체크.
		...
		if( !kChangeParamQuestIDSet.empty() )
		{
			...
			if( !kNextOrder.empty() )
			{
				PgAction_ReqModifyItem kItemModifyAction(IMEPT_QUEST, GndKey(), kNextOrder, BM::Stream(), true);//뭐인지는 모르겠는데 사망 여부 상관 없을거 같다.
				kItemModifyAction.DoAction(pkCaster, NULL);
			}
		}
		return true;
...
void PgAction_ModifyPlayerData::ResultProcess(CUnit* pkCaster, CUnit* pkTarget)
{
	...
	case CIE_ChangeClass_ByGMCmd://커멘드로 클래스 바꾼 경우는. 정보 초기화.
		{
			CONT_PLAYER_MODIFY_ORDER kOrder;
			SPMOD_AddSkill kAddSkillData(0, true);//리셋. 
	...
	case MCE_Fran:
		{
			if( S_OK == m_kRet )//성공 했으니 그라운드 박스 삭제
			{
				pkCaster->Send(m_kAddonPacket);
			}
		}break;
	...
	case CIE_Loot:
		{
			...
			{
				pkCaster->SendWarnMessage(400715);// 인벤토리에 공간이 부족합니다
			}
		}break;
	case MCE_CreateGuild:
		{
			if( S_OK == m_kRet )// 길드 만들어 줘
			{
				BM::Stream kPacket(PT_C_N_REQ_GUILD_COMMAND, pkCaster->GetID());
	...
	case CIE_QuestItem:
		{
			if( S_OK != m_kRet )// 실패
			{
				pkCaster->SendWarnMessage(700099);// 꽉차서 퀘스트 아이템 못먹어
			}
		}break;
	...
	//case IMEPT_QUEST_REWARD://퀘스트
	case IMEPT_QUEST_DIALOG:
	case IMEPT_QUEST_DIALOG_Accept:
	case IMEPT_QUEST_DIALOG_End:
	//case CIE_MissionQuestCard://미션퀘스트카드
	case CIE_GuildLevelUp://길드 레벨업
	...
	case CIE_UserQuestComplete:
		{
			...
			case S_OK:
				{
					DB_ITEM_STATE_CHANGE_ARRAY::const_iterator iter = m_kChangeArray.begin();//퀘스트 체크
					...
					if(CIE_Home_Equip == m_kCause) // 마이홈 아이템 장착 횟수 저장
					{
	...
	case CIE_Make:
		{
			...
			if(S_OK == m_kRet)
			{
				...
				{
					//성공 해야만. 업적 기록
					PgAddAchievementValue kMA(AT_ACHIEVEMENT_COOKER, iMakingSuccessCount, GndKey()); // 업적 기록
					kMA.DoAction(pkCaster,NULL);
				}
			...
			{
				pkPlayer->SendWarnMessage(400715); //인벤토리에 공간이 부족합니다.
			}
...
/*	case CIE_Manufacture:
	...
	case CIE_JOBSKILL_DELETE:
		{
			...
			RefreshGroundQuestInfo(pkCaster);	//직업스킬 삭제하면 퀘스트 다시 체크하자. 스킬 소유 여부로 퀘스트 체크하는것 추가되었음(2011.11.14) By 조현건
		}break;
	...
	{//특정아이템 획득시 한줄공지 체크.
		GET_DEF(CItemDefMgr, kItemDefMgr);
		DB_ITEM_STATE_CHANGE_ARRAY::const_iterator iter = m_kChangeArray.begin();//축하메시지 체크
		while(m_kChangeArray.end() != iter)
		{
			DB_ITEM_STATE_CHANGE_ARRAY::value_type const& rkElement = (*iter);
...
bool PgAction_CreateItem::DoCreateItem(CUnit* pkCaster,CONT_PLAYER_MODIFY_ORDER & kOrder) const
{
	...
	{
		tagPlayerModifyOrderData_Insert_Fixed kAddData((*craete_order_itor), kItemPos, true);//1씩 감소.
...
{
	int iTimeVal = 10000; //기본값 10000 (10초)
...
bool PgAction_PickUpItem::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	...
	if( true == pkPC->IsDead() )
	{
		pkPC->SendWarnMessage(20102);//먹을 수 있는 상태가 아닙니다
		return false;
	}
	...
	if( m_dwCanAnyPickUpItemTime > ::DifftimeGetTime( pkGndItemBox->CreateDate(), BM::GetTime32() ) )
	{
		...
		if( !bMine ) // 내가 아니라면.
		{
			pkPC->SendWarnMessage(20101);//소유권이 없습니다.
			return false;
		}
	}
	...
	{	// 인스턴스 아이템이라면 여기서 바로 이펙트 걸고 아이템 제거함.
		...
		{	// 박스가 사라진다고 알림
			...
			{	// 리젠 간격이 0 이상일 때만, 0인 경우는 원하는 시점에 리젠을 시키고 싶을 때 설정하기 때문에.
	...
	// 일단은 아이템 박스에 아이템은 하나 만이다 가정하고서 진행 (기획 변경시 변경 되어야 하는 부분)
	if( 0 != pkGndItemBox->ElementSize() )
	{
		PgBase_Item kItem;
		if( S_OK == pkGndItemBox->PopItem(kItem, true) )
		{
			CUnit *pkItemOwner = pkCaster; // 주인 확인

			{ // 파티일 중에는 파티 옵션에 따라, 주인은 따로 있다.
				...
				if( kItemShareCheck.DoAction( pkCaster, NULL ) )
				{
					...
					if( pPlayer )
					{
						if( pPlayer->HaveExpedition() )			// 원정대 있으면 원정대 룰대로 아이템 획득.
						{
							...
							if( ShareItem.DoAction( pkCaster, NULL ) )
							{
								...
								||	pkItemOwner->IsDead() )
								{
									pkItemOwner = pkCaster; // 죽었거나 없으면 Caster를 다시 오너로 셋팅
								}
							}
						}
						else		// 아니면 일반 파티 룰대로.
			...
			{ // 보통 상태는 소유자 인벤토리로
				...
				{
					pkPC->SendWarnMessage(iNotHaveEmptyIven);//인벤토리가 가득 찼습니다.
					return false;
				}
				...
				&&	S_OK != pkInven->CanInsert(kItem) )
				{
					pkItemOwner->SendWarnMessage(iNotHaveEmptyIven);//인벤토리가 가득 찼습니다.
					// 이놈이 먹을 수 없으면 주울려고 한 놈이 먹는다.
					pkItemOwner = pkPC;
				}
			...
			pkGndItemBox->PopItem( kItem, false ); // 아이템을 꺼내는건 나중에 한다
...
bool PgAction_PopPetItem::DoAction( CUnit* pkCaster, CUnit *pkNothing )
{
	if ( pkCaster )
	{
		...
		{
			// 이 함수는 Pet한테서 뽑기만 하는 것이므로, Player의 빈자리이어야만 한다.
...
{
	if(!pkCaster)
	{// 여기서 실패나면 길드금고 위치 잠근거 풀 수 가 없는데. 답이 없네.
		return false;
	}
...
bool PgAction_MoveItem::DoAction( CUnit* pkCaster, CUnit* pkPet )
{
	...
	if ( true == bIsPet )
	{
		...
		// ItemGuid 를 Pos와 함께 보내는 이유 ?
		// ContentsServer 에서 Pos와 GUID를 함께 확인하도록 하기 위함.
		// ContentsServer 가 느릴 경우, Pos만 전송하면, 같은
		...
		case IT_FIT:
			{
				...
				{
					// Pet Item을 장착하네...어쩔수 없이 예외처리..(이전펫의 MP를 Save해주어야 한다.)
					MakeUnEquipPetOrder( m_kCasterPos, kTargetItem, kOrder );// Order는 아이템이 이동한 다음에 처리가 되니까 Pos를 이동후의 위치로
				}
			}break;
		case IT_FIT_CASH:
			{
				...
				case IT_FIT:
					{
						...
						{
							// Pet Item을 장착하네...어쩔수 없이 예외처리..(이전펫의 MP를 Save해주어야 한다.)
							MakeUnEquipPetOrder( m_kTargetPos, kCasterItem, kOrder );// Order는 아이템이 이동한 다음에 처리가 되니까 Pos를 이동후의 위치로
						}
				...
				case IT_SHARE_RENTAL_SAFE4:
					{
						...
						case LOCAL_MGR::NC_CHINA:
							{
								// 중국은 꺼내는건 기간이 만료되어도 상관 없이 꺼낼수 있어야 한다.
							}break;
						default:
							{
					...
					case IT_FIT:
						{
							...
							if(pkPet->GetPetType() == EPET_TYPE_3) //라이딩펫은 장착 불가.
							{
								return 358;
							}
							...
							{
								return 22020;// 장착형 아이템이 아니다.
							}

							int const iGenderLimit = pItemDef->GetAbil(AT_GENDERLIMIT);
							if(0 == (iGenderLimit & pkTargetUnit->GetAbil(AT_GENDER)))//성별 검사.
							{
								return 22030;// 펫 전용 아이템이 아닙니다.
							}
							...
							case E_ACCESSDENIED:
								{
									return 359;//펫의 진화가 낮아 장착 할 수 없습니다.
								}break;
							default:
								{
									return 358;//펫의 종류에 맞지 않는 아이템 입니다.
								}break;
							}
		...
		if(pItemDef)
		{
				...
				|| kTargetPos.x == IT_FIT_CASH)
			{
				if(EQUIP_POS_NONE != eEquipPos )
				{
					...
					{// 장착 해제 불가 어빌이 있으면
						iErrorRet = 20026;	// 장착/해제 할 수 없는 상태입니다.
						goto __SEND_ERROR;
					}
				}
			...
			case IT_FIT_CASH:
				{
					...
					{
						iErrorRet = 1409;	// 먼저 봉인을 해제해야 합니다.
						break;
					}
					...
					if(!pItemDef->CanEquip())//장착형 아이템이면
					{
						iErrorRet = 22020;
						break;
					}
					...
					int const iGenderLimit = pItemDef->GetAbil(AT_GENDERLIMIT);
					if(0 == (iGenderLimit & pkCaster->GetAbil(AT_GENDER)))//성별 검사.
					{
						switch( iGenderLimit )
						{
					...
					// Class Limit
					__int64 const i64ClassLimit = pItemDef->GetAbil64(AT_CLASSLIMIT);
					//if(0 == (i64ClassLimit & (__int64)pow((double)2, (double)pkCaster->GetAbil(AT_CLASS))))//클래스 검사
					...
					case EQUIP_POS_WEAPON:
						{
							//캐쉬 무기의 경우 기본 장착된 무기가 있어야 한다.
							//장착하고 있는 기본 무기와 비교 한다.
							PgBase_Item kWeapon;
							if (SUCCEEDED(pkCaster->GetInven()->GetItem(SItemPos(IT_FIT, EQUIP_POS_WEAPON), kWeapon)))
							{
								...
								{
									//장착하고 있는 기본 무기와 다른 타입이라면 장착하지 못한다.
									iErrorRet = 22028;
									break;
								}
							}
							...
							if( 0 != iMainToolType )
							{
								...
								else
								{
									...
									for( int iCount = 0; iCount < CHECK_SUBTOOL_COUNT; ++iCount )
									{
										...
										{//보조도구를 작용하고 있고, 현재 장착하려는 주 도구와 다른 채집타입이라면 장착 해제 한다.
								...
								{// 장착중인 무기가 채집 도구라면
									if( JobSkillToolUtil::GetToolType( kMainWeapon.ItemNo() ) )
									{
										...
										for( int iCount = 0; iCount < CHECK_SUBTOOL_COUNT; ++iCount )
										{
											...
											{// 보조도구를 착용중이라면 같이 장착 해제한다.								
												SItemPos kFitTargetPos(kTargetPos.x, kTargetPos.y+1);

												//장비 인벤에 비어 있는 곳이 있을 경우 보조도구도 같이 해제 한다.
			...
			default:
				{
					...
					else
					{
						...
						case IT_SHARE_RENTAL_SAFE4:
							{
								// 거례 불가 상태의 아이템은 공유 창고에 넣을 수 없다.
								if(false == CheckEnableTrade(kCasterItem,ICMET_Cant_UseShareRental))
								{
									iErrorRet = 20025;//<TEXT ID="20025" Text="이동할 수 없는 위치입니다."/>
									break;
								}
							}// break; 문을 사용하지 말것
						case IT_SAFE:
...
{
	// 나중에 CoolTime에 추가해 주기 위해 임시로 저장
...
void PgAction_MoveItem::AddCoolTime( PgInventory * pkInv ,CUnit * pkCaster)
{
	...
	{
		// Item Cooltime 에 추가
		//	MapServer에서 이상 없으면, 실제로 CoolTime을 적용시키자.
...
{
	// 먼저 보낸 포지션 체인지 오더가 처리된 후에 이오더가 처리되니까 포지션을 반대로 넣어주어야 한다.
	...
	if ( pkPet )
	{
		...
		DWORD const dwServerElapsedTime = g_kEventView.GetServerElapsedTime() + 2000;//2초정도는 버리자
...
bool PgAction_UseItem::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	////////////////////////////////////////
	// CAUTION :::::::::::::::::::::::::::
	// m_bRealAction == true 일때만 정말로 아이템의 효과를 적용시켜 주어야 한다.
	// 이유 : 실제로 소모되는 아이템이 아닌데, 소모아이템으로 등록되어 있는 것이 몇가지 있다(예:Racipe, 샤인스톤 등)
	// 이 아이템들을 마우스 오른쪽 클릭으로 소모했을 때, 실제로 소모되지 않기 위해, 실제로 소모가능한 아이템인지 검사해야 한다.
	//////////////////////////////////////////////////////////////////////////////
	if ( !m_pkGround )
	{
		VERIFY_INFO_LOG(false, BM::LOG_LV0, __FL__ << _T("Ground is NULL") );
	...
	{//사망 했다면.
		...
		if(UICT_REVIVE != pkDef->GetAbil(AT_USE_ITEM_CUSTOM_TYPE)) // 예수의 십자가는 사용자는 살아 있어야 하고 효과를 받는 사람은 죽어있어야 한다.
		{
			if(ITEM_USE_STATE_DEAD != pkDef->GetAbil(AT_ITEM_USE_STATUS))
			{
				LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
...
bool PgAction_UseItem::DoInstantEffect(CItemDef const* pkDef, CUnit* pkCaster, CUnit* pkTarget)
{
	...
	case UICT_SMS:
		{
			...
			if(pkPlayer)
			{
				...
				{
				case SMS_IN_GROUND:	// 자신이 위치한 맵에만 보낸다.
					{
				...
				case SMS_IN_CHANNEL: // 체널 전체 맵에 보낸다.
					{
				...
				case SMS_IN_REALM:	 // 랠름 전체 맵에 보낸다.
					{
						BM::Stream kPacket(PT_M_T_REQ_SMS);
						kSMSHeader.WriteToPacket(kPacket);
						kSendExtraDataPackInfo.PushToPacket(kPacket);
	...
	case UICT_SUPER_GROUND_FEATHER:
		{
			if(false == pkCaster->IsAlive())	// 이미 다른 이유 때문에라도 살아 났다면 처리 할 필요 없음... 아이템만 날린거?
			{
...
bool PgRegQuickSlot::DoAction(CUnit* pkUser, CUnit* pkNothing)
{
	...
	{
		GET_DEF(CItemDefMgr, kItemDefMgr);
		CItemDef const* pkItemDef = kItemDefMgr.GetDef(m_rkQuickInvInfo.ID());

		if (pkItemDef
			&& pkItemDef->GetAbil(AT_ATTRIBUTE) & ICMET_Cant_UseQuickSlot)
		{
			BM::Stream kPacket(PT_M_C_NFY_WARN_MESSAGE, 2517);
...
bool IsAllNeedRepair( CUnit* pkCaster )
{
	...
	PgInventory* pkInv = pkCaster->GetInven();	
	CONT_HAVE_ITEM_DATA kFitCont;
	pkInv->GetItems( IT_FIT, kFitCont );	// 장착한 모든 아이템	
	...
	while( kFitItor != kFitCont.end() )
	{
		...
		switch( eEquipPos )
		{
		case EQUIP_POS_HELMET:		// 투구
		case EQUIP_POS_SHOULDER:	// 어깨
		case EQUIP_POS_SHIRTS:		// 상의
		case EQUIP_POS_PANTS:		// 하의
		case EQUIP_POS_GLOVE:		// 신발
		case EQUIP_POS_BOOTS:		// 장갑
		case EQUIP_POS_WEAPON:		// 무기
			{
				if( !kItem.EnchantInfo().IsNeedRepair() )
				{// 하나라도 수리요구 플래그가 꺼져있으면 업적 달성 조건 안됨
					return false;
				}
				else
				{
...
void PgDeathPenalty::ProcessEquipPenalty(CUnit* pkCaster, CONT_PLAYER_MODIFY_ORDER &kOrder)
{
	...
	{
		return;// 내구도 소모 방지 어빌이 있으면 여기서 리턴
	}
	...
	while(iIndex)
	{
		...
		//		WORD const wDecCount = static_cast<WORD const>((float)(pkItemDef->GetAbil(AT_DEFAULT_AMOUNT)*(100)) /(float)100);//100% 테스트용.
				WORD const wNowCount = kItem.Count();
				if(wNowCount > wDecCount)
				{//Now 가 많으니 단순 까임.-> - 되어서 오버플로우 때문에.
					SPMOD_Modify_Count kDelData(kItem, kItemPos, -wDecCount);//깜.
				...
				{//채집도구인데 다음 내구도가 0이하로 떨어지면 무조건 삭제
				...
				{//넘어서면. NeedRepair 상태로 전환. 내구를 깔 필요는 없음.
...
//	if (pkUnit->GetState() == US_DEAD)//맵이동의 경우 dead 상태가 아닐 수도 있기 때문에.
	...
	if(m_bIsPenalty)
	{
		...
		ProcessEquipPenalty(pkUnit, kOrder);// 사망시 내구도 패널티.
	...
	if (itor_map!=pkDefMap->end())
	{
		...
		{
			return true;	//배틀스퀘어, 길드전일 경우 무적 이펙트 걸지 말자
		}
	}
	...
	kCreate.eOption = SEffectCreateInfo::ECreateOption_CallbyServer;
	pkUnit->AddEffect( kCreate);//무적 이펙트를 걸어줌
...
//////////////////////////////////////////////////////////////////////////////
PgThrowUpPenalty::PgThrowUpPenalty(SGroundKey const &kGndKey, bool const bMissionOutMove)// 미션 포기 패널티.
	:m_kGndKey(kGndKey)
...
bool PgThrowUpPenalty::DoAction(CUnit* pkUser, int const iExpRate)
{
	...
	{//미션에서 나오는 것은 부활을 시키고 나오는 것이라. 예외적으로 패널티를 여기서 먹임.
		if (m_bMissionOutMapMove)
		{
			PgPlayer* pkPlayer = dynamic_cast<PgPlayer*>(pkUser);
			pkPlayer->Alive(EALIVE_NONE, E_SENDTYPE_BROADALL | E_SENDTYPE_MUSTSEND, pkUser->GetAbil(AT_C_MAX_HP)/2, pkUser->GetAbil(AT_C_MAX_MP)/2);//다알려야지
			...
			PgDeathPenalty kAction(m_kGndKey, LURT_MapMove, true);//어떻게든 나가는건 패널티를 먹임.
			kAction.DoAction(pkUser, NULL);
		}
		else
		{
			// Mission에서 죽었는데, 바로 Logout 했음.
			// Penalty를 바로 적용할 수 없고, Penalty를 주어야 한다고 표시만 한다.
			//	이유 : Penalty명령이 ContentsServer에 받았을 때, player정보가 이미 사라졌을 경우가 있다.
			pkUser->SetAbil(AT_HP, HP_VALUE_MISSION_MAP_PENALTY);
		}
	}
	else
	{//사망 아닌 상태. 사망 아니니 사망 패널티 없음.
		SEffectCreateInfo kCreate;
		kCreate.eType = EFFECT_TYPE_PENALTY;
		kCreate.iEffectNum = ms_iEffectNo;
		kCreate.iValue = iExpRate;
		kCreate.eOption = SEffectCreateInfo::ECreateOption_CallbyServer;	
		pkUser->AddEffect( kCreate );// 패널티 이펙트를 붙이고. 
...
bool PgAction_ReqUseItem::CheckItemEffect_Player(CItemDef const* const pkDef, CUnit* pkCaster, PgBase_Item const & kItem, DWORD & Result)
{
		...
		||	iRecoveryJSProductPointValue )
		{
			...
			if( pkPlayer )
			{
				if( iRecoveryJSExhaustionRate || iRecoveryJSExhaustionValue )
				{
					...
					{
						SPMO kIMO(IMET_JOBSKILL_ADD_EXHAUSTION, pkCaster->GetID(), SPMOD_JobSkillExhaustion(-iAddValue)); // 피로도 회복
				...
				if( iRecoveryJSBlessPointRate || iRecoveryJSBlessPointValue )
				{
					...
					{
						SPMO kIMO(IMET_JOBSKILL_ADD_BLESSPOINT, pkCaster->GetID(), SPMOD_JobSkillExhaustion(-iAddValue)); // 피로도 회복
						m_kOrder.push_back( kIMO );
						++iUseEffectCount;
					}
					else
	...
	case UICT_FORCED_ENCHANT:
		{
			...
			int const iLevel = BM::Rand_Range(iResultLvMax,iResultLvMin);
	...
	case UICT_COUPLE_ITEM: //커플용 아이템일 때
		{//iCustomValue1 = 커플 사이의 거리
	...
	case UICT_CASH_PACK:
		{
			...
			if(NULL != pkPlayer)
			{
				...
				m_kCashAdd = SPMO(IMET_ADD_CASH, pkPlayer->GetID(), SPMOD_AddCash(pkPlayer->GetMemberGUID(), static_cast<__int64>(iCustomValue1), static_cast<__int64>(iCustomValue2), pkPlayer->Name(), CIE_Cash_Pack));
				++iUseEffectCount;

				BM::Stream kPacket(PT_M_C_NOTI_CASH_PACK);
				kPacket.Push(static_cast<__int64>(iCustomValue1));
				kPacket.Push(static_cast<__int64>(iCustomValue2));
				pkPlayer->Send(kPacket);
			}
		}break;
	...
	case UICT_SAFE_EXTEND_MAX_IDX:
		{
			...
			switch(g_kLocal.ServiceRegion())
			{//신규 런칭되는 국가들은 프리미엄 아이템이 없고, 같은 인벤토리 확장 아이템 사용
	...
	case UICT_ELIXIR:
		{//
			...
			{
				//회복이 필요혀.
				int iRecoverValue = __min(iMaxValue - iNowValue, kItem.Count()*100);//1포인트당 100만큼. 회복
				int const iCheckValue = ( (LOCAL_MGR::NC_USA == g_kLocal.ServiceRegion()) ? 0 : 99 );// 북미는 회복할 양이 있으면 무조건 사용, 다른곳은 100이상부터 사용
				if ( iCheckValue < iRecoverValue )
				{
					...
					SPMOD_Modify_Count kDelData(kItem, m_kCasterPos, -iDecDur);//1씩 감소.
	...
	case UICT_REVIVE:
		{
			...
			SPMOD_AddRankPoint kAddRank( E_RANKPOINT_USEPHOENIXFEATHER_COUNT, 1 );// 랭킹 올려.
	...
	case UICT_STATUS_RESET:
		{
			...
			{
				pkCaster->SendWarnMessage(790657);	// 초기화 되어 있어 사용할 수 없습니다.
			}
			else
			{
	...
	case UICT_RECOVERY_STRATEGYFATIGABILITY:
		{	// 전략 피로도 회복 아이템 사용.
			int const RecoveryAmount  = pkDef->GetAbil(AT_USE_ITEM_CUSTOM_VALUE_1);	// 전략 피로도 회복량.

			SSyncVariable SyncVarialbe = g_kEventView.VariableCont();
			int MaxStrategyFatigability = SyncVarialbe.iMaxStrategyFatigability;	// 최대 피로도. 최대 피로도를 넘지 않는 선에서 회복시켜야 함.

			int CurFatigability = pkCaster->GetAbil(AT_STRATEGY_FATIGABILITY);	// 현재 피로도.
			if( CurFatigability == MaxStrategyFatigability )
			{	// 현재 피로도가 최대치라면 아이템을 사용하지 않음.
				pkCaster->SendWarnMessage(750046);
				return false;
			}
			
			int FinalValue = RecoveryAmount + CurFatigability;
			if( FinalValue > MaxStrategyFatigability )	// 피로도 최대값 보다 큰가?
			{
				FinalValue = MaxStrategyFatigability; // 크면 최대값으로 변경.
			}

			pkCaster->SetAbil(AT_STRATEGY_FATIGABILITY, FinalValue, true);	// 피로도 변경.
		}break;
	case UICT_R_RECOVERY_STRATEGYFATIGABILITY:
		{	// 전략 피로도 회복 아이템 사용.
			...
			if( RecoveryAmout > MaxStrategyFatigability )	// 피로도 최대값 보다 큰가?
			{
				RecoveryAmout = MaxStrategyFatigability; // 크면 최대값으로 변경.
			}

			pkCaster->SetAbil(AT_STRATEGY_FATIGABILITY, RecoveryAmout, true);	// 피로도 변경.
		}break;
...
bool PgAction_ReqUseItem::CheckItemEffect_Pet(CItemDef const* const pkDef, CUnit* pkCaster, PgPet *pkTargetPet, PgBase_Item const & kItem)
{
	...
	case UICT_ELIXIR:
		{//
			...
			{
				//회복이 필요혀.
				int iRecoverValue = std::min<int>( iMaxValue - iNowValue, kItem.Count() * 10 );// 펫은 1포인트당 10만큼. 회복
				...
				if ( iCheckValue < iRecoverValue )
				{
					...
					{
						SPMOD_Modify_Count_Pet kDelData( pkCaster->GetID(), kItem, m_kCasterPos, -iDecDur );//1씩 감소.
...
int PgAction_ReqUseItem::CheckEnableUseEffect( CItemDef const* const pkDef, int const iItemNo, int const iEffectNo, CUnit* pkTarget )
{
	...
	{// HP, MP 회복류의 경우 HP,MP 가 MAX이면 무조건 실패 해야 한다.
		return iError;
	}
	else
	{
...
bool PgAction_ReqUseItem::FilterQuest(CUnit * pkUnit,CItemDef const* const pItemDef)
{
	...
	{
		iQuestNo = pItemDef->GetAbil(i);		// 해당 퀘스트를 완료 해야 사용 가능 하다.
	...
	{
		iQuestNo = pItemDef->GetAbil(i);		// 해당 퀘스트가 완료 하지 않아야 사용 가능 하다.
	...
	{
		iQuestNo = pItemDef->GetAbil(i);		// 해당 퀘스트를 진행중이어야만 사용 가능 하다.
	...
	{
		iQuestNo = pItemDef->GetAbil(i);		// 해당 퀘스트를 진행 중이면 사용 불가능 하다.
		if(iQuestNo && pkQuest->IsIngQuest(iQuestNo))
		{
...
int PgAction_ReqUseItem::CheckUseTime( CItemDef const &rkItemDef )
{
	...
	if(iStartTime > 0 || iEndTime > 0)	// Start/End 모두 0시 설정은 안된다.(하고 싶으면 24~24로 설정해야 한다.)
	{
		if (iStartTime > 24 || iStartTime > iEndTime)	// 24시넣고 싶으면 20시~26시 이런식으로 입력하면 된다.
		{
...
bool PgAction_ReqUseItem::DoAction( CUnit* pkItemOwner, CUnit* pkPlayer )
{
	...
	if((ICMET_Cant_UsePVP & pItemDef->GetAbil(AT_ATTRIBUTE)) && (GATTR_PVP == m_pkGround->GetAttr()))
	{	// 사용할 수 없는 장소입니다.
		pkPlayer->SendWarnMessage(20027);
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
		return false;
		...
		{
			{
				...
				{
					int const iMaxExhaustion = JobSkillExpertnessUtil::GetMaxExhaustion_1ST(pkTrgPlayer->GetPremium(), pkTrgPlayer->JobSkillExpertness().GetAllSkillExpertness()); // 1차 주 스킬만 피로도가 있다
			...
			}
			{
				...
				if( 0 < iLimitUseBlessPoint )
				{
					...
					int const iMaxBlessPoint = JobSkillExpertnessUtil::GetMaxExhaustion_2ND(pkTrgPlayer->GetPremium(), pkTrgPlayer->JobSkillExpertness().GetAllSkillExpertness()); // 2차 주 스킬만 축복게이지가 있다
	...
	&& (byUseInvenType == m_kCasterPos.x))//본인 메인 인벤 자리가 아니면 사용 불가. -> 즉. 창고 아이템은 안된다는 얘기.
	{
		if ( !m_bAnsServerCheck )
		{
			int const iMapNo = pItemDef->GetAbil(AT_MAP_NUM);
			if ( iMapNo )
			{// Scroll Item은 센터로 부터 체크를 받아야 한다.
		...
		// Item 사용이라면, 정말로 사용할 수 있는지 검사 해 준다.
		DWORD Result = 0;
		...
		case UICT_REDICE_ITEM_OPTION:
		case UICT_REPAIR_ITEM:
			{//CheckInstantEffect 함수 안에서 계산된다. 여기선 사용 하지 않음
			}break;
		case UICT_CALL_SAFE:
		case UICT_CALL_SHARE_SAFE:
			{
				if(0 == kItem.EnchantInfo().IsTimeLimit()) // 기간제 아이템은 무조건 사용
				{
					SPMOD_Modify_Count kDelData(kItem, m_kCasterPos, -1);//1씩 감소.
		...
		case UICT_ELIXIR:
			{// 영생의 물약은 CheckInstantEffect 함수 안에서 계산된다. 여기선 사용 하지 않음
			}break;
		case UICT_REST:
		case UICT_REST_EXP:
		case UICT_COUPLE_ITEM:
            {
                if(kItem.EnchantInfo().IsTimeLimit()) // 기간제 아이템은 무조건 사용
                {
                    PgAction_UseItem kAction( m_pkGround, kItem, 1);
                    kAction.DoAction(pkItemOwner, NULL);
		...
		case UICT_PET_EGG:
			{
				// 펫을 부화 시켜야 하네~~
				SPMOD_HatchPet kData( kItem, m_kCasterPos, -1 );// 1씩 감소
		...
		case UICT_PET_FEED:
			{
				SItemPos kPetItemPos = PgItem_PetInfo::ms_kPetItemEquipPos;// 아이템 포지션이 않오면 착용하고 있는 아이템
				...
				case PET_FEED_TYPE_2:
					{
						if ( static_cast<BYTE>(iType2) != kPetDef.GetPetType() && 
							 static_cast<BYTE>(iType3) != kPetDef.GetPetType() ) //2가지 펫타입 종류에 사용할 수 있게 하자
						{
							return false;
						}
						...
						{
							return false;// 기간제 펫이 아니기 때문에 음식을 줄 필요가 없다.
						}
				...
				case PET_FEED_TRAINING:
					{
						...
						{
							__int64 const i64NowTime = (g_kEventView.GetLocalSecTime(CGameTime::HOUR) + 1i64 );// 1시간을 더줘야 분단위 손해를 만회
		...
		case UICT_ACTIVATE_SKILL:
			{
				...
				{
					pkPlayer->SendWarnMessage(119);//습득할수 없는 스킬입니다.
					return false;
				}
		...
		case UICT_SUPER_GROUND_MOVE_FLOOR:
			{
				...
				m_kOrder.push_back( SPMO(IMET_MODIFY_COUNT|IMC_DEC_DUR_BY_USE, pkItemOwner->GetID(), SPMOD_Modify_Count(kItem, m_kCasterPos, -1)) ); //1씩 감소.
			}break;
		case UICT_MISSION_DEFENCE_POTION:
			{
		...
		case UICT_GUILD_INV_EXTEND_TAB:
			{
				...
				EInvType const kInvType = static_cast<EInvType>(IT_NONE); // 임시 값: 컨텐츠에서 처리
				BYTE const kExpendNum = static_cast<BYTE>(iCustomValue1); // 확장 할 칸 수
		...
		default:
			{//
				//아이템 유즈 허가를 받음. 아템 서버는 아이템 지워주는 작업까지 할 것.
				SPMOD_Modify_Count kDelData(kItem, m_kCasterPos, -1);//1씩 감소.
				SPMO kIMO(IMET_MODIFY_COUNT|IMC_DEC_DUR_BY_USE, pkItemOwner->GetID(), kDelData);
				m_kOrder.push_back(kIMO);
			}break;
		...
		{
			return true;// 맵 이동 아이템은 여기서 리턴해준다. 처리는 밖에서 한다. 좀 꼴이 뭐하지만 방법이......
		}

		// 맵 이동 아이템이 아니면 여기서 아이템 수정 요청 처리
...
{
//	bool const bRet = g_kShineStoneMgr.BuildPlayerStone(m_kGndKey, pUser);//샤인스톤 해당맵 초기화.
//	return bRet;
	return true;
}
...
bool PgAction_PopItemBag::DoCreatePack(CUnit *pUser)
{
	...
	PgAction_CreateItem kCreateItem(m_eCause,m_kGndKey,m_kOrderList);//아이템 지급 요청
		...
		if(::RouletteRate((*item_cont_itor).second.iSuccessRateControlNo, iRetIDX, MAX_ITEM_CONTAINER_LIST))
		{
			...
			if( S_OK == kItemBagMgr.GetItemBagByGrp(iResultBagGroupNo, static_cast<short>(iLevel), kItemBag))
			{
				...
				if(S_OK == kItemBag.PopMoney(iMoney, iAddMoneyDropRate))
				{
					if(iMoneyAddRate)
					{
						iMoney += SRateControl::GetValueRate<int>( iMoney, iMoneyAddRate );//이벤트 보너스.
					}
...
bool PgAction_RewardItem::DoAction(CUnit *pkUser, CUnit* Nothing)
{
	...
	PgAction_CreateItem kCreateItem(m_eCause,m_kGndKey,kItemOrderList);//아이템 지급 요청
...
{
	if(	pkCaster
	&&	!m_bIsIgnoreDeath//죽음 무시 플래그. 꺼진 상태이고
	&&	!pkCaster->IsAlive())//죽어있다면 실패.
	{
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
		return false;
	}
	...
	if(	pkTarget
	&&	!m_bIsIgnoreDeath//죽음 무시 플래그. 꺼진 상태이고
	&&	!pkTarget->IsAlive()//죽어있다면 실패.
	&&	UT_MYHOME != pkTarget->UnitType()) // 마이홈이 아니면 ( 마이홈이 왜 죽어 있는건지 참... )
	{
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
		return false;	
	}
...
//Clear!!
	...
	if ( SUCCEEDED(pkCaster->PopDropItem(kTempContDropItem)) )//기존꺼 클리어.
	{
...

		{
			...
			if(0 == iItemContainerNo)
			{
				...
				&&	lwIsRandSuccess(iBaseDropMoneyRate) )
				{
					if(iMoneyAddRate)
					{
						iBaseDropMoney = ((iBaseDropMoney*iMoneyAddRate)/100);//이벤트 보너스.
					}
...
=====================================================================================================================================
	기획 요청으로 아이템 드랍 횟수와 돈 드랍 횟수를 분리 했으며 해당 어빌이 들어 있지 않으면 기본 1개 드랍으로 설정
=====================================================================================================================================
		{
			...
			while( iRetMoneyCount )
			{
				...
				{//확률표를 돌아 인덱스가 나왔다.
					if(S_OK == akItemBag[iIDX].PopMoney(iMoney, iAddMoneyDropRate))
					{
						if(iMoneyAddRate)
						{
							iMoney = ((iMoney*iMoneyAddRate)/100);//이벤트 보너스.
						}
...
bool PgAction_DecEquipDuration::GetEquipRandomPos(bool const bIsAttacker, CUnit* pkCaster, CONT_DEC_POS& rkOut, size_t const iMaxCount)//내구도 깎을때 포지션을 받아오는 부분
{
	if( pkCaster->GetAbil(AT_PROTECTION_GALL) )
	{
		return false;
...
//		return g_aiDefenceCandidate[pos]; 
		...
		while( PgArrayUtil::IsInArray(pkBegin, g_aiDefenceCandidate) )
		{
			...
			&&	!kOutItem.EnchantInfo().IsNeedRepair())//수리 요구면. 깔 수 없음.
			{
...
//#endif
	...
	if( GetEquipRandomPos(m_bIsAttacker, pkCaster, kDecPos, iMaxDecPosCount) )// 포지션이 있어야함.
	{
		CONT_PLAYER_MODIFY_ORDER kOrder;
...
{
	if(m_kContRepair.empty())//타겟이 없으면 전부 수리.
	{
		static int const iCandiSize = sizeof(g_aiDeathPenaltyCandidate)/sizeof(int);
...
{//개별 수리.
	if(pkCaster)
	{
		...
		{
			{
				SPMOD_Add_Money kDelMoneyData(-iNeedMoney);//필요머니 빼기.
				SPMO kIMO(IMET_ADD_MONEY, pkCaster->GetID(), kDelMoneyData);
				kOrder.push_back(kIMO);
			}
...
bool PgAction_ReqRemoveInvItem::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	if(pkCaster)
	{
		...
		while(itor != m_kOrder.end())
		{
			...
			{
				SPMOD_Modify_Count kDelData(kItem, (*itor).kCasterPos, 0, true);//삭제.
				...
				if(0 != m_iDeleteType)
				{
					if(kItem.EnchantInfo().Rarity() > 0) // 유저가 아이템 파괴를 요청 했을경우는 레어도가 없는 아이템은 파괴 할 수 없다.
					{
						//여기부터 아이템 깨먹는것.
...
//		PgAction_AddExp
//////////////////////////////////////////////////////////////////////////////
__int64 PgAction_AddExp::ms_i64TakePCExpRate = 1000i64;//PC로 부터 받는 경험치의 만분율
...
{
	// UT_ENTITY가 몬스터 잡은 경우에도 처리해 주어야 한다.
	//if( !m_iAddExp )//경험치가 0이면
	//{
	//	LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
	//	return false;
	//}
	...
	case UT_SUMMONED:
		{
			...
			if(pSummoned)
			{
				//kPartyGuid = pSummoned->Party();//필요할까?
			}
		}break;
	case UT_PLAYER:
		{
			...
			if( pkPlayer )
			{
				...
				else if( 0 == (AEC_PetExp & m_eCause) ) // 펫이 주는 경험치는 메크로 검사에 걸리지 않음
				{
					CGameTime kCurTime;
	...
	if( !bShareParty		//파티가 없거나
	|| 0 > m_iAddExp	//감소
	)
	{
		return PassAdvance(pkCaster, pkTarget);//즉시 지급
	}
	...
	if( bFindParty )
	{
		...
		__int64 const i64RetExp = PgAction_AddExp::ResultExp(m_iAddExp, 100);	// 100은 기본값
	...
	{
		goto __PASS_ADVANCE;//뭔지 모르지만 패스
	}
	return true;

__PASS_ADVANCE://뭔진 몰라도 실패
...
{
	// iAddExpRate = PlayerBasic(100) + 각종버프(콤보+경험치아이템+채널입장버프+휴식경험치+마이홈가구배치버프등등) + 파티분배
	/* 
		사냥에 의한 경험치 공식 =
			몬스터경험치 * 레벨차에의한보정 * 이벤트경험치 * iAddExpRate
	*/
	/*
...
bool PgAction_AddExp::PassAdvance(CUnit* pkCaster, CUnit* pkTarget)
{
	...
	if( UT_PLAYER == pkCaster->UnitType() )
	{
		...
		if( pPlayer )
		{
			...
			if( DefenceBonus > 0 )
			{
				...
				m_iAddExp = static_cast<int>( m_iAddExp * DefenceBonus );	// 경험치 보너스 비율 적용. 소수점 이하 버림.
			}
		}
	}
	...
	static int iRestBonusExpRate = g_kEventView.VariableCont().iExpAdd_AddedExpRate_Hunting;	//휴식경험치 보너스
	__int64 const i64MaxRestBonusExp = pkCaster->GetAbil64(AT_REST_EXP_ADD_MAX);
	__int64 const i64CurExp = pkCaster->GetAbil64(AT_EXPERIENCE);
	if ( 0 == (AEC_PetExp & m_eCause) )
	{
		// 펫이 주는 것이 아닐 경우에만 적용 한다.
		if (i64CurExp < i64MaxRestBonusExp) // 펫이 주는 경험치는 휴식 경험치에 적용 받지 않는다.
		{
	...
	case UT_PLAYER:
		{
			...
			&&	iCanAddExpMaxLevel >= iDiffLv )//레벨차가 범위 안이면 ok
			{
				...
				if(S_OK == pkInv->GetActivateExpCardItem(kItem,kItemPos))
				{
					...
					if(true == kItem.Get(kExpCard))
					{
						...
						if( kExpCard.MaxExp() <= iNextExp ) // 꽉 찼다. 초기화...
						{
				...
				if(0 < i64UserExp)// 경험치 카드로 들어갈 경험치량이 100%가 아니면 남은건 유저에게 준다.
				{
					if(true == IsRestMode)
					{
						double const dExpPer = i64UserExp/i64RetExp;

						__int64 const i64OrgExp = PgAction_AddExp::ResultExp(m_iAddExp,iFinalAddExpRate+m_iAddedExpRate) * dExpPer;	// 아이템, 이벤트 적용된 최종 경험치
						__int64 const i64ExtraExp = std::max<__int64>(0,i64RetExp - i64OrgExp) * dExpPer; // 추가 휴식 경험치 
			...
			if( !kOrder.empty() )
			{
				if( (AEC_KillMonster|AEC_PetExp|AEC_AlramMission) & m_eCause )
				{
					...
					kItemModifyAction.DoAction(pkCaster, NULL);//누가 죽었는지는 중요치 않다.
				} 
...
bool PgAction_ShareExpParty::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	...
	&&	iCanAddExpMaxLevel >= iDiffLv )//레벨차가 범위 안이면 ok
	{
		...
		{
			// 플레이어의 현재 경험치 값이 ContentsServer로 부터 update되지 않아, 조건문이 약간 부정확할 수 있다.
			iPrivateAddExpPer += iRestBonusExpRate;
			bIsRestExpMode = true;
		}

		// static_cast<int>(fPartyBonusRate * 100.f) <-- fPartyBonusRate = 0.3000001f 로 저장되어 경험치가 1만큼 차이 날 수 있다.
...
bool PgAction_ShareGold::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	...
	{
		if(m_kCause == MCE_Loot)// 파티 없고 단독 사냥일 경우 경험치
		{
	...
	else
	{
		VEC_GUID kVec;
		bool bFindParty = false;
		if( bExpedition )		// 원정대라면 원정대 멤버를 모두 얻어옴.
		{
...
__int64 PgAction_ShareGold::PartyMemberShareGold(__int64 const iAddGold, int const iMemberCount)
{
	...
	return std::max(iResultGold, 1i64); //최소 1
}
...
bool PgAction_DropItemBox::DoAction( CUnit* pkCaster, CUnit* pkTarget )
{
	...
	if(	pkCaster && pkPC )
	{
		...
		int const iLevel = pkPC->GetAbil( AT_LEVEL );
		int const iSuccessRateCtrlNo = pkCaster->GetAbil(AT_ITEM_BAG_SUCCESS_RATE_CONTROL);
		int const iAddMoneyDropRate = (pkEventAbil ? pkEventAbil->GetAbil(AT_ADD_MONEY_RATE) : 0);
		...
		int const iEventItemContainer = (pkEventAbil ? pkEventAbil->GetAbil(AT_EVENT_ITEM_CONTAINER) : 0);
		if( 0 < iEventItemContainer )
		{
			CONT_DEF_ITEM_CONTAINER const *pkContContainer = NULL;
			g_kTblDataMgr.GetContDef(pkContContainer);
	...
	//넣기 패킷으로 구성.
{
	...
	{//인벤 비우기
		...
		SPMO kIMO(IMET_MODIFY_COUNT|IMC_POS_BY_EXCHANGE, kCaster, kDelData);//교환 플래그로 실제 지워지게는 안함.
		kOrder.push_back(kIMO);
...
}
{
	...
	{//인벤 비우기
		SPMOD_Modify_Count kDelData((*itor).kItem, (*itor).kPos, 0, true );
		SPMO kIMO(IMET_MODIFY_COUNT|IMC_POS_BY_EXCHANGE, kTarget, kDelData);//교환 플래그로 실제 지워지게는 안함.
		kOrder.push_back(kIMO);
...
}
{
	...
	{//아이템 추가
		SItemPos kItemPos;//Temp Value
		SPMOD_Insert_Fixed kAddData((*itor).kItem, kItemPos, true);//1씩 감소.
		SPMO kIMO(IMET_INSERT_FIXED|IMC_POS_BY_EXCHANGE, kTarget, kAddData);//캐스터의 아이템을 타겟에게
		kOrder.push_back(kIMO);
...
}
{
	...
	{//아이템 추가
		SItemPos kItemPos;//Temp Value
		SPMOD_Insert_Fixed kAddData((*itor).kItem, kItemPos, true);//1씩 감소.
		SPMO kIMO(IMET_INSERT_FIXED|IMC_POS_BY_EXCHANGE, kCaster, kAddData);//타겟의 아이템을 캐스터에게
		kOrder.push_back(kIMO);
...
//			TradeListToOrderCont(m_kInfo.kTargetGuid, m_kInfo.kCasterGuid, m_kInfo.kTargetItemCont, kOrder);
			...
			//줄돈 제거
			//if (-m_kInfo.i64CasterMoney + m_kInfo.i64TargetMoney)//같은 돈을 교환하면 이 루틴을 타지 못함!
			{
	...
	case ESMT_REVIVE_MISSION:
		{
			...
			if ( m_pkGnd->GetAttr() & GATTR_MISSION )
			{
				...
				if( pkUser->HaveExpedition() )	// 원정대에 가입되어 있다면, 자신의 상태 변화를 알려야 함.
				{
					BM::Stream Packet(PT_M_N_NFY_EXPEDITION_MODIFY_MEMBER_STATE);
				...
				PgDeathPenalty kAction(m_pkGnd->GroundKey(), LURT_Gold, true);//돈 써서 살아나는것은. 패널티.
				kAction.DoAction(pkUser, NULL);
			}
			else
			{
...
//{	//레벨 * 스킬 포인트
//	//초기 지급 스킬은 NeedSkillPoint가 0 이어야함.
//	
//	//PgPlayer* pkPlayer = dynamic_cast<PgPlayer*>(pkCaster);
//	//PgMySkill* pkMySkill = pkPlayer->GetMySkill();
//
...
{	
	...
	int newCount = kItem.Count() - m_iDivCount;
	if(newCount <= 0)
	{
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
		return false;
	...
	{
		kItem.Count(newCount);
		pkInv->Modify(m_kItemPos, kItem);
	}
}
{
	kDividedItem.Guid(BM::GUID::Create());//New Guid
	kDividedItem.Count(m_iDivCount);

	tagPlayerModifyOrderData_Insert_Fixed kAddItem(kDividedItem, kEmptyItemPos, false);
	SPMO kIMO(IMET_INSERT_FIXED, pkCaster->GetID(), kAddItem);
	kOrder.push_back(kIMO);
}
	...
	case EFollow_Cancel:
		{
			...
			{
				// pkTargetPlayer 가 존재 하지 않더라도 성공해야 한다.
...
bool PgAction_ReqGuildLevelUp::DoAction(CUnit *pkCaster, CUnit *Nothing)
{
	...
	CONT_DEF_GUILD_LEVEL::const_iterator def_iter = pkDefGuildLevel->find(m_sGuildLv);//현재 레벨
	...
	CONT_DEF_GUILD_LEVEL::const_iterator nextdef_iter = pkDefGuildLevel->find(m_sGuildLv+1);//다음 레벨
	if( pkDefGuildLevel->end() == nextdef_iter )
	{
		m_kResult = GCR_Max;//현재가 최대 레벨
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
		return false;
	}
	...
	CONT_PLAYER_MODIFY_ORDER kOrder;
	SPMOD_Add_Money kDelMoneyData(-rkNextDefGuildLevel.iGold);//필요머니 빼기.
	SPMO kIMO(IMET_ADD_MONEY, pkCaster->GetID(), kDelMoneyData);
	kOrder.push_back(kIMO);
	...
	kAddonPacket.Push(false);//테스트 아님
...
{	
	...
	CONT_PLAYER_MODIFY_ORDER kOrder;
	SPMOD_Add_Money kDelMoneyData(-iCreateMoney);//필요머니 빼기.
	SPMO kIMO(IMET_ADD_MONEY, pkCaster->GetID(), kDelMoneyData);
	kOrder.push_back(kIMO);
...
bool PgAction_ReqGuildLearnSkill::DoAction(CUnit *pkCaster, CUnit *Nothing)
{
	...
	CONT_PLAYER_MODIFY_ORDER kOrder;
	SPMOD_Add_Money kDelMoneyData(-rkGuildSkillDef.iGold);//필요머니 빼기.
	SPMO kIMO(IMET_ADD_MONEY, pkCaster->GetID(), kDelMoneyData);
	kOrder.push_back(kIMO);
	...
	SPMOD_AddSkill kAddSkillData(m_iSkillNo);//꼭 SP를 먼저 빼라.
	kOrder.push_back(SPMO(IMET_ADD_SKILL, pkCaster->GetID(), kAddSkillData));
...
bool PgAction_ReqTrapDamage::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	...
	// pkCaster : 현재는 NULL
	// pkTarget : Trap 에 맞은 Unit
	float const fDamageRate = 0.08f;	// Damage 크기 (남은 HP에서 해당 Rate 값 만큼)
	int const iCurrentHP = pkTarget->GetAbil(AT_HP);
	int const iDamage = (m_pkGround->GetAttr() == GATTR_RACE_GROUND) ? 0 : __max(10, int(iCurrentHP * fDamageRate)); //달리기 이벤트에선 트랩에 걸려도 대미지 까지 말자
	...
	if ( 0 >= iRemainHP )
	{
		// 죽으면 자살이니까 Target을 Clear
		// SetAbil에서 VOnDie를 호출하니까 미리 Clear해놓아야 한다.
...
bool PgAction_ReqCollisionDamage::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	...
	// pkCaster : 충돌데미지를 준 놈
	// pkTarget : 충돌데미지를 맞은 Unit
	
	int iSkillNo = pkCaster->GetAbil(AT_COLLISION_SKILL);	//터치 데미지 입을 때 써야 할 스킬
	GET_DEF(CSkillDefMgr, kSkillDefMgr);
	if(0==iSkillNo)	//없으면 기본
	{
	...
	else
	{
		...
		// 터치 공격일 경우는 회피가 되지 않아야 한다.
		// 임시로 명중률을 극한으로 올려 회피가 되지않도록 처리
...
{
	// 특정 Ground 에 입장하면 생기는 버프(=Effect) 효과 주기
	if (m_pkGround == NULL)
	{
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
		return false;
...
bool PgAction_GroundEffect::CheckGroundEffect(PgGroundEffect const *pkGEffect, CUnit* pkCaster)
{
	...
	if(iGenderLimit != 0 && 0 == (iGenderLimit & pkCaster->GetAbil(AT_GENDER)))//성별 검사.
	{
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
		return false;
	}
...
bool PgAction_ReqAddCp::DoAction(CUnit* pkCaster, CUnit* pkTarget)
{
	...
	{
		CONT_PLAYER_MODIFY_ORDER kOrder;
		SPMOD_Add_CP kModifyCP((int)m_iAddCp);//cp 수정
...
HRESULT PgAction_AnsMoveToPartyMember::Process(CUnit* pkCaster)
{
	...
	if(!pkPlayer)
	{
		return E_CANNOT_MOVE_MAP;// 마땅히 뿌려줘야 할 에러가 없음 이동 불가 처리 하자.
	}
	...
	case UICT_SHARE_RENTALSAFE_GOLD:
		{
			...
			// 골드 아이템으로 확장 할 수 있는 건 only 공유금고1번 뿐!!
...
void PgAction_EventStart::DoAction(CONT_EVENT_STATE::mapped_type & element)
{
	...
	case ET_MONEY_GIVE_RATE:
		{
			if(		!element.bEventStarted// 한번 발생 시키면 다시 발생하면 안된다.
				&&	kTbl.aIntData[1]	
			)	
			{
				...
				kEventMsg.Push( true );//그라운드 번호만 체크
...
//				m_kContNfy.push_back(NFY_EVENT(element.m_kTable.aIntData[0], element.m_kTable.aStringData[0]));// 메세지 등록
				SEventMessage kEventMsg;
				kEventMsg.PriType(PMET_GROUND_MGR);
...
void PgAction_EventEnd::DoAction(CONT_EVENT_STATE::mapped_type & element)
{
	...
	case ET_MONEY_GIVE_RATE:
		{
			if (	element.bEventStarted // 발생되지 않은 이벤트는 취소 시킬 필요 없다.
				&&	kTbl.aIntData[1] 
			)
			{
				SGroundKey const kTargetGndKey( kTbl.aIntData[1] );
...
E_ERROR_FIT_TO_INV const PgFitToPrimaryInv::Process(CUnit * pUser)
{
	...
	if((IT_FIT == m_kItemPos.x) && (EQUIP_POS_MEDAL == m_kItemPos.y))	// 훈장은 인벤토리로 들어가지 않고 북으로 들어가야 함 -_-;; 왜~~~~~~~~~~~~~~~~~~~~~!!!!!!!!!!!!!!!!!
	{
	...
	{
		SItemPos kTmpPos;
		if(!pkInv->GetFirstEmptyPos(static_cast<EInvType>(pkDef->PrimaryInvType()),kTmpPos))	// 장착 아이템은 겹치는것 없음 빈공간 없으면 기본 인벤으로 보낼수 없음
		{
...
*/
	...
	else
	{
		...
		{
			__int64 const i64NewUseTime = (kItemCopy.EnchantInfo().UseTime() + iUseTime);	// 덴장... 
			if(MAX_CASH_ITEM_TIMELIMIT < i64NewUseTime)
			{
				return CSR_TIMELIMIT_OVERFLOW;
			}
	...
	SPMOD_Enchant kEnchantData( kItem, kItemPos, kItemCopy.EnchantInfo(),kItemCopy.CreateDate());//변경될 인첸트
...
bool PgAction_ReqRegUserPortal::DoAction(CUnit* pUser, CUnit* pkTargetUnit)
{
	...
	while(iter != kCont.end())	// 임시 -_-;; 지금은 하나만 저장한다.
	{// 하나만 저장하는것이므로 지울땐 모두 싹지운다.
...
bool PgAction_MissionPartyMemberCheck::DoAction(CUnit* pUser, CUnit* Nothing)
{
	...
	if( pkPlayer->HaveParty() )
	{
		...
		{
			// 같은 그라운드가 아닌 파티원은 목록을 가져 오지 않기 때문에
			return false;
		}
...
HRESULT const PgAction_ReqRollbackEnchant::Process(CUnit* pUser)
{
	...
	SPMOD_Enchant kEnchantData( kItem, kItemPos, kCopyItem.EnchantInfo());//변경될 인첸트
...
HRESULT const PgAction_ReqOpenLockedChest::Process(CUnit* pUser)
{
	...
	{
	case UICT_BLANK:
		{// 꽝!! 아무것도 없음 그냥 상자랑 열쇠만 날려!!
			kPacket.Push(iRewardType);
			kPacket.Push(iItemNo);
		}break;
	case UICT_ONECEMORE:
		{// 다시 한번 나왔다. 뭐 그냥 클라이언트에게만 한번 더 하라고 알리면됨 서버 데이터 수정 필요 없음!!
			kPacket.Push(static_cast<HRESULT>(S_OK));
			kPacket.Push(iRewardType);
			kPacket.Push(iItemNo);
			pUser->Send(kPacket);
		...
		if( gamble_itr != pkDef->end() )
		{
			...
			if( kGamble.bTimeType > 0 && kGamble.sUseTime > 0 )
			{
				if((0 < pRewardDef->GetAbil(AT_ENABLE_USE_TIMELIMIT)) || (true == pRewardDef->CanEquip()))
				{
					...
					{
						rkRetItem = PgBase_Item();// 초기화 시켜주자.
			...
			{
				int iMaxEnchantLv = 0;
				if ( true == PgItemRarityUpgradeFormula::GetMaxGradeLevelRestrict( ::GetItemGrade( rkRetItem ), pRewardDef->IsPetItem(), iMaxEnchantLv, rkRetItem) )
				{
...
//					int const iCustomValue2 = pItemDef->GetAbil(AT_USE_ITEM_CUSTOM_VALUE_2);
					...
					if ( S_OK == hRet )
					{
						...
						{
							// 컨텐츠 서버에서 아이템 소비가 결정되고 나면 이녀석을 다시 서버로 날려서 아이템 생성을 요청한다.
							// 인벤토리에 빈공간이 없을 경우 골라 받기를 방지 하기 위해서 오더를 나눠서 처리 함
							CONT_PLAYER_MODIFY_ORDER kOrder;
	...
	case ET_PCROOMEVENT_EFFECT:
		{
			for(int i = 0;i < MAX_EVENT_ARG;++i)
			{
				if(0 < kEvent.m_kTable.aIntData[i])
				{
					...
					kCreate.eOption = SEffectCreateInfo::ECreateOption_CallbyServer;
					pkPlayer->AddEffect(kCreate);// 패널티 이펙트를 붙이고. 
				}
			}
		}break;
	default:
...
HRESULT const PgAction_ReqUseExpCard::Process(CUnit* pUser)
{
	...
	kTime.SetTime(pUser->GetAbil(AT_ACHIEVEMENT_EXPCARD_DATE));// 차후 원시 데이터 셋팅 함수 생기면 변경할것
	...
	if(!(kTime == kToday))
	{
		int const iUseDate = kToday.GetTime();// 차후 원시 데이터 셋팅 함수 생기면 변경할것
	...
	pUser->SetAbil(AT_ACHIEVEMENT_EXPCARD_COUNT,pUser->GetAbil(AT_ACHIEVEMENT_EXPCARD_COUNT)+1);// 미리 어빌을 설정하여 두번 통과 하지 못하게 막는다
...
HRESULT const PgAction_ReqHomeEquipItem::Process(CUnit* pUser,CUnit * pTarget)
{
	...
	if(pkMyHome->OwnerGuid() != pUser->GetID() || (MAS_NOT_BIDDING != pkMyHome->GetAbil(AT_MYHOME_STATE)))// 주인이 아니거나 경매 또는 블럭 당한 상태이면 아이템 배치를 못함
	{
		return E_MYHOME_NOT_HAVE_AUTHORITY;
	}
...
bool PgAction_PartyBufRefresh::DoAction(CUnit* pkUser, CUnit* pkTarget)
{
	...
	if( 0 < kVec.size() )
	{
		...
		for( user_itr=kVec.begin(); user_itr!=kVec.end(); ++user_itr )
		{
			...
			if( pkPlayer ) // 없으면 같이 있는 파티원이 아니기 때문에 패스.
			{
				// 이 사람은 지금 전직을 했다. 이전 직업의 버프를 삭제해야 한다.
...
bool PgAction_ReqOpenTreasureChest::DoAction(CUnit* pUser, CUnit* pTarget)
{
	...
	if(iterChest == pkCont->end()) //DB 등록되지 않은 상자?
	{
...
HRESULT PgAction_Enchant_Shift::CheckInsurance(CUnit* pUser)
{
	...
	for(int i = ESIP_INSURANCEP1;i <= ESIP_INSURANCEM3; ++i)
	{
		...
		{//보험 스크롤이 맞지 않을 경우
			...
			{//일본이 아닐 경우에만 캐시 보험 인지 다시 체크하도록 변경.
				GET_DEF(CItemDefMgr, kItemDefMgr);
...
bool MakeEnchantInfo(PgBase_Item const & kSrcItem, PgBase_Item & kTargetItem, bool const bShiftSoul, bool const bShiftSpecial, bool const bShiftSocket, bool const bShiftLv)
{
	...
	if(true == bShiftLv)
	{
		...
		int iMaxLv = 0;
		if ( false == PgItemRarityUpgradeFormula::GetMaxGradeLevelRestrict( eItemGrade, false, iMaxLv, kTargetItem) )
		{
			return false;
		}
...
bool PgAction_Enchant_Shift::IsEnableShift(eEnchantShiftRate const & eRate, SEnchantShiftInfo const & kInfo)const
{
	...
	int iMaxLv = 0;
	if( false == PgItemRarityUpgradeFormula::GetMaxGradeLevelRestrict( eItemGrade, false, iMaxLv, kCopyItem) )
	{
		return false;
	}
...
HRESULT PgAction_JobSkill3_CreateItem::Process(CUnit* pUser, CUnit* /*pTarget*/)
{
	...
	SPMO kIMO(IMET_JOBSKILL_ADD_EXHAUSTION, pUser->GetID(), SPMOD_JobSkillExhaustion(iNeedProductPoint)); // 피로도소모
	m_kOrder.push_back( kIMO );
...
HRESULT const PgAction_ReqDefExchangeGemStore::Process(CUnit* pUser)
{
	...
	if( ResultItemCnt )
	{
		...
		//지정된 위치의 아이템 먼저 삭제
		if( false == pItemDef->IsAmountItem() ) // 겹쳐지는 아이템
		{
...
HRESULT PgAction_SoulTransfer_Extract::Process(CUnit* pUser, CUnit* pTarget)
{
	...
	if(pUser)
	{
		while(1)
		{
			...
			//추출하고
			PgBase_Item kTransitionItem;	//영력보관구슬 생성
...
HRESULT PgAction_SoulTransfer_Transition::Process(CUnit *pUser, CUnit *pTarget)
{
	...
	if(pUser)
	{
		while(1)
		{
			...
			if( 0 < kTransitionInfo.iNeedMoney )
			{
				SPMOD_Add_Money kDelMoneyData(-kTransitionInfo.iNeedMoney);//필요머니 빼기.
...
HRESULT PgAction_Extract_Element::Process(CUnit *pUser, CUnit *pTarget)
{
	ESoulTransMsg kMessage = ESTM_SERVER_ERR;
	HRESULT hRes = E_FAIL;
	int iResultItemNo = 0;
	...
	if(pUser)
	{
		while(1)
		{
			...
			m_kPacket.Pop(kItemPos);
			PgBase_Item kItem;
			if( E_FAIL == pInv->GetItem(kItemPos, kItem) )
			{//인벤에 아이템 없으면 에러...
				break;
			}
			GET_DEF(CItemDefMgr, kItemDefMgr);
			...
			int iItemBagNo = 0;
			if( 0 != pItemDef->GetAbil(AT_EXTRACT_ELEMENT_FIRE) )
			{
				iItemBagNo = pItemDef->GetAbil(AT_EXTRACT_ELEMENT_FIRE);
			}
			else if( 0 != pItemDef->GetAbil(AT_EXTRACT_ELEMENT_ICE) )
			{
				iItemBagNo = pItemDef->GetAbil(AT_EXTRACT_ELEMENT_ICE);
			}
			else if( 0 != pItemDef->GetAbil(AT_EXTRACT_ELEMENT_NATURE) )
			{
				iItemBagNo = pItemDef->GetAbil(AT_EXTRACT_ELEMENT_NATURE);
			}
			else if( 0 != pItemDef->GetAbil(AT_EXTRACT_ELEMENT_CURSE) )
			{
				iItemBagNo = pItemDef->GetAbil(AT_EXTRACT_ELEMENT_CURSE);
			}
			else if( 0 != pItemDef->GetAbil(AT_EXTRACT_ELEMENT_DESTROY) )
			{
				iItemBagNo = pItemDef->GetAbil(AT_EXTRACT_ELEMENT_DESTROY);
			}
			else if( 0 != pItemDef->GetAbil(AT_EXTRACT_ELEMENT_RANDOM) )
			{
				iItemBagNo = pItemDef->GetAbil(AT_EXTRACT_ELEMENT_RANDOM);
			}
			else
			{
				break;
			...
			PgItemBag kItemBag;
			GET_DEF(CItemBagMgr, kItemBagMgr);
			//kItemBagMgr.GetItemBagByGrp(iExtract_Fire, iLevel, kItemBag);
			...
			{
			case 99800110://저급 화염속성 원소
				{
					iAchievementType = AT_ACHIEVEMENT_ATTR_LOW_FIRE;
				}break;
			case 99800120://고급 화염속성 원소
				{
					iAchievementType = AT_ACHIEVEMENT_ATTR_HIGH_FIRE;
				}break;
			case 99800210://저급 냉기속성 원소
				{
					iAchievementType = AT_ACHIEVEMENT_ATTR_LOW_ICE;
				}break;
			case 99800220://고급 냉기속성 원소
				{
					iAchievementType = AT_ACHIEVEMENT_ATTR_HIGH_ICE;
				}break;
			case 99800310://저급 자연속성 원소
				{
					iAchievementType = AT_ACHIEVEMENT_ATTR_LOW_NATURE;
				}break;
			case 99800320://고급 자연속성 원소
				{
					iAchievementType = AT_ACHIEVEMENT_ATTR_HIGH_NATURE;
				}break;
			case 99800410://저급 저주속성 원소
				{
					iAchievementType = AT_ACHIEVEMENT_ATTR_LOW_CURSE;
				}break;
			case 99800420://고급 저주속성 원소
				{
					iAchievementType = AT_ACHIEVEMENT_ATTR_HIGH_CURSE;
				}break;
			case 99800510://저급 파괴속성 원소
				{
					iAchievementType = AT_ACHIEVEMENT_ATTR_LOW_DESTROY;
				}break;
			}			
			...
			PgAction_ReqModifyItem kItemModifyAction_Del(CIE_ExtractElement, m_kGndKey, kOrder_Del);
			...
			PgAction_ReqModifyItem kItemModifyAction_Add(CIE_ExtractElement, m_kGndKey, kOrder_Add, kAddPacket);
...
*/
	...
	SPMOD_Modify_Count kDelData(kItem, kSrcItemPos, -1);//1씩 감소.
	SPMO MODS_Del(IMET_MODIFY_COUNT|IMC_DEC_DUR_BY_USE, pUser->GetID(), kDelData);
...
{ //0~9사이의 점수 값을 Rate값으로 변환하여 해당 아이템백 원소를 선택
	GET_DEF(CItemBagMgr, kItemBagMgr);
	PgItemBag kItemBag;
	...
	int iRate = ++byScore;
	iRate = iRate * iMaxRate / 10; //byScore의 범위는 반드시 0~9 이내여야 한다.
	int iAccumRate = 0;
