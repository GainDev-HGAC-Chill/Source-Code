...
void PgMissionGround::OnTick1s()
{
	...
	case INDUN_STATE_PLAY:
		{
			...
			{
				if( m_iEleite == 0 )	// 정예가 안죽었으면... 실패 처리 가능
				{
				...
				else if( 0 != m_dwNextWave_StartDelay && 0 != m_dwNextWave_Delay ) 
				{
					if( (0 != m_iNowWave) && ((0 == iMonsterCount) || ( true == BM::TimeCheck(m_dwNextWave_StartDelay, m_dwNextWave_Delay) )) )
					{
						...
						if( (false == bLastWave) || (0 == iMonsterCount) )
						{
							...
							{
								// 마지막 웨이브인데 몬스터는 다 죽지 않고 시간이 되면 패스
							}
							else
							{
								// Wave 연출로 인해서 처음엔 안한다.
								m_dwNextWave_StartDelay = 0;
								m_dwNextWave_Delay = 0;
	...
	case INDUN_STATE_CLOSE:
		{// Owner가 없으면 여기서 쫒아낸다(위험하니까 한명씩 쫒아내자)
...
//			return; 해야하나 말아야 하나
	...
	while ((pkPlayer = dynamic_cast<PgPlayer*> (PgObjectMgr::GetNextUnit(UT_PLAYER, kItor))) != NULL)
	{
		...
		pkPlayer->AutoHeal(dwElapsed, fAutoHealMultiplier);//HP/MP가 변경되면
		UpdatePartyUnitAbil(pkPlayer, PMCAT_HP|PMCAT_MP);//1초 마다 파티 HP/MP 갱신
...
bool PgMissionGround::LogOut(BM::GUID const &rkCharGuid)
{
	...
	CUnit* pkUnit = PgObjectMgr::GetUnit(rkCharGuid);//이 그라운드에 있는가.
	if(pkUnit)
	{
		if(m_eState == INDUN_STATE_PLAY)
		{
	...
	case UT_PLAYER:
		{
			...
			pkUser->DefenceMssionExpBonusRate(0.0f); // 혹시모르니 초기화.
...
// 					}
			...
			if ( true == bLog )
			{
				...
				if ( user_itr != m_kConUser.end() )
				{
					...
					kLog.Set( 0, static_cast<int>(2) );// 비정상 종료
	...
	case UT_BOSSMONSTER:
		{
			...
			if( pkMonster )
			{
				...
				std::pair<CONT_MON_NUM::iterator, bool> bRet = m_kContMonsterNum.insert(std::make_pair(iMonNo, 1));
				if( !bRet.second )
				{
					++(*bRet.first).second;
				}
	...
	case UT_OBJECT:
		{
			...
			std::pair<CONT_OBJECT_NUM::iterator, bool> bRet = m_kContObjectNum.insert(std::make_pair(iObjectNo, 1));
			if( !bRet.second )
			{
				++(*bRet.first).second;
			}
	...
	case MO_CHAPTER:
		{
			...
			case MC_MON_ALL_KILL:
				{
					Broadcast(BM::Stream(PT_M_C_NFY_REJECT_STAGE_MAP_MOVE,2));//몬스터 다 잡아야 맵을 이동 할 수 있다!!!
					return false;
				}break;
			case MC_MON_TARGET_KILL:
				{
...
void PgMissionGround::SendMapLoadComplete( PgPlayer *pkUser )
{
	...
	if( bAddMissionUser || (NULL == PgMission::GetMissionUser(pkUser->GetID())) )
	{
		...
		kInfo.iAccExp = -pkUser->GetAbil64(AT_EXPERIENCE);	// 처음에 경험치의 minus를 저장해둔다
...
bool PgMissionGround::SwapStage( BM::Stream * const pkPacket )
{
	...
	{// 3. 소환체 컨테이너를 만듬
		...
		while ( (pkUnit = PgObjectMgr2::GetNextUnit(UT_SUMMONED, kItor)) != NULL )
		{
			...
			{
				std::pair<CONT_UNIT_SUMMONED_MAPMOVE_DATA::iterator, bool> kPair = kContUnitSummonedData.insert(std::make_pair(pkSummoned->Caller(), SUserSummonedMapMoveData()));
	...
	for ( ; unit_itr!= kTempUnitArray.end(); ++unit_itr )
	{
		...
		if ( pkUser )
		{
			...
			PgAction_QuestMissionPercent kQuestAction(GroundKey(), GetMissionKey().iKey);//미션 달성률퀘스트
...
void PgMissionGround::ReStartMission( PgMission* pkMission, ConStageResource& rkContResource )
{
	...
	if( true == SwapStage_Before( 0, NULL ) )
	{
		...
		//if( true == bDefenceMode ) //무엇 때문에 디펜스모드만이라고 조건을 달아놓았는지 모르겠지만, 
		{							 //어쨌건 현재는 카오스도 스크롤 아이템 수량이 감소되어야 하므로 조건 풀어놓았고 아직은 작동되는듯 하다.
...
void PgMissionGround::EndDefenceMission()
{
	...
	kPacket.Push(static_cast<__int64>(m_dwStageExp));			// 추가 경험치 주기	
	...
	if( true == bResultItem )
	{
		...
		{			
			...
			if ( pkUser )
			{
				...
				CONT_MISSION_DEFENCE_STAGE_BAG::mapped_type kStageData;
				if( true == GetDefenceStage(m_iNowStage+1, kStageData) )	// 스테이지는 0부터 시작 하므로...
				{
					CONT_MISSION_DEFENCE_STAGE::value_type &kValue = kStageData.kCont.at(0);
				...
				{
					// 보상 아이템 3~4개, 머니팩 1개 최대 4개 이상 줄 수가 없다.
...
void PgMissionGround::EndMission(BM::Stream * const pkPacket)
{
	...
	for( user_itr=m_kConUser.begin(); user_itr != m_kConUser.end(); ++user_itr )
	{
		...
		if ( pkUser )
		{
			...
			{// 살신성인 업적: 자신보다 5~10레벨 낮은 유저와 미션 완료 했을 경우( 상대방이 40레벨 이하라면 3점, 그 외는 1점 )
				const int iLevel = pkUser->GetAbil(AT_LEVEL);
				const int iConditionMinLevel = iLevel - 10; // 업적 달성할 수 있는 상대방의 최소 레벨
				const int iConditionMaxLevel = iLevel - 5;	// 업적 달성 할 수 있는 상대방의 최대 레벨
				const int iAdvantageLevel = 40;				// 40레벨 이하는 추가 점수
				const int iAdvantageScore = 3;				// 추가 점수는 3점
				const int iMaxScore = 5;					// 최대 5점을 넘기지 못한다.
			...
			pkUser->SetAbil(AT_ACHIEVEMENT_MISSION_HIT,0);// 현재 미션에서 맞은카운트 삭제
			...
			&&	rkMissionUser.kRank > 0)
			{
				...
				if(iRetCount || (rkMissionUser.kRank >= MRANK_F) )//개수 있을때.
				{
					...
					if( 0 != m_kMissionResult.iRouletteBagGrpNo ) // 보여주기 용으로 S Rank 아이템 백을 얻는다.
					{
	...
	case INDUN_STATE_READY:
		{
			...
			// 유저의 숫자에 따라서 몬스터 난이도를 변경
			if ( 0 == m_iNowStage )
			{
				// 중안에 나간 유저를 반영할려면 위에서 '0 == m_iNowStage'를 없애면 된다!!
			...
			if( bRet )
			{
				...
				{	// 전략 디펜스라면 피로도에 따른 추가 경험치 비율을 계산해야 됨.
			...
			if( true == IsDefenceGameMode() )
			{
				...
				for( user_itor = m_kConUser.begin() ; user_itor != m_kConUser.end() ; ++user_itor )
				{
					...
					if( pPlayer )
					{
						// defence 모드일 경우 이 어빌을 세팅해주어야 옵션 증폭값을 제외하는 루틴을 타지 않음.
	...
	case INDUN_STATE_PLAY:
		{
			if( 0 == m_iNowStage )
			{
				//CUnit *pkUnit = NULL;// 첫번째 맵에서만 미션 퀘스트 시작 가능
				...
				//{
				//	PgAction_RemoveMissionQuest kRemoveMissionQuest;//실패, 이전 미션 퀘스트들을 제거
...
//	//////////////////////////////////////////////////
//
//	BM::Stream kNfyPacket(PT_M_C_ANS_MISSION_QUEST);//미션 퀘스트 소지 아이템 공지
...
//						PgCheckQuestBegin kCheckQuest(rkElement.iQuestID);//시작 가능한지 체크
//						if( kCheckQuest.DoAction(pkUnit, NULL) )//시작 가능하면
//						{
...
//					const ContMissionQuestVec::value_type &rkCurMissionQuest = kNewVec[iCur];//랜덤!
//
//					BM::Stream kAddonPacket(PT_M_I_REQ_START_MISSION_QUEST);
//					rkCurMissionQuest.WriteToPacket(kAddonPacket);//이 퀘스트를 시작 시켜라
//
//					CONT_PLAYER_MODIFY_ORDER kOrderList;
//					tagPlayerModifyOrderData_Add_Any kDelData(iItemNo, -1);//선택한 아이템 소모
...
//			}
//		}break;
//	case MQC_Nfy_CardItem://이런것들 오면 안되
...
void PgMissionGround::EndMission_Before()
{
	...
	{
		// 이벤트 맵에서의 히든 레벨 클리어 할 경우 "SSS"등급 정보 초기화
		iValue = 0;
	...
	if( true == bEventMap )
	{
		// 통합 랭킹으로 다 같이 파티원들이 "SSS" 등급 완료 판정을 받아도 된다.		
	...
	PgAction_ReqModifyItem kItemModifyAction(CIE_MissionUpdate, GroundKey(), kOrder, BM::Stream(), true);//미션 끝날때 죽어 있더라도. 갱신 되도록.
	kItemModifyAction.DoAction(NULL, NULL);
}
...
bool PgMissionGround::SwapStage_Before( size_t const iStage, CUnit *pkUnit )
{
	...
	if ( pkUnit )
	{
		...
		///////////////////////////////////////////////
		// 위에 CheckMonsterInMap() 주석 처리 MissionOptionChapterCheck() 에서 중복 처리한다.
	...
	while (itor_user != m_kConUser.end())
	{
		...
		CUnit* pkAbilUnit = PgObjectMgr::GetUnit(kCharGuid);//이 그라운드에 있는가.
		if(pkAbilUnit)
		{
			{// 각성 상태와
				CONT_PLAYER_MODIFY_ORDER kEventOrder;
	...
	CONT_PLAYER_MODIFY_ORDER kOrder;
	MissionReport_Update kMissionUpdate(iStage != 0, kGuidVec, GetMissionKey(), GetStageBitFlag(m_iNowStage));	// iStage==0일때는 Update하면 안된다.
	SPMO kIMO(IMET_SWAP_MISSION, BM::GUID::NullData(), kMissionUpdate);
	kOrder.push_back(kIMO);
	BM::Stream kAddon;
	kAddon.Push(iStage);
	PgAction_ReqModifyItem kItemModifyAction(CIE_MissionUpdate, GroundKey(), kOrder, kAddon, true);//미션 스테이지 넘어갈때 사망에 관련 없이 업데이트.
	kItemModifyAction.DoAction(NULL, NULL);
	return true;
}
	...
	while ((pkPlayer = dynamic_cast<PgPlayer*> (PgObjectMgr::GetNextUnit(UT_PLAYER, kItor))) != NULL)
	{
		...
		if( user_itr != m_kConUser.end() )
		{
			...
			if( pkUser )
			{
				...
				// Ability 점수가 100% 넘길시 총점게이지 갱신하기
				...
				if( rkMissionUser.kPoint[MPOINT_DEMAGE] < iNowDemage )
				{
					...
					if( 0 < iClass )
					{
						...
						if(iClassLimit & UCLIMIT_MARKET_FIGHTER)
						{
							if( true == ClassDemage() )
							{
								// 점수 반영은 2대 맞을 경우만 반영을 한다.(기사 클래스만...)
								ClassDemage(false);
							}
							else
							{
...
//		int const iClass = CalcClass(pkUnit->GetAbil(AT_CLASS));
		...
		VEC_INT Vec_Class;
		Vec_Class.push_back(pkUnit->GetAbil(AT_BASE_CLASS));		// 기본 클래스를 먼저 넣음.

		GetLowClassType(iClass, Vec_Class);						// 모든 클래스 번호를 구함.

		PgBase_Item kItem;
		bool bRet = GetGadaCoinItemResult(iLevel, Vec_Class, 1, kItem); // 아이템은 1개만 만든다.
		if( false == bRet )
		{
...
bool PgMissionGround::GetGadaCoinItemResult(int const iLevel, VEC_INT & Vec_Class, int const iCount, PgBase_Item & kItem)
{
	...
	{
		VEC_INT::iterator class_iter = Vec_Class.begin();		// 현재 클래스의 하위 클래스 해당하는 아이템 백 그룹을 구한다.
...
void PgMissionGround::AddMonsterEnchant()
{
	...
	CONT_MISSION_DEFENCE_STAGE_BAG::mapped_type kStageData;
	if( false == GetDefenceStage(m_iNowStage+1, kStageData) )	// 스테이지는 0부터 시작 하므로...
	{
		return;
	}
	...
	int const iMonsterNo = kElement.iMonsterNo;					//kValue.iAddMonster_GroupNo -> 테이블 참조해서 몬스터 번호 빼내기
	...
	if( true == GetDefenceWave(kValue.iWave_GroupNo, iNowWave, kWaveData) )	// Wave도 시작이 0부터 시작하므로...
	{
			...
			else
			{
				...
				if(pkMonster)
				{
					if(kElement.iItemNo && (BM::Rand_Range(100, 1) <= kElement.iDropRate))
					{
						...
						if(false==PgGroundUtil::InsertAlwaysDropItem(pkMonster, kElement.iItemNo, iDropCount)) //테이블에 설정된 기본 드롭 갯수*백발백붕 아이템 사용 갯수
						{
	...
	if( false == bSetAbilMonster )
	{
		if( iObjectAttackMonCount < iObjectAttackMonMaxCount )
		{
			...
			if( IsDefenceMode() )
			{
				...
				{
					pkMonster->SetAbil(AT_AI_TARGETTING_RATE, ms_iDefenceChangeTargetRate);	// XXX %확률로 타겟 변경
				}
			}
			else if( IsDefenceMode7() || IsDefenceMode8() )
			{
				...
				{
					pkMonster->SetAbil(AT_AI_TARGETTING_RATE, ms_iDefence7ChangeTargetRate);	// XXX %확률로 타겟 변경
				}
			}
...
void PgMissionGround::SetDefenceGroundInfo()
{
	...
	if( IsDefenceMode() || IsDefenceMode7() )
	{
		if( 0==m_iNowStage && 1==m_iNowWave )
		{
			...
			if( IsDefenceMode() )	// 7,8모드는 수호석 어빌로 셋팅(DB)
			{
				// 타워도 팀 셋팅 1로 해야 된다.
				CUnit* pkUnit = NULL;
				PgObjectMgr::GetFirstUnit(UT_OBJECT, kItor);
...
bool PgMissionGround::IsDefenceLastWave()
{
	...
	case MO_DEFENCE:
		{
			CONT_MISSION_DEFENCE_STAGE_BAG::mapped_type kStageData;
			if( true == GetDefenceStage(iNowStage, kStageData) )	// 스테이지는 0부터 시작 하므로...
			{
				CONT_MISSION_DEFENCE_STAGE::value_type &kValue = kStageData.kCont.at(0);					

				CONT_MISSION_DEFENCE_WAVE_BAG::mapped_type kWaveData;
				if( true == GetDefenceWave(kValue.iWave_GroupNo, iNextWave, kWaveData) )	// Wave도 시작이 0부터 시작하므로...
				{
					// Wave 있다.
					return false;
				}
	...
	case MO_DEFENCE8:
		{
			...
			if( true == GetDefence7Stage(iNowStage, kStageData) )	// 스테이지는 0부터 시작 하므로...
			{
				...
				if( true == GetDefence7Wave(iNowStage, iNextWave, kKeyData, kWaveData) )	// Wave도 시작이 0부터 시작하므로...
				{
					// Wave 있다.
					return false;
				}
...
void PgMissionGround::DefenceNextStage(int const iNowStage, int const iNowWave)
{
	...
	eDefenceMsg = DefenceNextWaveMonsterGen(iRegenStage, iNowWave+1);	// 0부터 시작하므로...
    m_dwCheckReReqWavePacket = 0;
...
/*EDefenceMsg PgMissionGround::DefenceNextWaveMonsterGen(int const iNowStage, int const iNowWave)
{
	...
	if( true == GetDefenceStage(iNowStage, kStageData) )	// 스테이지는 0부터 시작 하므로...
	{
		...
		if( true == GetDefenceWave(kValue.iWave_GroupNo, iNowWave, kWaveData) )	// Wave도 시작이 0부터 시작하므로...
		{
			...
			m_dwNextWave_Delay = static_cast<DWORD>(kWaveValue.iWave_Delay);	// 다음 진행될 Wave Time		

			DefenceWaveMonsterGenerate(kWaveData);

			return D_OK;
		}
		// 해당 Stage의 Wave가 더이상 없다는 의미
		return D_WAVE_END;
	}
	// 해당 Stage가 더이상 없다는 의미
	return D_STAGE_END;
}*/
	...
	case MO_DEFENCE8:
		{
			...
			if( true == GetDefence7Stage(iNowStage, kStageData) )	// 스테이지는 0부터 시작 하므로...
			{
				...
				CONT_MISSION_DEFENCE7_WAVE_BAG::key_type	kKeyData;
				CONT_MISSION_DEFENCE7_WAVE_BAG::mapped_type kWaveData;
				if( true == GetDefence7Wave(iNowStage, iNowWave, kKeyData, kWaveData) )	// Wave도 시작이 0부터 시작하므로...
				{
...
void PgMissionGround::NextStageSend()
{
	...
	if( ( 0 == m_iCountSelectItem && GetSelectedDirection() != kMonsterGen )
		|| 0 == m_iNowStage )
	{//아이템 안썼고, 방향도 못맞췄음 / 첫 스테이지엔 안나옴.
		m_bSelectSuccess = false;

		// 실패시 성공룃수 초기화
		m_iSelect_SuccessCount = 0;
		// 실패시 실패된 방향은 몬스터 나와야 된다
		eMonsterGen = kMonsterGen;
	}
	else
	{//맞추기 성공해서 카운트 증가
	...
	{		
		...
		{	// 전략디펜스 모드에서 스테이지 시작할 때 전략 피로도를 감소.
		...
		if( pItemDef )
		{
			...
			if( iNeedItemCount <= iHaveItemCount )
			{
				if( m_kConUser.size() > m_iCountSelectItem )
				{
					...
					kLog.Set( 1, static_cast<int>(iNeedItemCount) );	// iValue2 사용할 갯수
					kLog.Set( 2, static_cast<int>(GetMissionNo()) );	// iValue3 미션번호
					kLog.Set( 3, static_cast<int>(GetLevel()) );	// iValue4 미션 난이도
					kLog.Set( 4, static_cast<int>(m_iNowStage) );	// iValue5 스테이지
...
void PgMissionGround::SetDirection(CUnit *pkOwner, BM::Stream* pkNfy)
{
	...
	if( true == bRet && NULL != pkMissionInfo)
	{
		if(0 != pkMissionInfo->m_kBasic.iEventMission)
		{ //이벤트 미션일 경우
			bStart = (m_iCountSelectItem >= m_kConUser.size()); //백발백중 아이템을 전부 쓰지 않았다면 바로 시작하지 않고 대기
		}
	}
	...
	//if( (0 == m_iNowStage) && (0 == m_iNowWave) )
	{
		...
		while((pkUnit = PgObjectMgr::GetNextUnit(UT_OBJECT, kItor)) != NULL)
		{
			...
			if( 0 < iTeam )
			{
				...
				std::pair<CONT_OBJECT_TEAM::iterator, bool> kPair = m_kObjectTeam.insert(std::make_pair(iTeam, kObjectGuid));
				if( kPair.second )
				{
					int iObjectHp = pkUnit->GetAbil(AT_HP);
					std::pair<CONT_OBJECT_HP::iterator, bool> kPair2 = m_kObjectHp.insert(std::make_pair(iTeam, iObjectHp));
...
int PgMissionGround::GetStageTotalMonsterCount()const
{
	...
	int iNextWave = (m_iNowStage == 0) && (m_iNowWave == 0) ? 2 : m_iNowWave + 1; // 진행될 남은 웨이브. 디펜스의 첫스테이지 첫웨이브는 2번째 웨이브부터 계산해야 된다.
		...
		{			
			...
			if ( pkUser )
			{
				...
				if(GetConstellationKey().PartyGuid.IsNull())
				{
					...
					CONT_MISSION_DEFENCE7_STAGE_BAG::mapped_type kStageData;
					if( true == GetDefence7Stage(m_iNowStage+1, kStageData) )	// 스테이지는 0부터 시작 하므로...
					{
						CONT_MISSION_DEFENCE7_STAGE::value_type const& kValue = kStageData.kCont.at(0);	
	...
	if( pkUnit )
	{
		...
		if( IsDefenceItemList(iItemNo) )
		{
			...
			if( rkGuid.IsNotNull() )
			{
				...
				{
					std::pair<CONT_DEFENCE_ITEM::iterator, bool> kPair = m_kContDefenceItem.insert(std::make_pair(rkGuid, CONT_DEFENCE_ITEM::mapped_type()));
					iter = kPair.first;
				}
				else
				{
			...
			case DI_D7_POINT_COPY:
				{
					...
					if( PointCopying() )
					{
						pkUnit->SendWarnMessage(799398); // 이미 포인트 복제기가 사용 중입니다.
						return;
					}

					if( m_iNowStage < 10 )
					{	// 11스테이지부터 사용가능..
						pkUnit->SendWarnMessage(799396); // 11Stage부터 사용 가능합니다.
						return;
					}
		...
		if( pkGuardian->end() != iter )
		{
			...
			if( m_kContStrategicPoint.end() != iter )
			{
				if( 0 < kValue.iNeed_StrategicPoint )
				{
					...
					if( pPlayer )
					{
						int SingleDiscount = pPlayer->GetGuardianDiscountRate(kValue.GuardianType);	// 설치하려는 타입의 가디언 할인률.
						int AllDiscount = pPlayer->GetGuardianDiscountRate(GT_ALLTYPE);		// 모든 타입 할인률.

						int rate = std::max(SingleDiscount, AllDiscount);	// 둘중에 큰 것만 적용.

						int DiscountCost = (Need_StrategicPoint * rate) / 10000;	// 할인율이 만분률.
...
bool PgMissionGround::SetGuardian(std::wstring kName, int const iGuardianNo, BM::GUID const kGuid)
{
	...
	if( (0 < iGuardianNo) && (kGuid.IsNotNull()) )
	{
		...
		if( m_kGuardianInstall.end() == iter )
		{		
			std::pair<CONT_GUARDIAN_INSTALL::iterator, bool> kPair = m_kGuardianInstall.insert(std::make_pair(kName, CONT_GUARDIAN_INSTALL::mapped_type()));
			iter = kPair.first;
		}
		(*iter).second = kInfo;
...
bool PgMissionGround::UpGradeGuardian(CUnit* pkUnit, BM::Stream* pkNfy)
{
	...
	{
		// 가디언 번호 잘못된 데이터 또는 이미 가디언이 설치되어 있지 않는 경우
		pkUnit->SendWarnMessage(401135);
	}
	return false;
}
...
bool PgMissionGround::InsertGuardian(CUnit* pkUnit, BM::Stream* pkNfy)
{
	...
	if( (0 < iGuardianNo) && (false == IsGuardianPos(kTriggerName)) )
	{
		...
		if( true == IsNeedGuardianStrategicPoint(pkUnit->GetAbil(AT_TEAM), iGuardianNo) )
		{
			...
			if( pkGuardian->end() != iter )
			{
				...
				if( pkGround )
				{
					// 다른 가디언이 있으면 이펙트 적용(각자 가디언마다 자신의 이펙트만 적용)
					/*VEC_INT kAddEffect;
					BM::GUID rkGuid;
	...
	{
		// 가디언 데이터 번호가 잘못되었거나, 이미 설치가 되어 있는 경우
...
{
	std::pair<CONT_STRATEGIC_POINT::iterator, bool> bRetValue = m_kContStrategicPoint.insert(std::make_pair(iTeam, iStrategicPoint));
	if( !bRetValue.second )
	{
...
void PgMissionGround::Defence7SkillLearn(CUnit* pkUnit, BM::Stream * const pkPacket)
{
	...
	//IGuardianNo를 클라에서 넘어온 정보를 믿을수 없음
	...
	case DS_ADD_POINT:
		{
			if( true == IsNeedGuardianSkillPoint(iTeam, iGuardianNo, iEffectNo) )
			{
				...
				int const iAddStrategicPointPercent = std::max(pkEffectDef->GetAbil(AT_STRATEGIC_POINT), 0);	// 획득량 %					
		    ...
		    while( m_kGuardianInstall.end() != iter )
		    {
			    ...
			    if( pkEntity != NULL && pkEntity->IsUnitType(UT_ENTITY) )
			    {
				    if( pkUnit->GetID() == pkEntity->Caller() )
				    {
						...
						//Logout에서 호출하는 ReleaseUnit에서 소환체를 지우기때문에 제거
						//pkTargetUnit에 AddSummonUnit을 해줘야 할 이유를 찾지 못해 처리하지 않음
...
void PgMissionGround::CheckMonsterDieQuest(CUnit* pkCaster, CUnit* pkMonster)
{
	...
	&&	pkMonster )
	{
		...
		if( pkPC )
		{
			if( BM::GUID::NullData() != pkPC->PartyGuid() )//파티가 있으면
			{
				VEC_GUID kGuidVec;
				...
				if( bFindMember )
				{
					...
					while( kGuidVec.end() != guid_iter )
					{
						...
						if( pkPartyMember )
						{
							PgAction_MonsterQuestCheck kMonsterQuestAction(GetGroundNo(),GroundKey(), iCurCombo, GetKey(), GetLevel(), GetStageCountExceptBonus());
...
void PgMissionGround::Defence7RelayStage(CUnit * pkUnit, bool const bRemoveGuardian)
{
	...
	if( IsDefenceMode7() )
	{
		int const RELAY_DELAY_TIME = 60000;//60초후에 시작
		if(pkUnit)
		{
...
void PgMissionGround::Defence7PointCopy(PgPlayer * pPlayer)
{
	if( m_iNowStage < 10 )
	{
		return; // 11스테이지 전에는 사용할수 없다(Stage는 0부터 시작).
	}

	if( !PointCopying() )		// 아이템 복제기를 처음 사용.
	{
		PointCopying(true);	// 아이템 복제기 사용상태로 바꿈.

		m_kPointAccumCount = 0;	// 처음이니까 누적횟수 1번.
		AccumPoint(CaclAccumPoint());	// 현재 누적 포인트 계산(Stage N = Power(2,N-1)).
		++m_kPointAccumCount; // 누적회수 증가.
		int const NextAccumPoint = CaclAccumPoint(); // 다음 스테이지의 누적 포인트.

		++m_kAccumMonsterKillableCount;	// 제거 가능한 몬스터 수 증가.

		BM::Stream Packet(PT_M_C_NFY_USE_POINT_COPY_ITEM);
		Packet.Push(pPlayer->Name());	// 사용한 캐릭터 이름.
		Packet.Push(PointAccumCount());	// 누적 횟수.
		Packet.Push(AccumPoint()); // 누적 점수.
...
void PgMissionGround::AccumPointToStrategyPoint()
{
	if( PointCopying() )
	{
		...
		if( pPlayer )
		{
			...
			SetStrategicPoint(iTeam, iAccumPoint);	// 누적 포인트를 전략 포인트로 전환.

			BM::Stream Packet(PT_M_C_NFY_ACCUMPOINT_TO_STRATEGICPOINT);
			Packet.Push(AccumPoint());
			Broadcast(Packet);

			AccumPoint(0);	// 누적 포인트 초기화.
			PointCopying(false);	// 복제기 아이템 사용 끝남.
			AccumMonsterKillableCount(0);	// 제거 가능한 몬스터 수 초기화.
...
void PgMissionGround::KillMonsterByAccumPoint()
{
	if( PointCopying() )
	{
		...
		if( pPlayer )
		{
			CONT_OBJECT_MGR_UNIT::iterator Itor; // 현재 맵에 있는 모든 몬스터들의 목표 구간 조사.
			...
			{
				int GoalSection = pMonster->GetAbil(AT_ROADPOINT_INDEX);	// 몬스터의 목표 구간을 얻어 와서.
			...
			int MonsterCount = 0;
			KillableMonsterList::iterator iter = ContKillableMonster.begin();	// 구간별 몬스터 리스트.
			for( ; iter != ContKillableMonster.end() ; ++iter )
			{
				KillableMonsterList::mapped_type & Element = (*iter).second;
				if( MonsterCount < Element.size() )	// 현재 구간에 있는 몬스터가 제거해야할 몬스터 수보다 많다면.
				{	// HP 순으로 정렬해서 HP가 많은 순으로 제거.
				...
				for( ; monster_iter != Element.end() ; ++monster_iter )
				{
					...
					{	// 제거할 몬스터의 숫자만큼 제거했다면 종료.
						break;
					}

					CUnit * pMonsterUnit = dynamic_cast<CUnit*>( (*monster_iter) );
					::OnDamaged(pPlayerUnit, pMonsterUnit, 0, pMonsterUnit->GetAbil(AT_HP), this, g_kEventView.GetServerElapsedTime());	// 몬스터 제거.
					++MonsterCount;
				}
			}
...
void PgMissionGround::SendMissionState(CUnit * pkUnit)
{
	...
	kCPacket.Push(GetLevel()+1);//Client는 1부터 레벨이...
...
// 전략 디펜스 모드에서 스테이지를 시작할 때, 참여중인 플레어어들의 전략피로도를 감소시킨다.
void PgMissionGround::DecreaseStrategyFatigueDegree()
{
	...
	int DecPerStageFatigability = 0;		// 스테이지당 피로도 감소치.
	...
	while( (pUnit = PgObjectMgr::GetNextUnit(UT_PLAYER, unit_iter)) != NULL )
	{
		...
		if( pPlayer )
		{
			int const Abil = pPlayer->GetAbil(AT_RECOVERY_STRATEGY_FATIGABILITY);	// 피로도 회복 물약 캐시 아이템을 사용했다면 피로도 감소되지 않음.
			if( 0 == Abil )
			{
				// 현재의 피로도를 가져온다.
				...
				if( PrevFatigability > 0 )
				{
					...
					// 새로운 피로도로 Set.
					pPlayer->SetAbil(AT_STRATEGY_FATIGABILITY, static_cast<short>(NewFatigability), true);	// 피로도를 전송해서 UI 표시에 사용.
				}
			}
		}
	}
...
// 전략 디펜스 모드에서 피로도에 따른 추가 경험치 비율 계산.
void PgMissionGround::CalcBonusExpDefence7()
{
	...
	while( (pUnit = PgObjectMgr::GetNextUnit(UT_PLAYER, unit_iter)) != NULL )
	{
		...
		if( pPlayer )
		{
			...
			float BonusExpRate = SyncVariable.fBonusExpRate;		// 최소 추가 비율.
			float FatigabilityDivValue = static_cast<float>(SyncVariable.iFatigabilityDivValue);	// 피로도 계수.

			float FinalRate = (BonusExpRate + (Fatigability / FatigabilityDivValue));

			int TemResult = static_cast<int>((FinalRate * 10));	// 소수점 1자리 이하 버림.
