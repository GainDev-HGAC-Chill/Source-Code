...
bool PgLocalPartyMgrImpl::AddItemRule(BM::GUID const& rkPartyGuid, PgPartyItemRule*& pkOut)//파티 아이템 습득 룰을 만든다
{
	...
	std::pair<ContPartyItemRule::iterator, bool> kRet = m_kPartyItemRule.insert( std::make_pair(rkPartyGuid, pkNewRule) );
	if( !kRet.second )
	{
	...
	while(kVec.end() != guid_iter)
	{
		...
		if( bFindMember )
		{
			if ( g_kProcessCfg.ChannelNo() == pkMember->kChnGndKey.Channel() )
			{
				...
				std::pair<ContPartyMemberGndKey::iterator, bool> kRet = kSet.insert( kMemberGndKey );
				if( !kRet.second )
				{
					//
				}
...
bool PgLocalPartyMgrImpl::AddExpeditionItemRule(BM::GUID const & ExpeditionGuid, PgExpeditionItemRule *& pOut)	// 원정대 아아템 습득 룰을 만든다.
{
	...
	std::pair<ContExpeditionItemRule::iterator, bool> Ret = m_ExpeditionItemRule.insert( std::make_pair(ExpeditionGuid, pNewRule) );
	if( !Ret.second )
	{
	...
	while( guid_iter != Vec.end() )
	{
		...
		if( FindMember )
		{
			if( g_kProcessCfg.ChannelNo() == Member.kChnGndKey.Channel() )
			{
				...
				std::pair<ContExpeditionMemberGndKey::iterator, bool> Ret = Set.insert(MemberGndKey);
				if( !Ret.second )
				{
					//
				}
