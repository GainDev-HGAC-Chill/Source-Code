...
{// iSKillNo가 사용될때, 증가된 iLnkageLevel에 해당하는 iLinkageEffectNo
	...
	{// 이미 존재하면, 기존 정보에 추가해 넣어주고
		...
		//std::pair<CONT_LINKAGE_EFFECTNO::iterator, bool> kRet = rkElemCont.insert( std::make_pair(iLinkageLv, iLinkageEffectNo) );
		//if(!kRet.second)
		//{// 레벨이 겹치는 경우는 메세지
...
{
	std::pair<CONT_LINKAGE_SKILL_INFO::iterator, bool> kRet = m_kCont.insert( std::make_pair(iSkillNo , rkInfo) );
	if(!kRet.second)
	{// 레벨이 겹치는 경우는 메세지
...
{
	std::pair<CONT_LINKAGE_EFFECTNO::iterator, bool> kRet = m_kContEachLinkageLvEffectNo.insert( std::make_pair(iLinkageLv, iLinkageEffectNo) );
	if(!kRet.second)
	{// 레벨이 겹치는 경우는 메세지
