...
const BM::GUID CItemTradeMgr::NewItemTrade(BM::GUID const &kFrom, BM::GUID const &kTo)
{
	...
	std::pair<TradeItemContConst::iterator, bool> target_ib = m_kTradeItemContByChar.insert( std::make_pair(pkNewInfo->kTargetGuid, pkNewInfo) );
	std::pair<TradeItemContConst::iterator, bool> caster_ib = m_kTradeItemContByChar.insert( std::make_pair(pkNewInfo->kCasterGuid, pkNewInfo) );
	...
	std::pair<TradeItemCont::iterator, bool> eRet = m_kTradeItemCont.insert( std::make_pair(pkNewInfo->kTradeGuid, pkNewInfo));
	if (!eRet.second)
	{
