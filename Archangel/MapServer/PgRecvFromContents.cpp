	...
	case PT_N_M_ANS_TAKE_COUPON:
		{
			...
			{// 돈과 캐시 지급은 다른 오더를 사용 하도록 한다.
				...
				if(f_Cash > 0)
				{
					kItemModifyAction.CashAdd(SPMO(IMET_ADD_CASH, pkPlayer->GetID(), SPMOD_AddCash(pkPlayer->GetMemberGUID(), static_cast<__int64>(f_Cash), 0ll, pkPlayer->Name(), CIE_Coupon)));
				}
