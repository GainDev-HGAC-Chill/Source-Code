...
{
	BM::CAutoMutex kWLock(m_kMutex, true);
	std::pair<CONT_CONSTELLATION_PARTY::iterator, bool> ret = m_PartyList.insert( std::make_pair(constellationMission.ConstellationKey.PartyGuid, std::make_pair(1,constellationMission)) );
	if( false == ret.second )
	{
