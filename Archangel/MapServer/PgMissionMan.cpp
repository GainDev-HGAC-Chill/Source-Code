...
HRESULT PgMissionManager::CreateInfo()
{
	...
	while(mission_itor != pContDefMission->end())
	{
		...
		std::pair<MissionKeyContainer::iterator, bool> ret = m_kMissionKeyMap.insert(std::make_pair(pMission->iKey, iMission));
		if( !ret.second )
		{
...
bool PgMissionManager::Add(TCHAR const* pkXmlPath)
{
	...
	std::pair<MissionContainer::iterator, bool> ret = m_kMissionMap.insert(std::make_pair(pkMission->ID(), pkMission));
	if( !ret.second )
	{
