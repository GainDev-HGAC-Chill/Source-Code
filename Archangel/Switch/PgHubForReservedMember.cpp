...
#endif
		std::pair<CONT_SWITCH_PLAYER::iterator, bool> insert_ret = m_kReserveMember.insert( std::make_pair(rkRSRM.guidOrderKey, pkElement) );
...
{//	쎈터에게 패킷 오면 호출 하도록.
	...
	if( E_TLR_SUCCESS == eRet )
	{
		...
		std::pair<CONT_SWITCH_PLAYER::iterator, bool> insert_ret = m_kContMemberSession.insert(std::make_pair( pkReserveData->guidMember, pkReserveData) );
		if( true == insert_ret.second )
