...
void PgLog::Set( size_t const iIndex, std::wstring const& rkString)
{
	...
	{
		std::pair<CONT_LOG_STRING::iterator, bool> kPair = m_kContString.insert( std::make_pair(iIndex, rkString) );
		if ( !kPair.second )
		{
...
{
	std::pair<CONT_LOG_CONT::iterator, bool> kInsert = m_kContLog.insert(std::make_pair(kLog.CharacterKey(),kLog));
	if(kInsert.second)
	{
