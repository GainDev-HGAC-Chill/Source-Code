...
namespace XUI 
{
	...
	protected:
		...
		{
			std::pair<ContHidePart::iterator, bool> result = m_kHidePart.insert(std::make_pair(HideKey, rkWndList));
	...
	typedef struct tagUIXMLData
	{
		...
		tagUIXMLData* RegistChild(std::wstring const& rkName, tagUIXMLData const& rkValue)
		{
			...
			{//없는것이다.
				std::pair<ChildHash::iterator, bool> ret = m_kChildHash.insert(std::make_pair(rkName, kTempData));
				if(ret.second)
				{
		...
		bool RegistAttr(std::wstring const& rkName, std::wstring const& rkValue)
		{
			...
			return true;
			//			std::pair<AttrList::iterator, bool> ret = m_kAttrList.insert(std::make_pair(rkName, rkValue));
			//			return ret.second;
		}

		bool RegistScript(std::wstring const& rkName, std::wstring const& rkValue)
		{
			std::pair<ScriptHash::iterator, bool> ret = m_kScriptHash.insert(std::make_pair(rkName, rkValue));
			return ret.second;
		}
		bool RegistGlobalScript(std::wstring const& rkName, std::wstring const& rkValue)
		{
			std::pair<ScriptHash::iterator, bool> ret = m_kGlobalScriptHash.insert(std::make_pair(rkName, rkValue));
			return ret.second;
		}
