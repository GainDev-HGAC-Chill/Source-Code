...
CXUI_Wnd* CXUI_Manager::Create(ID_TYPE const& ID, bool const bIsModal, ID_TYPE const& rkNewID)//여기서 ActivateList에 넣어서는 안됨.
{
	...
	if(pUIData)
	{
		if(!pWnd)
		{
			...
			if(!bIsModal && pWnd)
			{//모달리스는 버퍼로
				std::pair<XUI_HASH::iterator, bool> ret = m_kBufferdHash.insert(std::make_pair(wstrID, pWnd));
				assert(ret.second);
			}
		}
...
bool CXUI_Manager::RegistCreator(ID_TYPE const& ID, CXUI_Wnd &rkBase)
{
	...
	std::pair<XUI_HASH::iterator, bool> ret = m_mapCreator.insert(std::make_pair(wstrID, rkBase.VCreate()));
	...
	std::pair<KEY_WORD_HASH::iterator, bool> keyword_ret = m_mapKeyWord.insert( std::make_pair(wstrID,	rkBase.VType()) );
	...
	{//원상 복구 시켜주자
		if( NULL != (*iter) )
		{
			...
			{//열어 줘// 해쉬에 다시 넣는다.
				...
				if( !(*iter)->IsModal() )
				{
					std::pair<XUI_HASH::iterator, bool> ret = m_kBufferdHash.insert(std::make_pair((*iter)->ID(), (*iter)));
					assert(ret.second);
				}
			}
			else
...
{
	std::pair<COLOR_HASH::iterator, bool> ret = m_ColorMap.insert( std::make_pair(iIdx, dwColor) );
	if (!ret.second)
	{
