...
void CXUI_Config::VRegistAttr(std::wstring const& wstrName, std::wstring const& wstrValue)
{
	...
	REGIST_STATIC_FORM(1);//ATTR_STATIC_SET(1) °ú ÇÑ½Ö 
...
{
	std::pair<SOUND_INFO_HASH::iterator, bool> ret = m_mapSndInfo.insert( std::make_pair(iBtnType, SOUND_INFO()) );
...
{
	std::pair<STATIC_INFO_HASH::iterator, bool> ret = m_mapStaticInfo.insert( std::make_pair(iStaticType, STATIC_INFO()) );
