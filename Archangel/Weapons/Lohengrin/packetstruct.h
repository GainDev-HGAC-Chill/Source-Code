...
typedef enum eGroundAttribute
{
	...
	GATTR_FLAG_HIDE_PARTY_LIST	= GKIND_EXPEDITION | GKIND_ELEMENT | GKIND_EVENT, // 파티 찾기 리스트에서 감춤
...
typedef enum 
{
	...
	MAX_EQUIP_IDX = 36,
	MAX_EQUIP_IDX_EXTEND = 72,

	MAX_CONSUME_IDX = 36,
	MAX_CONSUME_IDX_EXTEND = 72,

	MAX_ETC_IDX = 36,
	MAX_ETC_IDX_EXTEND = 72,

	MAX_CASH_IDX = 36,
	MAX_CASH_IDX_EXTEND = 72,
...
typedef struct tagReqSoulStoneTrade
{
	...
	tagReqSoulStoneTrade(BM::GUID Serial, int const ItemNo, int const MaterialNo, int const MaterialNeedsCount, int TradeCount)
		:	gidSerial(Serial), iItemNo(ItemNo), iMaterialNo(MaterialNo), iMaterialNeedsCount(MaterialNeedsCount), iTradeCount(TradeCount)
	{
	}
	...
	bool ReadFromPacket(BM::Stream & rkPacket)
	{
		return (rkPacket.Pop(gidSerial)) && (rkPacket.Pop(iItemNo)) && (rkPacket.Pop(iMaterialNo)) && (rkPacket.Pop(iMaterialNeedsCount)) && (rkPacket.Pop(iTradeCount));
	}

	size_t min_size() const
	{
		return sizeof(gidSerial) + sizeof(iItemNo) + sizeof(iMaterialNo) + sizeof(iMaterialNeedsCount) + sizeof(iTradeCount);
	}
...
typedef enum eFriendMgr
{
	...
	FM_MAXFRIEND			= 999,			//최대 친구 등록 수는 40 (가장 기초)
...
int const MAX_SYSTEMINVEN_NUM = 12;// 시스템 인벤 하나에 최대 들어가는 아이템 종류 갯수
