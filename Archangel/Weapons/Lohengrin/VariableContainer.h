...
protected:
	...
	//bool GetActArg(int const iKind, SActArg*& pkOutActArg, bool const bCreate = false);	// public ±ÝÁö~
	template< typename T >
	HRESULT Set(int const iKind, int const iID, T const &rkValue, bool const bCreate = false)
	{
		...
		if (itor == m_kContVariable.end())
		{
			...
			std::pair<CONT_VARIABLE::iterator, bool> ibRet = m_kContVariable.insert(std::make_pair(iKind, SActArg()));
			itor = ibRet.first;
		}
