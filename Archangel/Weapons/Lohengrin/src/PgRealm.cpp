...
{//랠름은 같다고 보는거다.
	if ( !CProcessConfig::IsPublicChannel(rkServerList.nChannel) )
	{
		...
		{
			std::pair<CONT_CHANNEL::iterator, bool> ret = m_kContChannel.insert(std::make_pair(rkServerList.nChannel, PgChannel()));
			if(!ret.second)
			{
				assert(NULL);
...
void PgRealm::ReadFromPacket(BM::Stream &rkPacket, ERealm_SendType const eSendType)
{
...

	{
		...
		for(short usI=0; usI<nSize; usI++)
		{
			...
			{
				std::pair<CONT_CHANNEL::iterator, bool> ret = m_kContChannel.insert(std::make_pair(kChannel.ChannelNo(), kChannel));
				if(!ret.second)
				{
