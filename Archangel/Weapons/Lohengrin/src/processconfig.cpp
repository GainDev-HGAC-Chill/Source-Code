...
{
	BM::CAutoMutex kLock(m_kMutex, true);
	std::pair<CONT_SERVER_HASH::iterator, bool> kRet = m_kServerHash.insert(std::make_pair((tagServerIdentity)rkInfo, rkInfo));
	return kRet.second;
}
...
HRESULT CProcessConfig::GetServerInfo(short const nServerType, CONT_SERVER_HASH &rkCont)const
{
	...
	while(itor != m_kServerHash.end())
	{
		...
		{
			std::pair<CONT_SERVER_HASH::iterator, bool> kRet = rkCont.insert(std::make_pair((*itor).first, (*itor).second));
...
{
	BM::CAutoMutex kLock(m_kMutex, true);
	std::pair<CONT_MAP_CONFIG::iterator, bool> ret = m_kContMapCfg_Static.insert(rkKey);
	if(ret.second)
	{
		return S_OK;
	}
...
{
	BM::CAutoMutex kLock(m_kMutex, true);
	std::pair<CONT_MAP_CONFIG::iterator, bool> ret = m_kContMapCfg_Mission.insert(rkKey);
	if(ret.second)
	{
		return S_OK;
	}
