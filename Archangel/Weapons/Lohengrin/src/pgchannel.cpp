...
{
	std::pair<CONT_SERVER_HASH::iterator, bool> ret = m_kServers.insert(std::make_pair( *(SERVER_IDENTITY*)&slServer, slServer));
	if(ret.second)
	{
		return true;
	}
...
void PgChannel::ReadFromPacket(BM::Stream &rkPacket, ERealm_SendType const eSendType)
{
	...
	if((eSendType & ERealm_SendType_ServerInfo) != 0)
	{
		...
		for(size_t i = 0;  element_size > i; i++)
		{
			...
			std::pair<CONT_SERVER_HASH::iterator, bool> ret = m_kServers.insert(std::make_pair(kSI, kTbl));
			if(!ret.second)
			{
