...
//	DECLARE_TABLE(CONT_DEFGUILDHOUSE,		m_DefGuildHouse);
	...
	DECLARE_TABLE(CONT_DEF_SOULTRADE, m_kContDefItemSoulTrade);
	...
	// 이 주석 위까진 bin 파일로 빼고, 서버용 패킷도 만든다
	// ---------------------------------------------------------------
	// 이 주석 아래것들은 bin 파일로 빼지 않는다
	...
	// 이 위 것들은 bin 파일로 빼지 않는다
	// -------------------------------------------------------------------
	// 이 아래것들은 bin 파일로만 빼고, 패킷으로 만들지는 않는다(only bin)
...
private:
	bool m_bUTF8;
	mutable BM::ACE_RW_Thread_Mutex_Ext m_kMutex;	//중복호출이 가능해야만 한다.

	void ParseTEXTElement(TiXmlAttribute const* pkAttribute);
