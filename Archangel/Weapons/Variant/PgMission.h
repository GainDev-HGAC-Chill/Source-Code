...
typedef enum eMissionCountMax
{
	...
	MISSION_COMBO		= 5,
	MISSION_DOWN		= 7,
	MISSION_NORMALSCORE	= 100,	// 기본점수
	MISSION_COMBO_BONUS = 20, 
	MISSION_SCORE		= 4,
	MISSION_BONUSSCORE  = 30,
	MISSION_SCORE_PERSENT = 1000,
	MISSION_SCORE_CALC  = 10,
	MISSION_TOTAL_UP	= 5,	// 5% 총점에 적용되는 퍼센트(%)
	MISSION_OVERHIT		= 60,	// 60% 초과시 적용
	MISSION_OVERHITLEVEL = 10,	// 10레벨 이상일때는 미적용
	MISSION_STYLELEVEL = 10,	// 10레벨 이상일때는 미적용
}EMissionCountMax;
...
int const ms_aiResultRank[MISSION_RANK] = {115, 110, 90, 70, 60, 50, 40, 30, 0};// 미션 등급
int const ms_TotalScore_TotalUpPersent = ((MISSION_TOTALSCORE_MAX * MISSION_TOTAL_UP)/100);	// Sense 꽉차면 채워지는 %
int const ms_aiDownValue[MISSION_DOWN] = {0, 2, 3, 4, 5, 6, 8};
int const ms_iRewardItemMax = 4;	// 보상 아이템 최대 갯수(합계)

typedef enum eMissionPoint
{	
	MPOINT_MONSTER	= 0,//사냥점수
	MPOINT_COMBO	= 1,//콤보점수	
	MPOINT_ATTACK	= 2,//누적타격수
	MPOINT_DEMAGE	= 3,//피격점수
	MPOINT_DIE		= 4,//부활점수	
	MPOINT_BONUSEXP = 5,//추가EXP점수
	MPOINT_TIME		= 6,//시간
	MPOINT_TOTALSCORE = 7,
	MPOINT_MAX,
}EMissionPoint;
...
{
	MBONUS_NONE		= 0x00,
	MBONUS_PARTY	= 0x01,	//파티
	MBONUS_PCROOM	= 0x02,	//PC방
	MBONUS_EVENT	= 0x04,	//이벤트
	MBONUS_PREMIUM	= 0x08,	//프리미엄
	MBONUS_MISSION1	= 0x10,	//미션1
	MBONUS_MISSION2	= 0x20,	//미션2
	MBONUS_MISSION3 = 0x40,	//미션3
};
...
public:
	...
	{
		BM::CAutoMutex kLock(m_kMissionMutex);
		std::pair<ConUser::iterator, bool> kPair = m_kConUser.insert(std::make_pair(rkCharGuid,rkUserInfo));
		if ( !kPair.second )
		{
...
protected:
	...
	SET_GUID			m_kReqRestartUser;// 미션재시작을 요청한 유저
	SET_GUID			m_kReqDefenceNextStageUser;// 다음 스테이지 이동하기 위한 정보
	SET_GUID			m_kReqGadaCoinUse;			// 가다코인 사용한 유저
...
public:
	...
	virtual bool Build( CONT_DEF_MISSION_RESULT const &rkResult,
						...
						CONT_DEF_MISSION_ROOT const &rkRoot); //테이블 받아서 미션 정보 빌딩
