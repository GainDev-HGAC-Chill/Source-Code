...
bool PgNpcEventMgr::Parse(TiXmlElement const* pkRoot, int const iMapNo)
{
	...
	{
		std::pair<CONT_MAP_NPC_EVENT::iterator, bool> ret = m_kContMapNpcEvent.insert(std::make_pair(iMapNo, CONT_NPC_EVENT()));
		std::pair<CONT_MAP_NPC_EVENT::mapped_type::iterator, bool> ret_sec = ret.first->second.insert(std::make_pair(kID,kEvent));
	}
	return true;
}
