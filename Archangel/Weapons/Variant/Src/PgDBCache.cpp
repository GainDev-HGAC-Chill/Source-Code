...
bool PgDBCache::Q_DQT_DEFCLASS_PET_SKILL(CEL::DB_RESULT& rkResult)
{
	...
	while( rkResult.vecArray.end() != itr )
	{
		...
		if ( map_itr == map.end() )
		{
			std::pair<CONT_DEFCLASS_PET_SKILL::iterator, bool> kPair = map.insert( std::make_pair( kKey, CONT_DEFCLASS_PET_SKILL::mapped_type() ) );
			map_itr = kPair.first;
		}
...
bool PgDBCache::Q_DQT_DEFMAP( CEL::DB_RESULT &rkResult )
{
	...
	while(ItorEnd != Itor)
	{
...

		{
			if(kDefMap.iKey)
			{
				std::pair<SET_INT::iterator, bool> kContChkKey_Pair = kChkKey.insert(kDefMap.iKey);
...
{//select [MonsterNo], [Name], [Abil01], [Abil02], [Abil03], [Abil04], [Abil05], [Abil06], [Abil07], [Abil08], [Abil09], [Abil10], [Abil11], [Abil12], [Abil13], [Abil14], [Abil15] from dbo.TB_DefMonster
	CEL::DB_DATA_ARRAY::const_iterator itor = rkResult.vecArray.begin();
...
bool PgDBCache::Q_DQT_DEFSKILL( CEL::DB_RESULT &rkResult )
{
	...
	*/
	...
	while( rkResult.vecArray.end() != itor )
	{
		...
		std::pair<CONT_DEFSKILL::iterator, bool> kRet = map.insert( std::make_pair(element.SkillNo, element) );
		if(!kRet.second)
		{
...
bool PgDBCache::Q_DQT_DEFSKILL_NATIONCODE( CEL::DB_RESULT & Result )
{
	...
	g_kTblDataMgr.GetContDef(pConstContDef);	// DEFSKILL 컨테이너를 얻어옴.
	...
	CONT_DEFSKILL * pContDef = const_cast<CONT_DEFSKILL *>(pConstContDef);	// 국가코드가 추가된 테이블을 CONT_DEFSKILL 컨테이너에 넣기 위해 const 속성을 제거.
	if( NULL == pContDef )
	{
		VERIFY_INFO_LOG(false, BM::LOG_LV1, __FL__<<L"CONT_DEFSKILL is NULL");
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
	...
	if( !NationCodeUtil.IsEmpty() )
	{
		...
		{
			std::pair<CONT_DEFSKILL::iterator, bool> Ret = pContDef->insert( std::make_pair( (*itor).first, (*itor).second ) );
			if( !Ret.second )
			{
...
{//select [SkillAbilID], [Type01], [Value01], [Type02], [Value02], [Type03], [Value03], [Type04], [Value04], [Type05], [Value05], [Type06], [Value06], [Type07], [Value07], [Type08], [Value08], [Type09], [Value09], [Type10], [Value10] from dbo.TB_DefSkillAbil
	...
	while( rkResult.vecArray.end() != itor )
	{
		...
		std::pair<CONT_DEFSKILLABIL::iterator, bool> kRet = map.insert( std::make_pair(element.SkillAbilNo, element) );
		if(!kRet.second)
		{
...
bool PgDBCache::Q_DQT_DEFSKILLABIL_NATIONCODE( CEL::DB_RESULT & Result )
{
	...
	g_kTblDataMgr.GetContDef(pConstContDef);	// DEFSKILLABIL 컨테이너를 얻어옴.
	if( NULL == pConstContDef )
	{
	...
	CONT_DEFSKILLABIL * pContDef = const_cast<CONT_DEFSKILLABIL *>(pConstContDef);	// 국가 코드가 추가된 테이블을 CONT_DEFSKILLABIL 컨테이너에 넣기 위해 const 속성을 제거.
	if( NULL == pContDef )
	{
		VERIFY_INFO_LOG(false, BM::LOG_LV1, __FL__<<L"CONT_DEFEFFECT is NULL");
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
	...
	if( !NationCodeUtil.IsEmpty() )
	{
		...
		{
			std::pair<CONT_DEFSKILLABIL::iterator, bool> Ret = pContDef->insert( std::make_pair( (*itor).first, (*itor).second ) );
			if( !Ret.second )
			{
...
{//select [SetNo],[Limit],[ConditionType],[ConditionValue],[ResNo] from dbo.TB_DefSkillSet
	...
	while( rkResult.vecArray.end() != itor )
	{
		...
		std::pair<CONT_DEFSKILLSET::iterator, bool> kRet = map.insert( std::make_pair(element.bySetNo, element) );
		if(!kRet.second)
		{
...
bool PgDBCache::Q_DQT_DEFRES( CEL::DB_RESULT &rkResult )
{
	...
	while( rkResult.vecArray.end() != itor )
	{
		...
		(*itor).Pop( element.AddResNo[0] );	++itor;
		(*itor).Pop( element.AddResNo[1] );	++itor;
		(*itor).Pop( element.AddResNo[2] );	++itor;
...
bool PgDBCache::Q_DQT_LOAD_DEF_CHANNEL_EFFECT( CEL::DB_RESULT &rkResult )
{
	...
	while( rkResult.vecArray.end() != itor )
	{
		...
		std::pair<CONT_DEF_CHANNEL_EFFECT::iterator, bool> ibRet = map.insert( std::make_pair( std::pair<short,short>( element.sRealm, element.sChannel ), element ) );
...
bool PgDBCache::Q_DQT_DEFEFFECT_NATIONCODE( CEL::DB_RESULT & Result )
{
	CONT_DEFEFFECT const * pConstContDef = NULL;
	g_kTblDataMgr.GetContDef(pConstContDef);	// Effect 컨테이너를 얻어옴.
	if( NULL == pConstContDef )
	{
		VERIFY_INFO_LOG(false, BM::LOG_LV1, __FL__<<L"CONT_DEFEFFECT is NULL");
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
	...
	CONT_DEFEFFECT * pContDef = const_cast<CONT_DEFEFFECT *>(pConstContDef);	// 국가코드가 있는 테이블을 기존 CONT_DEFEFFECT 컨테이너에 넣기 위해 const 속성을 제거.
	if( NULL == pContDef )
	{
		VERIFY_INFO_LOG(false, BM::LOG_LV1, __FL__<<L"CONT_DEFEFFECT is NULL");
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
	...
	if( !NationCodeUtil.IsEmpty() )
	{
		...
		{
			std::pair<CONT_DEFEFFECT::iterator, bool> Ret = pContDef->insert( std::make_pair( (*itor).first, (*itor).second ) );
			if( !Ret.second )
			{
...
bool PgDBCache::Q_DQT_DEFEFFECTABIL_NATIONCODE( CEL::DB_RESULT & Result)
{ // TB_DefEffectAbil_Item에 Nation code 추가로 인해 UP_LoadDefEffectAbil2에서 TB_DefEffectAbil_Item Select 부분만 up_LoadDefEffectAbil_Item로 분리.
	CONT_DEFEFFECTABIL const * pConstContDef = NULL;
	g_kTblDataMgr.GetContDef(pConstContDef);	// EffectAbil 컨테이너를 얻어옴.
	if( NULL == pConstContDef )
	{
	...
	CONT_DEFEFFECTABIL * pContDef = const_cast<CONT_DEFEFFECTABIL *>(pConstContDef);	// 국가코드가 추가된 테이블을 CONT_DEFEFFECTABIL 컨테이너에 넣기 위해 const 속성을 제거.
	if( NULL == pContDef )
	{
		VERIFY_INFO_LOG(false, BM::LOG_LV1, __FL__<<L"CONT_DEFEFFECTABIL is NULL");
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
	...
	if( !NationCodeUtil.IsEmpty() )
	{
		...
		{
			std::pair<CONT_DEFEFFECTABIL::iterator, bool> Ret = pContDef->insert( std::make_pair( (*itor).first, (*itor).second ) );
			if( !Ret.second )
			{
...
bool PgDBCache::Q_DQT_DEFCHARACTER_BASEWEAR(CEL::DB_RESULT& rkResult)
{
	...
	while(rkResult.vecArray.end() != itor)
	{
		...
		std::pair<CONT_DEFCHARACTER_BASEWEAR::iterator, bool> ret = map.insert(std::make_pair(kElement.iWearNo, kElement));
	}
...
bool PgDBCache::Q_DQT_DEF_ITEM_PLUS_UPGRADE_NATIONCODE(CEL::DB_RESULT& rkResult)
{
	...
	/*	++count_itr;
		...
		if( !NationCodeUtil.IsEmpty() )
		{
			...
			{
				std::pair<CONT_DEF_ITEM_PLUS_UPGRADE::iterator, bool> Ret = pContDefItemTbl->insert( std::make_pair( (*iter).first, (*iter).second ) );
				if( !Ret.second )
				{
...
bool PgDBCache::Q_DQT_DEF_MONSTER_BAG_ELEMENTS(CEL::DB_RESULT& rkResult)
{
	...
	while(rkResult.vecArray.end() != itor)
	{
		...
		std::pair<CONT_DEF_MONSTER_BAG_ELEMENTS::iterator, bool> ret = map.insert(std::make_pair(kKey, kElement));
	}
...
bool PgDBCache::Q_DQT_DEF_MONSTER_BAG_CONTROL(CEL::DB_RESULT& rkResult)
{
	...
	while(rkResult.vecArray.end() != itor)
	{
		...
		std::pair<CONT_DEF_MONSTER_BAG_CONTROL::iterator, bool> ret = map.insert(std::make_pair(kElement.iParentBagNo, kElement));
	}
...
bool PgDBCache::Q_DQT_DEF_OBJECT(CEL::DB_RESULT& rkResult)
{
	...
	while(rkResult.vecArray.end() != itr)
	{
		...
		std::pair<CONT_DEF_OBJECT::iterator, bool> ret = map.insert(std::make_pair(kElement.iObjectNo, kElement));
		if ( !ret.second )
		{
...
bool PgDBCache::Q_DQT_DEF_OBJECTABIL(CEL::DB_RESULT& rkResult)
{
	...
	while(rkResult.vecArray.end() != itr)
	{
		...
		std::pair<CONT_DEF_OBJECTABIL::iterator, bool> ret = map.insert(std::make_pair(kElement.iObjectAbilNo, kElement));
		if ( !ret.second )
		{
...
bool PgDBCache::Q_DQT_DEF_ITEM_BY_LEVEL(CEL::DB_RESULT& rkResult)
{
	...
	while(rkResult.vecArray.end() != itr)
	{
		...
		std::pair<CONT_DEF_ITEM_BY_LEVEL::iterator, bool> kPair = map.insert( std::make_pair( iIndex, CONT_DEF_ITEM_BY_LEVEL::mapped_type() ) );
...
bool PgDBCache::Q_DEF_MISSION_DEFENCE_STAGE(CEL::DB_RESULT& rkResult)
{
	...
	for(int i = 0;i < iCount;++i)
	{
		...
		{			
			...
			std::pair<CONT_MISSION_DEFENCE_STAGE_BAG::iterator, bool> kRet = map.insert( std::make_pair(kKey, kTypeValue) );
			if( !kRet.second )
			{
...
bool PgDBCache::Q_DEF_MISSION_DEFENCE_WAVE(CEL::DB_RESULT& rkResult)
{
	...
	for(int i = 0;i < iCount;++i)
	{
		...
		{			
			...
			std::pair<CONT_MISSION_DEFENCE_WAVE_BAG::iterator, bool> kRet = map.insert( std::make_pair(kKey, kTypeValue) );
			if( !kRet.second )
			{
				(*kRet.first).second.kCont.push_back(kValue);
			}
...
bool PgDBCache::Q_DEF_MISSION_DEFENCE7_MISSION(CEL::DB_RESULT& rkResult)
{
	...
	for(int i = 0;i < iCount;++i)
	{
		...
		{			
			...
			std::pair<CONT_MISSION_DEFENCE7_MISSION_BAG::iterator, bool> kRet = map.insert( std::make_pair(kKey, kTypeValue) );
			if( !kRet.second )
			{
				(*kRet.first).second.kCont.push_back(kValue);
			}
...
bool PgDBCache::Q_DEF_MISSION_DEFENCE7_STAGE(CEL::DB_RESULT& rkResult)
{
	...
	for(int i = 0;i < iCount;++i)
	{
		...
		{			
			...
			std::pair<CONT_MISSION_DEFENCE7_STAGE_BAG::iterator, bool> kRet = map.insert( std::make_pair(kKey, kTypeValue) );
			if( !kRet.second )
			{
				(*kRet.first).second.kCont.push_back(kValue);
			}
...
bool PgDBCache::Q_DEF_MISSION_DEFENCE7_WAVE(CEL::DB_RESULT& rkResult)
{
	...
	for(int i = 0;i < iCount;++i)
	{
		...
		{			
			...
			std::pair<CONT_MISSION_DEFENCE7_WAVE_BAG::iterator, bool> kRet = map.insert( std::make_pair(kKey, kTypeValue) );
			if( !kRet.second )
			{
				(*kRet.first).second.kCont.push_back(kValue);
			}
...
{	
	...
	for(int i = 0;i < iCount;++i)
	{
		...
		{			
			...
			std::pair<CONT_MISSION_DEFENCE7_GUARDIAN_BAG::iterator, bool> kRet = map.insert( std::make_pair(kKey, kTypeValue) );
			if( !kRet.second )
			{
				(*kRet.first).second.kCont.push_back(kValue);
			}
...
bool PgDBCache::Q_DEF_MISSION_BONUSMAP(CEL::DB_RESULT& rkResult)
{
	...
	for(int i = 0;i < iCount;++i)
	{
		...
		//if( 0 < kValue.iUse )
		{
			std::pair<CONT_MISSION_BONUSMAP::iterator, bool> ret = map.insert(std::make_pair(kKey, kValue));
			if ( !ret.second )
			{
...
bool PgDBCache::Q_DQT_DEF_MISSION_CANDIDATE(CEL::DB_RESULT& rkResult)
{
	...
	while(rkResult.vecArray.end() != itor)
	{
		...
		std::pair<CONT_DEF_MISSION_CANDIDATE::iterator, bool> ret = map.insert(std::make_pair(kKey,kElement));
	}
...
bool PgDBCache::Q_DQT_DEF_MISSION_ROOT(CEL::DB_RESULT& rkResult)
{
	...
	while(rkResult.vecArray.end() != itor)
	{
		...
		std::pair<CONT_DEF_MISSION_ROOT::iterator, bool> ret = map.insert(std::make_pair(kElement.iMissionNo, kElement));
		if( !ret.second )
		{
			...
			while( class_iter != kClassVec.end() )
			{
				...
				if( 0 != iClass )
				{
					...
					else
					{
						...
						std::pair<CONT_MISSION_CLASS_REWARD_BAG::iterator, bool> kRet = map.insert( std::make_pair(kRewardKey, kValueType) );
						if( !kRet.second )
						{
							(*kRet.first).second.kCont.push_back(kValue);
						}
...
bool PgDBCache::Q_DQT_DEF_MISSION_RANK_REWARD(CEL::DB_RESULT& rkResult)
{
	...
	for(int i = 0;i < iItemCount;++i)
	{
		...
		{			
			...
			std::pair<CONT_MISSION_RANK_REWARD_BAG::iterator, bool> kRet = map.insert( std::make_pair(kKey, kTypeValue) );
			if( !kRet.second )
			{
				(*kRet.first).second.kCont.push_back(kValue);
			}
...
bool PgDBCache::Q_DQT_DEF_REVIVEPOS(CEL::DB_RESULT& rkResult)
{
	...
	while(rkResult.vecArray.end() != itor)
	{
		...
		std::pair<CONT_DEF_REVIVEPOS::iterator, bool> ret = map.insert(std::make_pair(kElement.iFromMapNo, kElement));
	}
...
bool PgDBCache::Q_DQT_DEF_ITEM_OPTION_ABIL(CEL::DB_RESULT& rkResult)
{
	...
	while( rkResult.vecArray.end() != itor )
	{
		...
		if( DBCacheUtil::IsDefaultNation(NationCodeStr) )
		{
			...
			{
				std::pair<CONT_TBL_DEF_ITEM_OPTION_ABIL::iterator, bool> kRet = DefaultMap.insert( std::make_pair(iOptionAbilNo, CONT_TBL_DEF_ITEM_OPTION_ABIL::mapped_type(iOptionAbilNo)) );
		...
		else
		{
			if( DBCacheUtil::IsCanNation< CONT_TBL_DEF_ITEM_OPTION_ABIL::key_type, BM::vstring, DBCacheUtil::AddError >(NationCodeStr, iOptionAbilNo, __FUNCTIONW__, __LINE__) )
			{
				...
				{
					std::pair<CONT_TBL_DEF_ITEM_OPTION_ABIL::iterator, bool> kRet = map.insert( std::make_pair(iOptionAbilNo, CONT_TBL_DEF_ITEM_OPTION_ABIL::mapped_type(iOptionAbilNo)) );
					find_iter = kRet.first;
				}
				if( map.end() != find_iter )
				{
...
bool PgDBCache::Q_DQT_DEF_PVP_GROUNDMODE( CEL::DB_RESULT& rkResult )
{
	...
	while ( itr != rkResult.vecArray.end() )
	{
		...
		// CONT_DEF_PVP_GROUNDGROUP의 mapped_type에 값을 채워준다.
		...
		{ // 타입이 존재하지 않으면 타입과 함께 내용도 컨테이너에 넣는다.
...
bool PgDBCache::Q_DQT_DEF_QUEST_MIGRATION(CEL::DB_RESULT& rkResult)
{
	...
	if( CEL::DR_SUCCESS == rkResult.eRet )
	{
		...
		while(rkVec.end() != result_iter)
		{
			...
			{
				std::pair<CONT_END_QUEST_MIGRATION_VER::iterator, bool> kRet = kCont.insert( std::make_pair(iMigrationRevision, CONT_END_QUEST_MIGRATION_VER::mapped_type(iMigrationRevision)) );
				if( kRet.second )
				{
					find_iter = kRet.first;
				}
...
bool PgDBCache::Q_DQT_DEF_QUEST_RANDOM_EXP(CEL::DB_RESULT& rkResult)
{
	...
	while(rkVec.end() != result_iter)
	{
		...
		std::pair<CONT_DEF_QUEST_RANDOM_EXP::iterator, bool> kRet = kMap.insert( std::make_pair(kNew.iLevel, kNew) );
		if( !kRet.second )
		{
...
bool PgDBCache::Q_DQT_DEF_QUEST_RANDOM_TACTICS_EXP(CEL::DB_RESULT& rkResult)
{
	...
	while(rkVec.end() != result_iter)
	{
		...
		std::pair<CONT_DEF_QUEST_RANDOM_TACTICS_EXP::iterator, bool> kRet = kMap.insert( std::make_pair(kNew.iLevel, kNew) );
		if( !kRet.second )
		{
			VERIFY_INFO_LOG(false, BM::LOG_LV1, __FL__<<L"Duplicate [TB_DefQuest_RandomExp] Level["<<kNew.iLevel<<L"]");
		}
...
bool PgDBCache::Q_DQT_DEF_QUEST_REWARD(CEL::DB_RESULT& rkResult)
{
	...
	while(rkVec.end() != result_iter)
	{
		...
		(*result_iter).Pop(kNew.iRewardPerLevel_Exp);		++result_iter;
		(*result_iter).Pop(kNew.iRewardPerLevel_Money);		++result_iter;
		...
		(*result_iter).Pop(kNew.i64DraClassLimit);		++result_iter;
		...
		(*result_iter).Pop(kNew.iWantedClearCount);		++result_iter;
		(*result_iter).Pop(kNew.iWantedItemNo);			++result_iter;
		(*result_iter).Pop(kNew.iWantedItemCount);		++result_iter;
		(*result_iter).Pop(kNew.iWantedCoolTime);		++result_iter;
		(*result_iter).Pop(kNew.iDra_WantedItemNo);		++result_iter;
		(*result_iter).Pop(kNew.iDra_WantedItemCount);		++result_iter;
		(*result_iter).Pop(kNew.bIsCanRemoteAccept);		++result_iter;
...
bool PgDBCache::Q_DQT_DEF_SPEND_MONEY(CEL::DB_RESULT& rkResult)
{
	...
	while(rkVec.end() != result_itor)
	{
		...
		std::pair<CONT_DEF_SPEND_MONEY::iterator, bool> kPair = kMap.insert(std::make_pair(kElement.iID,kElement));
		if( !kPair.second )
		{
...
bool PgDBCache::Q_DQT_DEF_GUILD_SKILL(CEL::DB_RESULT &rkResult)
{
	...
	while(rkVec.end() != result_iter)
	{
		...
		std::pair<CONT_DEF_GUILD_SKILL::iterator, bool> kRet = kCont.insert( std::make_pair(kNewElement.iSkillNo, kNewElement) );
		if( !kRet.second )
		{
...
std::pair<CONT_DEF_PROPERTY::iterator, bool> kRet = kCont.insert( std::make_pair(kNewElement.kKey, kNewElement) );
if( !kRet.second )
{
...
bool PgDBCache::Q_DQT_DEF_TACTICS_LEVEL(CEL::DB_RESULT &rkResult)
{
	...
	std::pair<CONT_DEF_TACTICS_LEVEL::iterator, bool> kRet;
	CEL::DB_DATA_ARRAY const &rkVec = rkResult.vecArray;
	CEL::DB_DATA_ARRAY::const_iterator result_iter = rkVec.begin();
	...
	if ( itr != rkResult.vecArray.end() )
	{
		...
		while ( itr != rkResult.vecArray.end() )
		{
			if ( iCount > 0 )
			{
				...
				std::pair<CONT_DEF_EMPORIA::iterator, bool> kPair = map.insert( std::make_pair( kElement.guidEmporiaID, kElement ) );
				if ( !kPair.second )
				{
			...
			else
			{
				...
				if ( iGrade < MAX_EMPORIA_GRADE )
				{
					...
					if ( itr != map.end() )
					{
						std::pair<CONT_DEF_EMPORIA_FUNCTION::iterator, bool> kPair = itr->second.m_kContDefFunc[iGrade].insert( std::make_pair( kElement.nFuncNo, kElement ) );
					}
					else
					{
...
void Q_DQT_DEF_ACHIEVEMENTS(CONT_DEF_ACHIEVEMENTS_SAVEIDX& kContAchievements_SaveIdx)
{
	...
	while( kContAchievements_SaveIdx.end() != iter )
	{
		...
		std::pair<CONT_DEF_ACHIEVEMENTS::iterator, bool> kRet = kCont[kNewElement.iType].insert(std::make_pair(kNewElement.iValue, kNewElement));
		if( kRet.second )
		{
...
bool PgDBCache::Q_DQT_DEF_LOAD_RECOMMENDATIONITEM(CEL::DB_RESULT &rkResult)
{
	...
	if((itr!=rkResult.vecArray.end()) && (return_iter != rkResult.vecResultCount.end()))
	{
		while(itr != rkResult.vecArray.end())
		{
			...
			std::pair<CONT_DEF_RECOMMENDATION_ITEM::iterator, bool> kPair = kCont.insert(std::make_pair(kElement.iIDX,kElement));
			if( !kPair.second )
			{
...
bool PgDBCache::Q_DQT_DEF_GROUND_RARE_MONSTER(CEL::DB_RESULT &rkResult)
{
	...
	while(itr != rkResult.vecArray.end())
	{
		...
		std::pair<CONT_DEF_GROUND_RARE_MONSTER::mapped_type::iterator, bool> kPair = kCont[kGroundNo].insert(std::make_pair(kElement.iMonNo,kElement));
		if( !kPair.second )
		{
...
bool PgDBCache::Q_DQT_DEF_TRANSTOWER(CEL::DB_RESULT &rkResult)
{
	...
	while(itr != rkResult.vecArray.end())
	{
		...
		std::pair<CONT_DEF_TRANSTOWER::iterator, bool> kPair = kCont.insert( std::make_pair( kGuid, CONT_DEF_TRANSTOWER_TARGET() ) );
		std::pair<CONT_DEF_TRANSTOWER_TARGET::iterator, bool> kPair2 = kPair.first->second.insert( kElement );
		if ( !kPair2.second )
		{
...
bool PgDBCache::Q_DQT_DEF_PARTY_INFO(CEL::DB_RESULT& rkResult)
{
	...
	while(itr != rkResult.vecArray.end())
	{
		...
		if( 0 < iUse )
		{
			std::pair<CONT_DEF_PARTY_INFO::iterator, bool> kPair = kCont.insert( std::make_pair( iContinent, CONT_DEF_PARTY_INFO_TARGET() ) );
			std::pair<CONT_DEF_PARTY_INFO_TARGET::iterator, bool> kPair2 = kPair.first->second.insert( kElement );
			if ( !kPair2.second )
			{
...
bool PgDBCache::Q_DQT_DEF_MISSION_RESULT(CEL::DB_RESULT& rkResult)
{
	...
	while(rkResult.vecArray.end() != itor)
	{
		...
		std::pair<CONT_DEF_MISSION_RESULT::iterator, bool> ret = map.insert(std::make_pair(kElement.iResultNo, kElement));
	}
...
bool PgDBCache::Q_DQT_DEF_OBJECT_BAG_ELEMENTS(CEL::DB_RESULT& rkResult)
{
	...
	while(rkResult.vecArray.end() != itr)
	{
		...
		std::pair<CONT_DEF_OBJECT_BAG_ELEMENTS::iterator, bool> ret = map.insert(std::make_pair(kElement.iElementNo, kElement));
		if ( !ret.second )
		{
...
bool PgDBCache::Q_DQT_DEF_OBJECT_BAG(CEL::DB_RESULT& rkResult)
{
	...
	while(rkResult.vecArray.end() != itr)
	{
		...
		std::pair<CONT_DEF_OBJECT_BAG::iterator, bool> ret = map.insert(std::make_pair(kElement.iBagNo, kElement));
		if ( !ret.second )
		{
...
bool PgDBCache::Q_DQT_LOAD_DEF_ITEM_AMP_SPECIFIC( CEL::DB_RESULT &rkResult )
{
	...
	while( rkResult.vecArray.end() != itor )
	{
		...
		if( DBCacheUtil::IsDefaultNation(kNationCodeStr) )
		{
			...
			std::pair<CONT_DEFITEM_AMP_SPECIFIC::iterator, bool> kRet = DefaultMap.insert(std::make_pair(iEquipType, empty));
		...
		else
		{
			...
			{
				SDefItemAmplify_Specific empty(iEquipType);
				std::pair<CONT_DEFITEM_AMP_SPECIFIC::iterator, bool> kRet = map.insert(std::make_pair(iEquipType, empty));
				kRet.first->second.kContSpecific.insert(std::make_pair(wAbilType, iRate));
			}
		}
	}
...
bool PgDBCache::Q_DQT_LOAD_DEF_DEATHPENALTY(CEL::DB_RESULT& rkResult)
{
	...
	while(iter != rkResult.vecArray.end())
	{
		...
		{
			std::pair<CONT_DEFDEATHPENALTY::iterator, bool> ibRet = kCont.insert(std::make_pair(kValue.iGroundNo, VEC_DEFDEATHPENALTY()));
	...
	while(iter != rkResult.vecArray.end())
	{
		...
		&&	pkDefMap->end() != map_iter )
		{
			...
			case GATTR_STATIC_DUNGEON:
				{
					std::pair<CONT_TBL_DEF_NPC_TALK_MAP_MOVE::iterator, bool> kRet = kCont.insert( std::make_pair(kKey, CONT_TBL_DEF_NPC_TALK_MAP_MOVE::mapped_type()) );
...
bool PgDBCache::Q_DQT_LOAD_DEF_JOBSKILL_ITEMUPGRADE(CEL::DB_RESULT &rkResult)
{
	...
	while( rkResult.vecArray.end() != iter )
	{
		...
		std::pair<CONT_DEF_JOBSKILL_ITEM_UPGRADE::iterator, bool> kRet = kContMap.insert( std::make_pair(kNewElement.iItemNo, kNewElement) );
		if( !kRet.second )
		{
...
bool PgDBCache::Q_DQT_LOAD_DEF_JOBSKILL_SAVEIDX(CEL::DB_RESULT &rkResult)
{
	...
	while( rkResult.vecArray.end() != iter )
	{
		...
		std::pair<CONT_DEF_JOBSKILL_SAVEIDX::iterator, bool> kRet = kContMap.insert( std::make_pair(kNewElement.iSaveIdx, kNewElement) );
		if( !kRet.second )
		{
...
bool PgDBCache::Q_LOAD_DEF_JOBSKILL_LOCATIONITEM(CEL::DB_RESULT& rkResult)
{
	...
	for(int i = 0;i < iCount;++i)
	{
		...
		std::pair<CONT_DEF_JOBSKILL_LOCATIONITEM::iterator, bool> kRet = map.insert( std::make_pair(kKey, kValue) );
		if( !kRet.second )
		{
...
bool PgDBCache::Q_LOAD_DEF_JOBSKILL_SKILL(CEL::DB_RESULT& rkResult)
{
	...
	for(int i = 0;i < iCount;++i)
	{
		...
		std::pair<CONT_DEF_JOBSKILL_SKILL::iterator, bool> kRet = map.insert( std::make_pair(kKey, kValue) );
		if( !kRet.second )
		{
			VERIFY_INFO_LOG( false , BM::LOG_LV0, __FL__ << _T("Data Error!!!") );
...
bool PgDBCache::Q_LOAD_DEF_JOBSKILL_TOOL(CEL::DB_RESULT& rkResult)
{
	...
	for(int i = 0;i < iCount;++i)
	{
		...
		std::pair<CONT_DEF_JOBSKILL_TOOL::iterator, bool> kRet = map.insert( std::make_pair( kKey, kValue ) );
		if( !kRet.second )
		{
			VERIFY_INFO_LOG( false , BM::LOG_LV0, __FL__ << _T("Data Error!!!") );
...
bool PgDBCache::Q_LOAD_DEF_SOCKET_ITEM(CEL::DB_RESULT& rkResult)
{
	...
	for(int i = 0;i < iCount;++i)
	{
		...
		std::pair<CONT_DEF_SOCKET_ITEM::iterator, bool> kRet = map.insert( std::make_pair(kKey, kValue) );
		if( !kRet.second )
		{
			VERIFY_INFO_LOG( false , BM::LOG_LV0, __FL__ << _T("Data Error!!!") );
...
bool PgDBCache::Q_DQT_LOAD_DEF_EXPEDITION_NPC(CEL::DB_RESULT &rkResult)
{
	...
	while( rkResult.vecArray.end() != iter )
	{
		...
		std::pair<CONT_DEF_EXPEDITION_NPC::iterator, bool> kRet = kContMap.insert( std::make_pair(kNewElement.kNpcGuid, kNewElement) );
		if( !kRet.second )
		{
...
bool PgDBCache::Q_DQT_PREMIUM_SERVICE( CEL::DB_RESULT &rkResult )
{
	...
	while( rkResult.vecArray.end() != itor )
	{
		...
		(*itor).Pop( element.iServiceNo );			++itor;
		(*itor).Pop( element.kTitle );				++itor;
		(*itor).Pop( element.byServiceType );		++itor;
		(*itor).Pop( element.byGrade );				++itor;
		(*itor).Pop( element.wUseDate );			++itor;
		for (int i=0; i<MAX_PREMIUM_ARTICLE; i++)
		{
			(*itor).Pop( element.iArticle[i] );			++itor;
		}
...
bool PgDBCache::Q_DQT_PREMIUM_ARTICLE( CEL::DB_RESULT &rkResult )
{
	...
	while( rkResult.vecArray.end() != itor )
	{
		...
		(*itor).Pop( element.iServiceTypeNo );		++itor;
		(*itor).Pop( element.wType );				++itor;
		(*itor).Pop( element.wLv );					++itor;
...
bool PgDBCache::Q_DQT_LOAD_DEF_EVENTMONSTERGROUP(CEL::DB_RESULT & Result)
{
	...
	while( Result.vecArray.end() != iter )
	{
		...
		std::pair<CONT_EVENTMONSTERGROUP::iterator, bool> InfoRet = ContMonsterInfo.insert( std::make_pair( NewElement.MapNo, NewElement ) );
		InfoRet.first->second.MonsterInfo.push_back(NewData);

		std::pair<CONT_DEF_EVENT_MONSTER_GROUP::iterator, bool> GroupRet = ContMonsterGroup.insert( std::make_pair(NewElement.MonsterGroupNo, ContMonsterInfo) );
		if( !GroupRet.second )
		{
			...
			{
				std::pair<CONT_EVENTMONSTERGROUP::iterator, bool> insterRet = GroupRet.first->second.insert( std::make_pair( NewElement.MapNo, NewElement ) );
...
bool PgDBCache::Q_DQT_LOAD_DEF_EVENTREWARDITEMGROUP(CEL::DB_RESULT & Result)
{
	...
	while( Result.vecArray.end() != iter )
	{
		...
		std::pair<CONT_DEF_EVENT_REWARD_ITEM_GROUP::iterator, bool> GroupRet = ContRewardItemGroup.insert( std::make_pair( NewElement.RewardItemGroup, Vec_RewardItemGroup) );
...
bool PgDBCache::Q_DQT_LOAD_DEF_EVENTSCHEDULE( CEL::DB_RESULT & Result )
{
	...
	while( Result.vecArray.end() != iter )
	{
		...
		if( 0 == ::_tcscmp( NewElement.EventType.c_str(), _T("BossBattle")) )
		{
			...
			if( 1 == NewElement.EventCount )
			{
				...
				if( pEventBossBattle )
				{
					...
					{
						NewElement.EventPeriod = boss_iter->second.LimitTime + 600; // 초
					}
					else
					{
		...
		else if( 0 == ::_tcscmp( NewElement.EventType.c_str(), _T("Race")) )
		{
			...
			if( 1 == NewElement.EventCount )
			{
				...
				if( pEventRace )
				{
					...
					{
						NewElement.EventPeriod = race_iter->second.LimitTime + 10; // 분
					}
					else
					{
		...
		std::pair<CONT_DEF_EVENT_SCHEDULE::iterator, bool> Ret = EventSchedule.insert( std::make_pair( NewElement.EventNo, NewElement) );
		if( !Ret.second )
		{
...
bool PgDBCache::Q_DQT_LOAD_DEF_EVENT_BOSSBATTLE( CEL::DB_RESULT & Result )
{
	...
	while( Result.vecArray.end() != iter )
	{
		...
		std::pair<CONT_DEF_EVENT_BOSSBATTLE::iterator, bool> Ret = BossBattle.insert( std::make_pair(NewElement.BossBattleEventNo, NewElement) );
		if( !Ret.second )
		{
...
*/
		std::pair<CONT_DEF_EVENT_RACE::iterator, bool> Ret = kContRaceEvent.insert( std::make_pair(NewElement.EventNo, NewElement) );
		if( !Ret.second )
		{
...
{// 이코드 지저분하게 만들지 마시오.
	...
	{	CEL::DB_QUERY kQuery( DT_LOCAL, DQT_LOAD_DEF_SPECIFIC_REWARD_EVENT, _T("EXEC [dbo].[up_LoadDefSpecificRewardEvent]"));	if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	...
	{	CEL::DB_QUERY kQuery( DT_DEF, DQT_DEFCLASS_NATIONCODE, _T("EXEC [dbo].[up_LoadDefClass_NationCode]"));			if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	...
	{	CEL::DB_QUERY kQuery( DT_DEF, DQT_DEFCLASS_ABIL_NATIONCODE, _T("EXEC [dbo].[up_LoadDefClassAbil_NationCode]")); if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	...
	{	CEL::DB_QUERY kQuery( DT_DEF, DQT_DEFSKILL_NATIONCODE, _T("EXEC [dbo].[up_LoadDefSkill_NationCode]"));			if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	...
	{	CEL::DB_QUERY kQuery( DT_DEF, DQT_DEFSKILLABIL_NATIONCODE, _T("EXEC [dbo].[up_LoadDefSkillAbil_NationCode]"));	if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	...
	{	CEL::DB_QUERY kQuery( DT_DEF, DQT_DEFEFFECT_NATIONCODE, _T("EXEC [dbo].[up_LoadDefEffect_NationCode]")); 		if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	...
	{	CEL::DB_QUERY kQuery( DT_DEF, DQT_DEFEFFECTABIL_NATIONCODE, _T("EXEC [dbo].[up_LoadDefEffectAbil_NationCode]"));if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	...
	{	CEL::DB_QUERY kQuery( DT_DEF, DQT_DEF_ITEM_PLUS_UPGRADE_NATIONCODE, _T("EXEC [dbo].[UP_LoadDefItemPlusUpgrade_NationCode]"));			if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	...
	{	CEL::DB_QUERY kQuery( DT_DEF, DQT_DEFCOOKING, _T("EXEC [dbo].[UP_LoadDefCooking]"));							if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	...
	{	CEL::DB_QUERY kQuery( DT_DEF, DQT_LOAD_DEF_ITEM_AMP_SPECIFIC, _T("EXEC [dbo].[up_LoadDefItemAmplify_Specific]"));	if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	...
	{	CEL::DB_QUERY kQuery( DT_DEF, DQT_LOAD_DEF_EXPEDITION_NPC, _T("EXEC [dbo].[up_LoadDefExpeditionNpc]"));	if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	{	CEL::DB_QUERY kQuery( DT_LOCAL, DQT_PREMIUM_SERVICE, _T("EXEC [dbo].[UP_LoadDefPremiumService]"));	if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	{	CEL::DB_QUERY kQuery( DT_LOCAL, DQT_PREMIUM_ARTICLE, _T("EXEC [dbo].[UP_LoadDefPremiumArticle]"));	if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}

	{	CEL::DB_QUERY kQuery( DT_LOCAL, DQT_LOAD_DEF_EVENTGROUP, _T("EXEC [dbo].[up_LoadDefEventGroup]")); if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ) { kStrQuery = kQuery.Command(); goto __RECORVER;} }
	{	CEL::DB_QUERY kQuery( DT_LOCAL, DQT_LOAD_DEF_EVENTMONSTERGROUP, _T("EXEC [dbo].[up_LoadDefEventMonsterGroup]")); if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ) { kStrQuery = kQuery.Command(); goto __RECORVER;} }
	{	CEL::DB_QUERY kQuery( DT_LOCAL, DQT_LOAD_DEF_EVENTREWARDITEMGROUP, _T("EXEC [dbo].[up_LoadDefEventRewardItemGroup]")); if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ) { kStrQuery = kQuery.Command(); goto __RECORVER;} }
	{	CEL::DB_QUERY kQuery( DT_LOCAL, DQT_LOAD_DEF_EVENT_BOSSBATTLE, _T("EXEC [dbo].[up_LoadDefEvent_BossBattle]")); if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ) { kStrQuery = kQuery.Command(); goto __RECORVER;} }
	{	CEL::DB_QUERY kQuery( DT_LOCAL, DQT_LOAD_DEF_EVENT_RACE, _T("EXEC [dbo].[up_LoadDefEvent_Race]")); if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ) { kStrQuery = kQuery.Command(); goto __RECORVER;} }
	{	CEL::DB_QUERY kQuery( DT_LOCAL, DQT_LOAD_DEF_EVENTSCHEDULE, _T("EXEC [dbo].[up_LoadDefEventSchedule]")); if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ) { kStrQuery = kQuery.Command(); goto __RECORVER;} }
	{	CEL::DB_QUERY kQuery( DT_DEF, DQT_LOAD_QUEST_LEVEL_REWARD, _T("EXEC [dbo].[up_LoadDefQuest_LevelReward]"));	if( S_OK != g_kCoreCenter.PushQuery(kQuery, bIsImmidiate) ){kStrQuery = kQuery.Command(); goto __RECORVER;}}
	return true;
__RECORVER:
	{
		//g_kTblDataMgr.Clear();
...
bool PgDBCache::OnDBExcute(CEL::DB_RESULT &rkResult)
{
	...
	case DQT_DEFEFFECT_NATIONCODE:				{ Q_DQT_DEFEFFECT_NATIONCODE(rkResult); }break;
	case DQT_DEFEFFECTABIL:						{ Q_DQT_DEFEFFECTABIL(rkResult); }break;
	case DQT_DEFEFFECTABIL_NATIONCODE:			{ Q_DQT_DEFEFFECTABIL_NATIONCODE(rkResult); }break;
	...
	case DQT_PREMIUM_SERVICE:					{Q_DQT_PREMIUM_SERVICE(rkResult);}break;
	case DQT_PREMIUM_ARTICLE:					{Q_DQT_PREMIUM_ARTICLE(rkResult);}break;
