...
void CSkill::AddCoolTime(int const iSkill, DWORD const dwCoolTime, int const iCoolTimeRate)
{
	...
	std::pair<MAP_COOLTIME::iterator, bool> kPair = m_kCoolTimeMap.insert( std::make_pair(iSkill, dwCalcCoolTime) );
	if ( !kPair.second )
	{
...
bool CSkillDefMgr::Build(const CONT_BASE &tblBase, const CONT_ABIL &tblAbil)
{
	...
	while( tblBase.end() != base_itor )
	{
		...
		std::pair<CONT_DEF::iterator, bool> kRet = m_contDef.insert( std::make_pair(pDef->No(), pDef) );
		if( !kRet.second )
		{
