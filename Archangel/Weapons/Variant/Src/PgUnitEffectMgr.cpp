		...
		case EFFECT_TYPE_ITEM:
			{
				...
				pkNew->Init();
				std::pair<ContEffect::iterator, bool> ret = m_kContItemEffect.insert(std::make_pair(iEffectKey,pkNew));
				if (!ret.second)
				{
		...
		default:
			{
				...
				pkNew->Init();
				std::pair<ContEffect::iterator, bool> ret = m_kContEffect.insert(std::make_pair(iEffectKey, pkNew));
				if (!ret.second)
				{
