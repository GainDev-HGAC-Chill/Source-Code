...
bool PgPlayer::SuspectHacking(short const sHackType, short const sAddIndex)
{
	...
	std::pair<CONT_HACKINDEX::iterator, bool> ret = m_kHackIndex.insert(std::make_pair(sHackType, 0));
...
{
	std::pair<CONT_GUARDIAN_INSTALL_DISCOUNT::iterator, bool> Ret = m_GuardianDiscount.insert( std::make_pair(GuardianType, RateValue) );
	if( !Ret.second )
	{
...
{
	std::pair<CONT_BLOCKCHAT_LIST::iterator, bool> ret = m_ChatBlockList.insert( std::make_pair(CharName, BlockType) );
