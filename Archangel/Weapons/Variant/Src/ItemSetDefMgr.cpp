	...
	///////////////////////////////////////////////////////////////////////////////
		...
		{//기본 리스트 번호 찾고 
			...
			if( iItemNo )
			{
				...
				if(pItemDef)
				{
					std::pair<CONT_ITEM_TO_SET::iterator, bool> ret = m_kContItemToSet.insert(std::make_pair(iItemNo, (*base_itor).second.iSetNo));
					if(!ret.second)
					{
		...
		std::pair<CONT_DEF::iterator, bool> kRet = m_contDef.insert( std::make_pair(pDef->SetNo(), pDef) );
		if( !kRet.second )
		{
