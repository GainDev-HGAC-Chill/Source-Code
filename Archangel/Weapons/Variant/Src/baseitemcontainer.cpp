...
HRESULT PgBaseItemContainer::ReadFromPacket(BM::Stream &kPacket)// 이따가 가상함수로 변경
{
	...
	while(const_size--)
	{
		...
		std::pair<CONT_ITEM_WRAPPER::iterator, bool> ret = insert(std::make_pair(kPos, PgItemWrapper(kItem, kPos)));
...
//				kChangeArray.push_back(kItemChange);
		...
		if( S_OK == (hReturn = CanModify(kItemWrapper)) )
		{
			...
			{//빈자리.
				...
				std::pair<CONT_ITEM_WRAPPER::iterator, bool> ret = insert(std::make_pair(kItemWrapper.Pos(), kItemWrapper));
				if(ret.second)
				{
...
/*		if(found_itor == end())
		{
			if(!bIsTest)
			{
				...
				std::pair<CONT_ITEM_WRAPPER::iterator, bool> ret = insert(std::make_pair(kInsertPos, kItemWrapper));//insert 는 여기서만 호출 하도록 하라.
				if(ret.second)
				{//있던거면 어뜨케??
