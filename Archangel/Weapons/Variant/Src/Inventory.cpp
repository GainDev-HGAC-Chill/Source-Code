...
HRESULT PgInventory::GetItems(EInvType const eInvType, ContHaveItemNoCount &rkOut, bool const bChkTimeOut)
{
	...
	while(pkCont->end() != item_iter)
	{
		...
		{
			std::pair<ContHaveItemNoCount::iterator, bool> kRet = rkOut.insert( std::make_pair(rkElement.ItemNo(), rkElement.Count()) );
			if( !kRet.second )
			{
...
HRESULT PgInventory::GetItems(EInvType const eInvType, CONT_HAVE_ITEM_POS &rOut, bool const bChkTimeOut)
{
	...
	while(pkCont->end() != item_iter)
	{
		...
		if(false == bChkTimeOut || false == rkElement.IsUseTimeOut())
		{
			std::pair<CONT_HAVE_ITEM_POS::iterator, bool> kRet = rOut.insert( std::make_pair(rkElement.ItemNo(), CONT_HAVE_ITEM_POS::mapped_type()) );
...
bool PgInventory::Modify(SItemPos const &kPos, PgBase_Item const &kItem)
{
	...
	if(S_OK == GetContainer((EInvType)kPos.x, pCont))
	{
		...
		else
		{
			if(kItem.ItemNo())
			{
				std::pair<ItemContainer::iterator, bool> ret = pCont->insert(std::make_pair(kPos, PgItemWrapper(kItem, kPos)));
				if( !ret.second )
				{
					LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
				}
	...
	assert(NULL && "PgInventory::Modify No InvCont");
	LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
	return false;
}
...
HRESULT PgInventory::RemoveItemAny(__int64 const iCause, SPMOD_Add_Any const &kData, DB_ITEM_STATE_CHANGE_ARRAY &kChangeArray,PgContLogMgr & kContLogMgr)
{
	...
	EInvType eFindInvType[] = {IT_EQUIP, IT_CONSUME, IT_ETC, IT_CASH, IT_SAFE, IT_CASH_SAFE, IT_FIT, IT_FIT_CASH, IT_SAFE_ADDITION, 
