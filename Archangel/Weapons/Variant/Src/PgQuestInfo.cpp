				...
				
				{
					...
					while( Reward().kClassRewardSet1.end() != reward1_iter )
					{
						...
						while( kClasses.end() != class_iter )
						{
							...
							{
								std::pair<ContClassCount::iterator, bool> kRet = kMale.insert( std::make_pair(iClassNo, 1) );
								if( !kRet.second )
								{
							...
							{
								std::pair<ContClassCount::iterator, bool> kRet = kFemale.insert( std::make_pair(iClassNo, 1) );
								if( !kRet.second )
								{
...
bool PgQuestInfo::ParseXml(TiXmlElement *pkNode)
{
	if( !pkNode )
	{
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
	...
	TiXmlElement *pkIDNode = pkNode->FirstChildElement("ID");
	if( !pkIDNode )
	{
		PgQuestInfoVerifyUtil::AddError(PgQuestInfoVerifyUtil::SQuestInfoError( this, PgQuestInfoVerifyUtil::ET_ParseError, __FUNCTIONW__, __LINE__, BM::vstring(m_kXmlFileName) << _T("is Can't have <ID> Element") ));
		return false;
	}

	m_kBasic.iQuestID = PgStringUtil::SafeAtoi(pkIDNode->GetText());
