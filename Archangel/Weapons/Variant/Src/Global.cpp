...
float CalcTotalElementalResistRateSimple(int const iAttackerLevel, int const iDefenderResiste)
{
	...
	float result = fDefenderResiste/(fDefenderResiste*0.3f + fAttackerLevel * (30 + fAttackerLevel * 2.0f) + 30.0f);
...
float const PgSkillHelpFunc::MAX_CRITICAL_RATE		= 10000.0f;
...
int PgSkillHelpFunc::GetCalcHitSuccessRate(CUnit* pkCaster,  CUnit* pkTarget, CSkillDef const* pkCasterSkillDef, int const iDecDodgeAbs, int const iDecDodgeRate)
{
	...
	{// 레벨 보정 부분
		int const iLevelDiff = pkCaster->GetAbil(AT_LEVEL) - pkTarget->GetAbil(AT_LEVEL);
...
int PgSkillHelpFunc::CalcSuccessValueToRate(eAbilityType const eType, CUnit* pkCaster, CUnit* pkTarget, CSkillDef const* pkSkillDef, int const iAddExceptionalFinalSuccessRate)
{
	...
	int const iCasterLv = pkCaster->GetAbil(AT_LEVEL);
	if(pkTarget
		&& 0 < iRegistRateType
		)
	{
	...
	// 최종적으로 적용할 율값을 얻어낸다
	return CalcSuccessValueToRate( iSuccessValueType, iAbilValue, iFinalSuccessRateType, iAddFinalSuccessRate, iCasterLv, iTargetLv, pkCaster);
}
...
{	
	float fValue = 0;
	//int iAdjustedCriticalPowerRate = 0;
	int iAdjustedCriticalPowerRate = pkCaster->GetAbil(AT_C_CRITICAL_POWER);
	/*if(pkCaster && UT_PLAYER == pkCaster->UnitType())
	{
		// 크리티컬 데미지 상승 최대값을 먼저 조율하고
		iAdjustedCriticalPowerRate = GetRateValue(fValue,AT_CRITICAL_DAMAGE_VALUE,pkCaster);
	
	}
	else
	{
		iAdjustedCriticalPowerRate = static_cast<int>( AdjustMaxValue(AT_CRITICAL_POWER, static_cast<float>(iCriticalPowerRate), iCasterLv, iTargetLv) );
	}*/
