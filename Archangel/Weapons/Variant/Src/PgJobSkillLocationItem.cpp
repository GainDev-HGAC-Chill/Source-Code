...
{
	std::pair<CONT_JOBSKILL_LOCATIONINFO::iterator, bool> kRet = m_kContLocationItem.insert(std::make_pair(kTriggerID, kValue));
	if(!kRet.second)
	{
...
void PgJobSkillLocationItem::GetAll(CONT_JOBSKILL_LOCATIONINFO& kLocationItem)
{
	...
	{
		std::pair<CONT_JOBSKILL_LOCATIONINFO::iterator, bool> kRet = kLocationItem.insert(std::make_pair(itor->first, itor->second));
		if( !kRet.second )
		{
		}
		++itor;
...
void PgJobSkillLocationUser::Set(KEY_TYPE const& kTriggerID, BM::GUID const kCharGuid, int const iSkillNo, int const iExpertness, DWORD const dwDrationTime, int const iItemNo, int const iUseExhaustion)
{
	...
	{
		std::pair<CONT_JOBSKILL_LOCATIONUSER::iterator, bool> kRet = m_kContLocationUser.insert(std::make_pair(kTriggerID, CONT_JOBSKILL_LOCATIONUSER::mapped_type()));
		iter = kRet.first;
	}
	...
	std::pair<VALUE_TYPE::iterator, bool> kRet = (*iter).second.insert( std::make_pair(kValue.kGuid, kValue) );
	if( !kRet.second )
	{
		(*kRet.first).second = kValue;
	}
