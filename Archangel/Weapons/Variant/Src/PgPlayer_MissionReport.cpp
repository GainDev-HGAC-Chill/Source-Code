...
void PgPlayer_MissionReport::ReadFromPacket_MissionReport( BM::Stream& rkPacket )
{
	...
	while ( iDataSize-- )
	{
		...
		{
			std::pair<CONT_PLAYER_MISSIONREPORT::iterator, bool> kPair = m_kContData.insert( std::make_pair( kKey, CONT_PLAYER_MISSIONREPORT::mapped_type() ) );
...
bool PgPlayer_MissionReport::Load( int const iArea, CEL::DB_DATA_ARRAY::const_iterator &itr )
{
	...
	{
		std::pair<CONT_PLAYER_MISSIONREPORT::iterator, bool> kPair = m_kContData.insert( std::make_pair( iArea, kVecMissionData ) );
		if ( !kPair.second )
		{
