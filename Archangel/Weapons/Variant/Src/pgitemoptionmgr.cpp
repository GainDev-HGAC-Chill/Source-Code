	...
	{//DB의 전체 내용으로.
		...
		while(pos_count > pos_index)
		{
			...
			{//맞는 포지션
				...
				std::pair<CONT_OPTION_CANDI::iterator, bool> ret = m_kContOptCandi.insert( std::make_pair(kKey, CONT_OPTION_CANDI::mapped_type()) );
				if(kOptElem.iOutRate && kOptElem.iOptionType)
				{
					std::pair<CONT_TYPE_RATE::iterator, bool> sub_ret = ret.first->second.insert(std::make_pair(kOptElem.iOptionType, kOptElem.iOutRate));
