...
void tagPartyOption::Default()
{
	...
	SetOptionLevel(3);			// Default -3, +3 Level
	m_kPartySubName.clear();
...
HRESULT PgParty::Add(SPartyUserInfo const& rkPartyMember, bool const bMaster)
{
	...
	std::pair<ContPartyMember::iterator, bool> kRet = m_kMemberArr.insert( std::make_pair(rkPartyMember.kCharGuid, rkPartyMember) );
	if( !kRet.second )
	{
...
{//맴버만 로딩한다.
	...
	for( BYTE i=0; cSize>i; ++i )
	{
		...
		std::pair<ContPartyMember::iterator, bool> kRet = m_kMemberArr.insert(std::make_pair(kTemp.kCharGuid, kTemp));//강제 추가
		assert(kRet.second);
	}
...
HRESULT PgGlobalParty::AddWait(BM::GUID const &rkCharGuid)
{
	...
	std::pair<ContWaitJoinList::iterator, bool> kRet = m_kContWaitJoinList.insert(rkCharGuid);// 파티에 가입하려는 유저를 대기자 명단에 올린다.
	if(kRet.second)
	{
...
void PgLocalParty::RefreshTotalLevel()
{
	...
	{
		ContPartyMember::mapped_type const &rkElement = iter->second;
		std::pair<ContGndTotalLevel::iterator, bool> kPair = m_kGndTotalLevel.insert( std::make_pair(rkElement.kChnGndKey, rkElement.sLevel) );
		if( !kPair.second )
		{
