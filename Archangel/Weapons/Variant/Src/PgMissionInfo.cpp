...
bool PgMissionInfo::ParseXml_Options(TiXmlElement* pkNode)
{
	...
	std::pair<ContMissionOptionChapterList::iterator, bool> ret = m_kDepend_MissionOptionChapterList.insert( std::make_pair(GroupNo(), m_kDepend_MissionOptionChapter) );
	if( !ret.second )
	{
...
bool PgMissionInfo::SetMissionOptionChapter(int const iGround)
{
	...
	for(int iLevel=0; iLevel!=MAX_MISSION_LEVEL; ++iLevel)
	{
		...
		{
			std::pair<ContMissionOptionChapterLevelList::iterator, bool> ret = m_kDepend_MissionOptionChapterLevelList.insert( std::make_pair(iLevel, m_kDepend_MissionOptionChapter) );
			if( !ret.second )
			{
...
bool PgMissionInfo::SetMissionOptionMission()
{
	...
	for(int iLevel=0; iLevel!=MAX_MISSION_LEVEL; ++iLevel)
	{
		...
		{
			std::pair<ContMissionOptionMissionLevelList::iterator, bool> ret = m_kDepend_MissionOptionMissionLevelList.insert( std::make_pair(iLevel, m_kDepend_MissionOptionMission) );
			if( !ret.second )
			{
...
bool PgDefenceMissionSkill::SetSkillInsert(int const iTeam, int const iSkillNo)
{
	...
	{
		std::pair<CONT_DEFENCE_SKILL::iterator, bool> kPair = m_kContDefenceSkill.insert(std::make_pair(iTeam, CONT_DEFENCE_SKILL::mapped_type()));
		iter = kPair.first;
	}
	else
	{
