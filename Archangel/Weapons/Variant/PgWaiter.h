...
public:
	...
	bool AddWaiter(const _Key& rkKey, const _Val& rkWaiter)
	{
		...
		if(pkElement)
		{
			std::pair<ContTotal::mapped_type::iterator, bool> ibRet = pkElement->insert(rkWaiter);
			return ibRet.second;
		}
		LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
		return false;
...
protected:
	bool Add(const _Key& rkKey, ContWaiter* &pkWaiter)
	{
		...
		if(m_kStack.end() == kTotal_iter)
		{
			...
			std::pair<ContTotal::iterator, bool> ibRet = m_kStack.insert(std::make_pair(rkKey, kElement));
			if(!ibRet.second)
			{
				LIVE_CHECK_LOG(BM::LOG_LV1, __FL__ << _T("Return false"));
				return false;
