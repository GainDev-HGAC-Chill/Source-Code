...
typedef struct tagPlayerModifyOrderData_AddCash
{
	...
	tagPlayerModifyOrderData_AddCash(BM::GUID const kMemberGuid, __int64 const i64AddCash, __int64 const i64AddBonus, std::wstring const& kCharName, EItemModifyParentEventType const kCause)
	{
		MemberGuId(kMemberGuid);
		AddCash(i64AddCash);
		AddBonus(i64AddBonus);
		CharName(kCharName);
		Cause(kCause);
	}
	CLASS_DECLARATION_S(BM::GUID, MemberGuId);
	CLASS_DECLARATION_S(__int64, AddCash);
	CLASS_DECLARATION_S(__int64, AddBonus);
	CLASS_DECLARATION_S(std::wstring, CharName);
