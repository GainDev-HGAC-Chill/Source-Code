...
void PgExpedition< _T_Party >::ReadFromPacket(BM::Stream &rkPacket)
{
	...
	for(int i = 0 ; i < nUserInfoSize ; ++i)
	{
		...
		std::pair<ContExpeditionUserInfo::iterator, bool> Ret = m_kUserInfo.insert(std::make_pair(UserInfo.kCharGuid, UserInfo));	// 무조건 추가함.
		if( !Ret.second )
		{
...
bool PgExpedition< _T_Party >::AddExpeditionMemberInfo(SContentsUser const & rkUserInfo, SExpeditionUserInfo & OutInfo, unsigned short const sTeam, bool bAlive)
{
	...
	std::pair<ContExpeditionUserInfo::iterator, bool> Ret = m_kUserInfo.insert(std::make_pair(rkUserInfo.kCharGuid, ExpeditionUserInfo));
	if( Ret.second)
	{
...
bool PgExpedition< _T_Party >::AddExpeditionMemberInfo(SPartyUserInfo* pkPartyUserInfo, SExpeditionUserInfo & OutInfo, unsigned short const sTeam, bool bAlive)
{
	...
	std::pair<ContExpeditionUserInfo::iterator, bool> Ret = m_kUserInfo.insert(std::make_pair(pkPartyUserInfo->kCharGuid, ExpeditionUserInfo));
	if( Ret.second)
	{
		OutInfo = Ret.first->second;
		return true;
...
HRESULT PgGlobalExpedition::AddWaitMember(BM::GUID const & rkCharGuid)
{
	...
	std::pair<ContWaitJoinList::iterator, bool> Ret = m_kJoinWaitList.insert( std::make_pair(rkCharGuid, dwCurTime) ); // 원정대 가입 대기자열에 등록.
	if( Ret.second )
	{
		return PRC_Success;
	}
