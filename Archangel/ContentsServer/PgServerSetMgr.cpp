...
void PgRealmUserManager::Locked_Recv_PT_IM_T_ANS_SWITCH_USER_ADD(HRESULT const hRecvRet, SSwitchPlayerData const &kSPD)
{
	...
	&& pData)
	{
		std::pair<CONT_CENTER_PLAYER_BY_KEY::iterator, bool> const ret = m_kContPlayer_MemberKey.insert( std::make_pair( kSPD.guidMember, pData) );
		if( ret.second )//넣기 성공
		{
			const std::pair<CONT_CENTER_PLAYER_BY_ID::iterator, bool> ret2 = m_kContPlayer_MemberID.insert( std::make_pair( kSPD.ID(), pData) );
...
void PgRealmUserManager::_ProcessRemoveCharacter( SCenterPlayerData* pkCenterPlayer )
{
	if (pkCenterPlayer->pkPlayer != NULL)
	{
		...
		SAFE_DELETE(pkCenterPlayer->pkPlayer);
	}
	SAFE_DELETE(pkCenterPlayer->pkBackupPlayer);
	return;
}
	...
	for ( ; key_itr != m_kContPlayer_MemberKey.end() ; ++key_itr )
	{
		...
		if ( pCData )
		{
			...
			if ( pCData->pkPlayer )
			{
				...
				{
					std::pair<CONT_CHANNEL_USERCOUNT::iterator, bool> kPair = rkCont.insert( std::make_pair( pCData->pkPlayer->GetChannel(), 0 ) );
					++(kPair.first->second);
					++iCount;
				}
			}
			...
			{
				std::pair<CONT_CHANNEL_USERCOUNT::iterator, bool> kPair = rkCont.insert( std::make_pair( pCData->kSwitchServer.nChannel, 0 ) );
				++(kPair.first->second);
				++iCount;
			}
		}
...
bool PgRealmUserManager::RemovePlayer(CONT_CENTER_PLAYER_BY_KEY::mapped_type pCPD)
{
		...
		&&	m_kContPlayer_MemberID.end() != m_kContPlayer_MemberID.find(pCPD->kID))
	{
		...
		if( pkPlayer
		&&	m_kContPlayer_CharKey.end() != m_kContPlayer_CharKey.find(pkPlayer->GetID())
		&&	m_kContPlayer_CharName.end() != m_kContPlayer_CharName.find(pkPlayer->Name()))
		{
...
HRESULT PgRealmUserManager::ProcessSelectPlayer(PgDoc_Player *pkPlayer, BM::GUID const &kSelectGuid, bool const bPassTutorial)
{
	...
	if ( GetPlayerInfo(pkPlayer->GetMemberGUID(), true, kLockPlayer) )
	{
		...
		std::pair<CONT_CENTER_PLAYER_BY_KEY::iterator, bool> kPair = m_kContPlayer_CharKey.insert(std::make_pair(pkPlayer->GetID(), pkMemberData));
		if ( true == kPair.second )
		{
			std::pair<CONT_CENTER_PLAYER_BY_ID::iterator, bool> kPair2 = m_kContPlayer_CharName.insert(std::make_pair(pkPlayer->Name(), pkMemberData));
...
{
	std::pair<CONT_EVENT_USER::iterator, bool> kRet = m_kContEventUser.insert(kCharGuId);
	return kRet.second;
}
...
void PgRealmUserManager::Locked_Recv_PT_M_I_REQ_CASH_MODIFY( BM::Stream * const pkPacket ) const
{
	...
	if (pkTran != NULL)
	{
		...
		pkTran->Bonus(kAddCash.AddBonus());
...
void PgRealmUserManager::Locked_Add_Home_Unit(BM::GUID const & kGuid,SMYHOME const kHome, CONT_HOME_ITEM const & kContItem, BM::Stream const & kAddonPacket)
{
	...
	std::pair<CONT_CENTER_PLAYER_BY_KEY::iterator, bool> const ret = m_kContPlayer_MemberKey.insert( std::make_pair( kGuid, pData) );
	if(true == ret.second )//넣기 성공
	{
		const std::pair<CONT_CENTER_PLAYER_BY_ID::iterator, bool> ret2 = m_kContPlayer_MemberID.insert( std::make_pair( pData->kID, pData) );
		if( true == ret2.second )//넣기 성공
		{
			std::pair<CONT_CENTER_PLAYER_BY_KEY::iterator, bool> ret3 = m_kContPlayer_CharKey.insert(std::make_pair(kGuid, pData));
			if ( true == ret3.second )
			{
				std::pair<CONT_CENTER_PLAYER_BY_ID::iterator, bool> ret4 = m_kContPlayer_CharName.insert(std::make_pair(std::wstring(kData.szName), pData));
...
bool PgRealmUserManager::Locked_RegistGuildInfo(const BM::GUID &rkGuildGuid, const std::wstring rkGuildName, const BYTE* pkInvExtern, const BYTE* pkExternIdx, CONT_SELECTED_CHARACTER_ITEM_LIST *pkItemList, const __int64 iMoney )
{
	...
	std::pair<CONT_CENTER_PLAYER_BY_KEY::iterator, bool> const kRet = m_kContPlayer_Guild.insert( std::make_pair( rkGuildGuid, pData) );
	if( !kRet.second )
	{
		SAFE_DELETE(pData);
