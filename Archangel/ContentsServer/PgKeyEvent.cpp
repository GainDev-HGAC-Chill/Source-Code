...
{
	if( LOCAL_MGR::NC_USA!=g_kLocal.ServiceRegion() )
	{
		CAUTION_LOG(BM::LOG_LV4, L"Only America ServiceRegion. Not Allow other nation.");
		return false;
	}

	static bool bNotAllow = false;
	if (bNotAllow)
	{
		CAUTION_LOG(BM::LOG_LV4, L"Not Allow two time");
		return false;
	}

	std::wstring q = L"SELECT ui.itemguid, ui.itemno, uie.[Enchant_01], uie.[Enchant_02], uie.[Enchant_03], uie.[Enchant_04] FROM [DR2_User].[dbo].[TB_UserItem_Enchant] as uie, [DR2_User].dbo.tb_useritem as ui where 0<(uie.[Enchant_01] & cast(9007199254609920 as bigint) ) and ui.itemguid = uie.itemguid and ui.itemno in(93008040,93008050,93008060,93008070,93008080,93008090,93008100,93008110,93008120,93008130,93008140,93008150,93008160,93008170,93008180,93008190,91100010,91200010,91300010,91400010,80101000,80101010,80101020,80102000,80102010,80102020,80103000,80103010,80103020,80104000,80104010,80104020,80105000,80105010,80105020,80106000,80106010,80106020,80107000,80107010,80107020,80108000,80108010,80108020,98002540,98002550,98002560,98002570,98002580,98002590,98002600,98002610,19000290,19000300,19000310,19000320,19000330,19000340,19000350,19000360,19000370,19000380,19000390,19000400,19000410,19000420,19000430,19000440,19000450,19000460,19000470,19000480,19000490,19000500,19000510,19000520,19000530,19000540,19000550,91200020,91200030,91200040,91200050,91200060,91200070,91200080,91200090)";

	CAUTION_LOG(BM::LOG_LV4, L"DQT_SELECT_CASHITEM_FOR_SOUL_LEVEL_DEC");

	CEL::DB_QUERY kQuery( DT_PLAYER, DQT_SELECT_CASHITEM_FOR_SOUL_LEVEL_DEC,q);
