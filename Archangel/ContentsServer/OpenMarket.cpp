...
{
	std::pair<CONT_OPENMARKET::iterator, bool> kRet = m_kContMarket.insert(std::make_pair(kUserMarket.OwnerGuId(),kUserMarket));
	if(!kRet.second)
	{
...
void PgOpenMarketMgr::BuildSearchKey(PgOpenMarket const & kUserMarket)
{
	...
	for(CONT_ARTICLEINFO_EX::const_iterator iter = kContArticleInfo.begin();iter != kContArticleInfo.end();++iter)
	{
		...
		m_kItemNameMap.BuildNameKey(kItemName,kArticle.kItem.ItemNo());

		m_kContSearchMgr[SARTICLE_KEY(AKT_OWNER_GUID,kOwnerGuid)][kOwnerGuid].insert(kArticle.kItemGuId);
		m_kContSearchMgr[SARTICLE_KEY(AKT_ITEM_NO,kArticle.kItem.ItemNo())][kOwnerGuid].insert(kArticle.kItemGuId);
		...
		{
			if( ITEM_SOUL_NO == kArticle.kItem.ItemNo() ) // 예외 소울은 장비탭에 들어가서, 장착위치로 SearchKey를 빌드 해준다(소울과 다스소울무기가 같이 검색되는 불편함 개선)
			{
				m_kContSearchMgr[SARTICLE_KEY(AKT_SMALL_CATEGORY,kArticle.kEquipPos)][kOwnerGuid].insert(kArticle.kItemGuId);
			}
		}
		...
		std::pair<CONT_MINIMUM_COST::iterator, bool> kRet = m_kContMinimumCost.insert(std::make_pair(kArticle.kItem.ItemNo(),kArticle.kArticleCost));
...
========================================================================================================================*/
	...
	if( !kSearchName.empty() )
	{
		...
		if( !bSearchNameType )
		{
			...
			m_kItemNameMap.FindItemId(kSearchName,kItemId);

			for(ITEM_NAME_MAP::SET_ID::iterator iter = kItemId.begin();iter != kItemId.end();++iter)
			{
				//kORKeyList.push_back(CONT_ARTICLE_KEY_LIST::value_type(AKT_ITEM_NO,(*iter)));
