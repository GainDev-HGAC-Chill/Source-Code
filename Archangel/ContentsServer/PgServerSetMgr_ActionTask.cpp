												...
												SRealmGroundKey const &kGndKey,	BM::Stream const &kAddonPacket, BM::Stream & rkPacket)
{
	...
	for( ; itor != kContOrder.end() ; ++itor )
	{
		...
		else
		{
			...
			std::pair<CONT_DOC_PLAYER::iterator, bool> kInsert = kContCopyPlayer.insert(std::make_pair(kOrder.ModifyOwner(), pkCopyPlayer));
			if ( kInsert.second )
			{
...
////////////////////////////////////////////////////////////////////
	...
	if(S_OK == hRet)//카피에다 시도.
	{
		...
		if(S_OK == hRet)//카피에다 시도.
		{
			...
			{//받을 사람 기준으로 정리
				...
				std::pair<RESULT_HASH::iterator, bool> insert_ret = kRetHash.insert(std::make_pair( SModifyOrderOwner(rkChangeInfo.CharacterGuid(), rkChangeInfo.OwnerType()), DB_ITEM_STATE_CHANGE_ARRAY()));
	...
	else
	{
		...
		while(kContOrder.end() != ret_iter)
		{
			...
			std::pair<RESULT_HASH::iterator, bool> kRet = kRetHash.insert(std::make_pair(rkOrder.ModifyOwner(), DB_ITEM_STATE_CHANGE_ARRAY()));
...
{
	std::pair<CONT_GEN_SYS_ITEM::iterator, bool> kRet = rkGenSysItem.insert(std::make_pair(rkOrder.OwnerGuid(),CONT_ITEM_CREATE_ORDER()));
