...
void PgIndunPartyMgr::SyncIndunParty(BM::Stream * pkPacket)
{
	...
	BM::CAutoMutex kLock(m_kMutex);
	std::pair<CONT_INDUN_PARTY_LIST::iterator, bool> kPair = m_kContIndunPartyList.insert(std::make_pair(iMapNo,SPartyInfo()));
	CONT_INDUN_PARTY_LIST::mapped_type & kValue = kPair.first->second;

	std::pair<CONT_INDUN_PARTY_CHANNEL::iterator, bool> kPair2 = kValue.kContChannel.insert(std::make_pair(sChannel, kCont));
