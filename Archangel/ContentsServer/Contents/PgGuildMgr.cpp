...
{
	std::pair<CONT_GUILD_ENTRANCE_APPLICANT_LIST::iterator, bool> kResult;
	kResult = m_kGuildApplicantList.insert(std::make_pair(kGuildEntranceApplicant.kCharGuid, kGuildEntranceApplicant));
	if( !kResult.second )
	{
		return ;
	}
...
bool PgGuildMgr::AddGuildName(std::wstring const& rkGuildName, BM::GUID const &rkGuildGuid)
{
	...
	std::pair<ContWstrToGuild::iterator, bool> ibRet = m_kNameToGuild.insert(std::make_pair(rkGuildName, rkGuildGuid));
	return ibRet.second;
}
...
bool PgGuildMgr::AddCharToGuild(BM::GUID const &rkCharGuid, BM::GUID const &rkGuildGuid)
{
	...
	std::pair<ContCharToGuild::iterator, bool> ibRet = m_kCharToGuild.insert(std::make_pair(rkCharGuid, rkGuildGuid));
...
{
	std::pair<ContGuild::iterator, bool> kRet;
	BM::GUID kGuildGuid;
	...
	kRet = m_kGuild.insert( std::make_pair(kNewGuid, pkNewGuild) );
	if( !kRet.second )
	{
		eRet = GCR_Failed;
		goto GCR_CREATE_RETURN;
