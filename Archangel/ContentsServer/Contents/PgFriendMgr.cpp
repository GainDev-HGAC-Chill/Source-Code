...
PgMessenger* PgFriendMgr::NewMessenger(BM::GUID const &rkGuid)
{
	...
	if(pkList)
	{
		std::pair<ContMessenger::iterator, bool> ibRet = m_kContMsgr.insert(std::make_pair(rkGuid, pkList));
		if(ibRet.second)
		{
			return pkList;
		}
...
//////////이 아래 실패 리턴은 __FAILED_OBJ_REMOVE 로 가시오.
	*pkFriendItem = kItem;
	...
	std::pair<ContFriendItem::iterator, bool> ibRet = m_kFriendList.insert(std::make_pair(pkFriendItem->CharGuid(), pkFriendItem));
	if(!ibRet.second)
	{
