...
bool PgMission_Rank::DoAction(ConPlayRankInfo& rkPlayerRankList,int& iClearTime_Top,int& iClearTime_Avg)
{
	...
	for(itr=rkPlayerRankList.begin();itr!=rkPlayerRankList.end();++itr)
	{
		...
		if(insert_itr!=m_kRankList.end())
		{
			...
			{
				itr->iNewRank = insert_itr->second.Standing();
			}
			else
			{
				itr->iNewRank = 0;
			...
			itr->iStanding = insert_itr->second.Standing();
			itr->kMemoGuid = insert_itr->second.GetID();
...
bool PgRankMgr::AddObj(SMissionKey const &rkMissionKey, PgMission_Rank_Base* pkRank)
{
	...
	std::pair<ConRank::iterator, bool> ret = m_kConRank.insert(std::make_pair(rkMissionKey, pkRank));
	return ret.second;
}
...
PgMission_Rank*	PgRankMgr::CreateMission_Rank(SMissionKey const &rkMissionKey)
{
	...
	std::pair<ConRank::iterator, bool> ibRet = m_kConRank.insert(std::make_pair(rkMissionKey, pkNew));
	if (!ibRet.second)
	{
...
void PgRankMgr::RecvPT_T_N_REQ_MISSION_RANKING(BM::Stream * const pkPacket)
{
	...
	if (pkPacket->Pop(elementCount))
	{
		static ConPlayRankInfo::value_type const kKeyForSize;

		if (pkPacket->RemainSize() >= elementCount * (kKeyForSize.min_size()))
		{
			while (elementCount--)
			{
				ConPlayRankInfo::value_type kKey;
				kKey.ReadFromPacket(*pkPacket);

				kCUserList.push_back(kKey);
			}
		}
	}
