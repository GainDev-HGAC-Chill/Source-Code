				...
				while ( itr != rkResult.vecArray.end() )
				{
					if ( iPackCount > 0 )
					{
						...
						if ( def_itr != pkContDefEmporia->end() )
						{
							...
							if ( pkPack )
							{
								std::pair<CONT_EMPORIA_PACK::iterator, bool> kPair = kContEmporiaPack.insert( std::make_pair( kEmporiaID, pkPack ) );
								if ( !kPair.second )
								{
						...
						if ( pack_itr != kContEmporiaPack.end() )
						{
							...
							if ( true == pack_itr->second->SetElement( static_cast<size_t>(kEmporiaKey.byGrade-1), kEmporia ) )
							{
								if ( !kOwner.IsEmpty() )
								{
									...
									std::pair<CONT_EMPORIA_FROMGUILD::iterator, bool> kPair = kContEmporiaFromGuild.insert( std::make_pair( kOwner.kGuildID, kElement ) );
									if ( !kPair.second )
									{
					...
					else if ( iChallengeBattleCount > 0 )
					{
						...
						{
							std::pair<CONT_TBL_EMPORIA_CHALLENGE_BATTLE2::iterator, bool> kPair = kContEmporiaChallengeBattle.insert( std::make_pair( kKey, CONT_TBL_EMPORIA_CHALLENGE_BATTLE2::mapped_type() ) );
							itr = kPair.first;
						}
					...
					else if ( iTournamentCount > 0 )
					{
						...
						{
							std::pair<CONT_TBL_EMPORIA_TOURNAMENT2::iterator, bool> kPair = kContEmporiaTournament.insert( std::make_pair( kKey, CONT_TBL_EMPORIA_TOURNAMENT2::mapped_type() ) );
							itr = kPair.first;
						}
					...
					else
					{
						...
						{
							std::pair<CONT_TBL_EM_MERCENARY::iterator, bool> kPair = kContEmporiaMercenary.insert( std::make_pair( kKey, CONT_TBL_EM_MERCENARY::mapped_type() ) );
							it_1 = kPair.first;
						}
						...
						if ( it_1 != kContEmporiaMercenary.end() )
						{
							...
							{
								std::pair<CONT_TBL_EM_MERCENARY_INDEX::iterator, bool> kPair = it_1->second.insert( CONT_TBL_EM_MERCENARY_INDEX::value_type() );
								it_2 = kPair.first;
								auto it2 = *it_2;
								it2.nIndex = kIndex.nIndex;
								it_1->second.erase(it_2);
								it_1->second.insert(it2);
							}
							...
							if ( it_2 != it_1->second.end() )
							{
								...
								{
									auto it2 = *it_2;
									std::pair<CONT_TBL_EM_MERCENARY_JOINGUILD::iterator, bool> kPair = it2.kContJoinGuild.insert( CONT_TBL_EM_MERCENARY_JOINGUILD::value_type() );
									it_3 = kPair.first;
									auto it3 = *it_3;
									it3.kJoinGuildID = kJoinGuildID;
									it2.kContJoinGuild.erase(it_3);
									it2.kContJoinGuild.insert(it3);
									it_1->second.erase(it_2);
									it_1->second.insert(it2);
								}
								...
								if ( it_3 != it_2->kContJoinGuild.end() )
								{
									auto it2 = *it_2;
									auto it3 = *it_3;
									it3.kContCharID.push_back(kCharID);
									it2.kContJoinGuild.erase(it_3);
									it2.kContJoinGuild.insert(it3);
									it_1->second.erase(it_2);
									it_1->second.insert(it2);
...
//			pkEmporia->ReleaseReserver();
	...
	for ( ; def_itr!=kTempDef.end() ; ++def_itr )
	{
		...
		if ( pkPack )
		{
			std::pair<CONT_EMPORIA_PACK::iterator, bool> kPair = rkContEmporiaPack.insert( std::make_pair( kElement.guidEmporiaID, pkPack ) );
