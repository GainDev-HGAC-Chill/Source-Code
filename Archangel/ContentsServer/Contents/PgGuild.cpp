...
bool PgGuild::Add(SGuildMemberInfo const& rkGuildMember)
{
	...
	std::pair<ContGuildMember::iterator, bool> ibRet = m_kMember.insert(std::make_pair(kTempInfo.kCharGuid, kTempInfo));
	if(ibRet.second)
	{
		//Success
		return true;
...
{
	std::pair<CONT_GUILD_INVENTORY_FIXING::iterator, bool> bSuccess;
	bSuccess = m_kContFixingItemPos.insert(rkPos);
	if( bSuccess.second )
	{
		return true;
	}
...
{
	std::pair<CONT_GUILD_INVENTORY_FIXING::iterator, bool> bSuccess;
	bSuccess = m_kContFixingItemPos.insert(rkCasterPos);
	if( !bSuccess.second )
	{
		return false;
	}
