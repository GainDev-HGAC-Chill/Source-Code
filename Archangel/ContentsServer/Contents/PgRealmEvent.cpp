...
bool PgRealmQuestEvent::ParseXml(TiXmlElement const* pkRoot)
{
	...
	while( pkSub )
	{
		...
		else if( 0 == strcmp("REWARD", szName) )
		{
			...
			{
				std::pair<CONT_TARGET_COUNT::iterator, bool> kRet = m_kContTargetCount.insert( std::make_pair(kTemp.iTargetCount, kTemp) );
				if( !kRet.second )
				{
					bRet = false;
				}
...
bool PgRealmEventMngImpl::ParseXmlImp(TiXmlElement const *pkElement )
{
	...
	while( NULL != pkElement )
	{
		...
		else if( 0 == strcmp(szElementName, "REALM_QUEST") )
		{
			...
			{
				std::pair<CONT_REALM_QUEST::iterator, bool> kRet = m_kContRealmQuest.insert( std::make_pair(kNewRealmQuest.RealmQuestID(), kNewRealmQuest) );
				if( !kRet.second )
				{
