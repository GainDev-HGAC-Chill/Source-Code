	...
	case EMPORIA_TERM_OF_CHALLENGE:
		{
			if ( kBattleID == m_kBattleID )
			{
				...
				{
					std::pair<CONT_EMPORIA_FROMGUILD::iterator, bool> kPair = kContGuild.insert( std::make_pair( battle_itr->kGuildInfo.kGuildID, SEmporiaHaveInfo(battle_itr->kGuildInfo, GetID())) );
...
HRESULT PgEmporiaPack< iNum >::AddChallenge( SEmporiaChallenge const *pkChallenge, bool const bSystem )
{
		...
		||	(EMPORIA_TERM_OF_CHALLENGE == m_iState) )
	{
		if ( pkChallenge )
		{
			std::pair<CONT_CHALLENGE_BATTLE::iterator, bool> kPair = m_kContChallenge.insert( std::make_pair( pkChallenge->kGuildInfo.kGuildID, *pkChallenge ) );
			if ( kPair.second )
			{
