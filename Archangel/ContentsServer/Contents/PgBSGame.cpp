	...
	bool Q_DQT_LOAD_BS_GAME(CEL::DB_RESULT &rkResult)
	{
		...
		{ // Item
			...
			for( int iCur = 0; iItemCount > iCur; ++iCur )
			{
				...
				std::pair<CONT_BS_ITEM::iterator, bool> kRet = kItemMap.insert( std::make_pair(kNewElement.iItemNo, kNewElement) );
				if( !kRet.second )
				{
		...
		{ // Game
			...
			for( int iCur = 0; iGameCount > iCur; ++iCur )
			{
				...
				int iTeamReawrdIdx = 0, iPtRewardIdx = 0, iLevelReawrdIdx = 0, iBonusRewardIdx = 0;
				...
				//std::pair<CONT_BS_GAME::iterator, bool> kRet = kBSGame.insert( std::make_pair(kNewElement.iGameIDX, PgBSGame(kNewElement)) );
				//if( kRet.second )
				{
...

				{
					...
					std::pair<CONT_DEF_BS_GEN_GROUND::iterator, bool> kPair = kContBSGenGround.insert(std::make_pair(kNewElement.iGroundNo, kNullData));
...
{
	std::pair<CONT_BS_GND_GAME::iterator, bool> kRet = m_kGndGame.insert( std::make_pair(rkBSGame.GameInfo().iGroundNo, rkBSGame.GameInfo().iGameIDX) );
	return kRet.second;
}
...
{
	std::pair<CONT_GUID_IDX::iterator, bool> kRet = m_kContGuidIdx.insert( std::make_pair(rkGuid, iGameIDX) );
	return kRet.second;
}
