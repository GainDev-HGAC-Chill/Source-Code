...
{
	if(argc!=2) {
		_tprintf(_T("USAGE: informer game-server-name-or-ip"));
		return(0);
	}
	...
	//TCHAR buf[32];
	TCHAR host[64] = _T("ncacn_ip_tcp");
	TCHAR ep[32] = _T("12345");

	if(RpcStringBindingCompose(NULL, (RPC_WSTR)host, (RPC_WSTR)argv[1], (RPC_WSTR)ep, NULL, &pszStringBinding)!=RPC_S_OK)
	{
		return -1;
	}

	if(RpcBindingFromStringBinding(pszStringBinding, &hrpc) != RPC_S_OK)
	{
		return -1;
	}
	...
	RpcTryExcept
	{
		int iErrorCode = GM_ERR_SUCCESS;

//		iErrorCode = KickPlayer(hrpc, _T("eric1"));
//		iErrorCode = ChangePassword(hrpc, _T("dtest3"), _T("1234"));		
//		iErrorCode = CreateAccount(hrpc, _T("raminer4"), _T("1234"));
		iErrorCode = FreezeAccount(hrpc, _T("dtest3"), 0);
//		iErrorCode = FreezeAccount(hrpc, _T("tangbohu003"), 100);
//		iErrorCode = ChangeBirthday(hrpc, _T("dtest3"), _T("800213"));
//		iErrorCode = AddPoint(hrpc, _T("temp"), _T("dtest3"), 100);
//		int iCash = GetPoint(hrpc, _T("dtest3"), &iErrorCode);
//		_tprintf(_T("return : %d"), iCash);

		_tprintf(_T("return : %hx"), iErrorCode);
		int i = 0;
		scanf("%d", i);
	}
	RpcExcept(EXCEPTION_EXECUTE_HANDLER)
	{
		unsigned short usExcept = RpcExceptionCode();
		_tprintf(_T("exception occured : %d"), usExcept);
		abort();
	}
	RpcEndExcept
...
2401 0x0961 네트워크 연결에 얼려 있는 파일이 있거나 처리되지 않은 요청... */