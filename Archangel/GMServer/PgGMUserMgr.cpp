...
bool PgGMUserMgr::WaitLogin(BM::Stream* const pkPacket )
{
	...
	*pData = kGmAuth;
	...
	std::pair<CONT_GM_AUTH_DATA_ID::iterator, bool> kPair = m_kGmUserWaitLogin.insert(std::make_pair(kGmAuth.ID(), pData));
	if(!kPair.second)	
	{
		return false;
	}
...
//			kGmAuth.SetName(kLogin.szName);
			...
			*pData = kGmAuth;
			...
			std::pair<CONT_GM_AUTH_DATA::iterator, bool> kPair = m_kGMUserLogined.insert(std::make_pair(kGmAuth.kMemberGuid, pData));
			if(kPair.second)
			{
				iErr = GE_SUCCESS;
			} 
