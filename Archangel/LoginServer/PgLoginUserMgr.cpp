...
bool PgLoginUserMgr::InsertAuth( CEL::CSession_Base const * pkSession, SAuthInfo &kAuthInfo )
{
	...
	std::pair<CONT_LOGIN_USER::iterator, bool> ret = m_kContTryLoginUser.insert( std::make_pair( pkSession->SessionKey(), kAuthInfo ) );
	return ret.second;
}
...
*/
	...
	*pkGmInfo = kGM;
	...
	std::pair<CONT_GM_AUTH_DATA_ID::iterator, bool> kPair = m_kContGmWaitLogin.insert(std::make_pair(kGM.ID(), pkGmInfo));
	if(!kPair.second)
	{
...
void PgLoginUserMgr::ProcessGmLogin(BM::Stream * const pkPacket)
{
	...
	if(iErrorCode == GE_SUCCESS)
	{
		...
		*pkData = kGmInfo;
		std::pair<CONT_GM_AUTH_DATA::iterator, bool> kPair =  m_kContGmLogined.insert(std::make_pair(kGmInfo.kMemberGuid, pkData));
		if(kPair.second)
		{
...
BM::GUID PgLoginUserMgr::InsertAutoLoginWait( SWaitAutoLogin const &kWAL )
{
	...
	std::pair<CONT_WAIT_AUTOLOGIN_USER::iterator, bool> kPair = m_kContWaitAutoLogin.insert( std::make_pair( BM::GUID::Create(), kWAL ) );
