...
void PgGalaServer::RecvPT_IM_CN_REQ_LOGIN_AUTH_GALA(CEL::CSession_Base *pkSession, BM::Stream * const pkPacket)
{
	...
	std::pair<CONT_REQUEST::iterator, bool> ibRet = m_kUserByRequest.insert(std::make_pair(pkNew->kAuthInfo.kRequestGuid, pkNew));
	...
	std::pair<CONT_ID::iterator, bool> ibRet2 = m_kUserByID.insert(std::make_pair(pkNew->kAuthInfo.strID, pkNew));
...
{
	std::pair<CONT_GTDX_REQUEST::iterator, bool> ibRet = m_kGTDXRequest.insert(std::make_pair(strID, SGTDXRequest(strID, dwRequestKey, eType, wReqPacket, kFromSI, pkAddonPacket)));
	return ibRet.second;
}
