...
void PgNcServer::RecvPT_IM_CN_REQ_LOGIN_AUTH_NC(CEL::CSession_Base *pkSession, BM::Stream * const pkPacket)
{
	...
	std::pair<CONT_USER_BY_GUSID::iterator, bool> pairGusIdResult = m_kContUserByGusId.insert(std::make_pair(std::string(reinterpret_cast<char*>(abyGusID)), pkNew));
	if( pairGusIdResult.second )
	{
		INFO_LOG(BM::LOG_LV7, __FL__ << L"m_kContUserByGusId.insert success");
		std::pair<CONT_USER_BY_USERID::iterator, bool> pairUserIdResult = m_kContUserByUserId.insert(std::make_pair(kRecv.iUID, pkNew));
