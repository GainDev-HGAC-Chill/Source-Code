...
CEL::SESSION_KEY kCashSessionKey;

bool CALLBACK OnF2( unsigned short const &rkInputKey)
{
	std::cout<< __FUNCTIONW__ << std::endl;
	
	CEL::ADDR_INFO kAddr(L"59.128.93.158", 9006);

	kCashSessionKey.WorkerGuid(g_kProcessCfg.ImmigrationConnector());
//	BM::GUID kConnectorGuid = g_kProcessCfg.ImmigrationConnector();
//	g_kCoreCenter.Connect(kAddr, kCashSessionKey, kConnectorGuid);
	g_kCoreCenter.Connect(kAddr, kCashSessionKey);
